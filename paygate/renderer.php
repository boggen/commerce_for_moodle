...
/**
 * This is the main renderer for the paygate section.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_paygate_renderer extends plugin_renderer_base {
    ...
    public function render_course_paymentgateway_users_table(course_paymentgateway_users_table $table,
        ...
        $buttons = $table->get_manual_paygate_buttons();
        $buttonhtml = '';
        if (count($buttons) > 0) {
            $buttonhtml .= html_writer::start_tag('div', array('class' => 'paygate_user_buttons'));
        ...
        if ($table->has_bulk_user_paymentgateway_operations()) {
            $content .= html_writer::start_tag('form', array('action' => new moodle_url('/paygate/bulkchange.php'), 'method' => 'post'));
            ...
            $content .= html_writer::tag('option', get_string('withselectedusers', 'paygate'), array('value' => ''));
            $options = array('' => get_string('withselectedusers', 'paygate'));
    ...
    /**
     * Renderers the paygate_user_button.
     *
     * @param paygate_user_button $button
     * @return string XHTML
     */
    protected function render_paygate_user_button(paygate_user_button $button) {
    ...
    protected function render_course_paymentgateway_other_users_table(course_paymentgateway_other_users_table $table) {
        ...
        $content .= html_writer::tag('div', get_string('otheruserdesc', 'paygate'), array('class'=>'otherusersdesc'));
        $content .= $this->output->render($table->get_paging_bar());
        $content .= html_writer::table($table);
        $content .= $this->output->render($table->get_paging_bar());
        $searchbutton = $table->get_user_search_button();
    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        $iconpaygateremove = $this->output->pix_url('t/delete');
        ...
        foreach ($roles as $roleid=>$role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $icon = html_writer::empty_tag('img', array('alt'=>$strunassign, 'src'=>$iconpaygateremove));
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach($groups as $groupid=>$name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                $icon = html_writer::empty_tag('img', array('alt'=>get_string('removefromgroup', 'group', $name), 'src'=>$iconpaygateremove));
...
class course_paymentgateway_table extends html_table implements renderable {
    ...
    /**
     * The users paygateled in this course
     * @var array
     */
    ...
    public function __construct(course_paymentgateway_manager $manager) {
        ...
        if ($plugin and paygate_is_enabled($plugin->get_name())) {
            $this->bulkoperations = $plugin->get_bulk_operations($manager);
        }
    }

    /**
     * Returns an array of paygate_user_buttons that are created by the different
     * paymentgateway plugins available.
     *
     * @return array
     */
    public function get_manual_paygate_buttons() {
        return $this->manager->get_manual_paygate_buttons();
    }
    ...
    public function initialise_javascript() {
        if (has_capability('moodle/role:assign', $this->manager->get_context())) {
            ...
            $modules = array('moodle-paygate-rolemanager', 'moodle-paygate-rolemanager-skin');
            $function = 'M.paygate.rolemanager.init';
            $arguments = array(
                'containerId'=>$this->id,
...
/**
 * Table control used for paygateled users
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
 ...
 *
 * Other users are users who have roles but are not paygateled.
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
    ...
    public function get_user_search_button() {
        ...
        if ($count == 1) {
            $this->manager->get_moodlepage()->requires->strings_for_js(array(
                    ...
                    'startdatetoday',
                    'durationdays',
                    'paygateperiod'), 'paygate');
            $this->manager->get_moodlepage()->requires->string_for_js('assignrole', 'role');

            $modules = array('moodle-paygate-otherusersmanager', 'moodle-paygate-otherusersmanager-skin');
            $function = 'M.paygate.otherusersmanager.init';
            $arguments = array(
                'courseId'=> $this->manager->get_course()->id,
                'ajaxUrl' => '/paygate/ajax.php',
