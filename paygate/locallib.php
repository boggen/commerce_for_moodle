 ...
 * with the functions that exist in paygatelib.php in relation to a single course.
 *
...
class course_paymentgateway_manager {
    ...
    /**
     * The total number of users paygateled in the course
     * Populated by course_paymentgateway_manager::get_total_users
     * @var int
     */
    protected $totalusers = null;
    /**
     * An array of users currently paygateled in the course
    ...
    protected function get_filter_sql() {
        ...
        // Status condition.
        if ($this->statusfilter === PAYGATE_USER_ACTIVE) {
            $sql .= " AND ue.status = :active AND e.status = :enabled AND ue.timestart < :now1
                    AND (ue.timeend = 0 OR ue.timeend > :now2)";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('enabled' => PAYGATE_INSTANCE_ENABLED,
                             'active' => PAYGATE_USER_ACTIVE,
                             'now1' => $now,
                             'now2' => $now);
        } else if ($this->statusfilter === PAYGATE_USER_SUSPENDED) {
            $sql .= " AND (ue.status = :inactive OR e.status = :disabled OR ue.timestart > :now1
                    OR (ue.timeend <> 0 AND ue.timeend < :now2))";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('disabled' => PAYGATE_INSTANCE_DISABLED,
                             'inactive' => PAYGATE_USER_SUSPENDED,
                             'now1' => $now,
                             'now2' => $now);
        }
...
// ====================================================================================================
    ...
    public function get_paymentgateway_instances() {
        if ($this->_instances === null) {
            $this->_instances = paygate_get_instances($this->course->id, false);
        }
    ...
    public function get_paymentgateway_instance_names() {
        if ($this->_inames === null) {
            ...
            foreach ($instances as $key=>$instance) {
                ...
                $this->_inames[$key] = $plugins[$instance->paygate]->get_instance_name($instance);
            }
        }
        return $this->_inames;
    }
     ...
     *
     * @param bool $onlyenabled return only enabled paygate plugins
     * @return array
     */

    public function get_paymentgateway_plugins($onlyenabled = true) {
        if ($this->_plugins === null) {
            $this->_plugins = paygate_get_plugins(true);
        }
        ...
        if ($this->_allplugins === null) {
            ...
            foreach (paygate_get_plugins(false) as $name=>$plugin) {
    ...
    /**
     * Unpaygates a user from the course given the users ue entry
     *
     * @global moodle_database $DB
     * @param stdClass $ue
     * @return bool
    ...
    public function unpaygate_user($ue) {
        global $DB;
        list ($instance, $plugin) = $this->get_user_paymentgateway_components($ue);
        if ($instance && $plugin && $plugin->allow_unpaygate_user($instance, $ue) && has_capability("paygate/$instance->paygate:unpaygate", $this->context)) {
            $plugin->unpaygate_user($instance, $ue->userid);
            return true;
        }
        return false;
    }
    ...
    public function get_user_paymentgateway_components($userpaymentgateway) {
        ...
        $instances = $this->get_paymentgateway_instances();
        $plugins = $this->get_paymentgateway_plugins(false);
        if (!$userpaymentgateway || !isset($instances[$userpaymentgateway->paygateid])) {
            return array(false, false);
        }
        $instance = $instances[$userpaymentgateway->paygateid];
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        foreach ($ras as $ra) {
            if ($ra->component) {
                if (strpos($ra->component, 'paygate_') !== 0) {
                    continue;
                }
                if (!$plugin = paygate_get_plugin(substr($ra->component, 6))) {
                    continue;
                }
...
//============================================================================
    public function edit_paymentgateway($userpaymentgateway, $data) {
        ...
        //Already checked in /paygate/users.php but checking again in case this function is called from elsewhere
        list($instance, $plugin) = $this->get_user_paymentgateway_components($userpaymentgateway);
        if ($instance && $plugin && $plugin->allow_manage($instance) && has_capability("paygate/$instance->paygate:manage", $this->context)) {
            if (!isset($data->status)) {
                $data->status = $userpaymentgateway->status;
            }
            $plugin->update_user_paygate($instance, $userpaymentgateway->userid, $data->status, $data->timestart, $data->timeend);
            return true;
        }
        return false;
    }
...
//============================================================================
     ...
     * course but have not been paygateled.
     *
     * @param core_paygate_renderer $renderer
    ...
    public function get_other_users_for_display(core_paygate_renderer $renderer, moodle_url $pageurl, $sort, $direction, $page, $perpage) {
        ...
        foreach ($userroles as $userrole) {
            ...
            if ($contextid == $this->context->id) {
                ...
                if ($userrole->component) {
                    $changeable = false;
                    if (strpos($userrole->component, 'paygate_') === 0) {
                ...
                $roletext = get_string('rolefromthiscourse', 'paygate', $a);
            } else {
                $changeable = false;
                switch ($userrole->contextlevel) {
                    case CONTEXT_COURSE :
                        // Meta course
                        $roletext = get_string('rolefrommetacourse', 'paygate', $a);
                        break;
                    case CONTEXT_COURSECAT :
                        $roletext = get_string('rolefromcategory', 'paygate', $a);
                        break;
                    case CONTEXT_SYSTEM:
                    default:
                        $roletext = get_string('rolefromsystem', 'paygate', $a);
                        break;
                }
            }
    ...
    public function get_manual_paygate_buttons() {
        $plugins = $this->get_paymentgateway_plugins(true); // Skip disabled plugins.
        $buttons = array();
        foreach ($plugins as $plugin) {
            $newbutton = $plugin->get_manual_paygate_button($this);
            if (is_array($newbutton)) {
                $buttons += $newbutton;
            } else if ($newbutton instanceof paygate_user_button) {
                $buttons[] = $newbutton;
            }
        }
        return $buttons;
...
//============================================================================
    ...
    public function get_filtered_paymentgateway_plugin() {
        ...
        return $plugins[$instance->paygate];
    }


/**
 * A button that is used to paygate users in a course
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
//============================================================================
//============================================================================
class paygate_user_button extends single_button {
    ...
    /**
     * Initialises the new paygate_user_button
     *
     * @staticvar int $count The number of paygate user buttons already created
    ...
    public function __construct(moodle_url $url, $label, $method = 'post') {
        ...
        $this->class = 'singlebutton paygateusersbutton';
        $this->formid = 'paygateusersbutton-'.$count;
    }
...
class paygate_ajax_exception extends moodle_exception {
    /**
     * Constructor
    ...
    public function __construct($errorcode, $link = '', $a = NULL, $debuginfo = null) {
        parent::__construct($errorcode, 'paygate', $link, $a, $debuginfo);
    }
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class paygate_bulk_paymentgateway_operation {
     ...
     */
    public function __construct(course_paymentgateway_manager $manager, paygate_plugin $plugin = null) {
     ...
     * @return paygate_bulk_paymentgateway_change_form|moodleform|false
     */
