 ...
 *
 * This api is mostly read only, the actual paygate and unpaygate
 * support is in each paygate plugin.
 *
 ...
 * @since Moodle 2.2
 */
class core_paygate_external extends external_api {
     ...
     * @since Moodle 2.4
     */
    public static function get_paygateled_users_with_capability_parameters() {
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) only users with active paymentgateways. Requires \'moodle/course:paygatereview\' .
     ...
     * a list of the users that are paygateled in the course and have that capability are returned.
     *
     ...
     * @since  Moodle 2.4
     */
    public static function get_paygateled_users_with_capability($coursecapabilities, $options) {
        global $CFG, $DB;
        ...
        $params = self::validate_parameters(self::get_paygateled_users_with_capability_parameters(),
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            // The course:paygateereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('moodle/course:paygatereview', $coursecontext);
            }
            ...
            foreach ($coursecapability['capabilities'] as $capability) {
                ...
                list($paygateledsql, $paygateledparams) = get_paygateled_sql($coursecontext, $capability, $groupid, $onlyactive);

                $sql = "SELECT u.* FROM {user} u WHERE u.id IN ($paygateledsql) ORDER BY u.id ASC";

                $paygateledusers = $DB->get_recordset_sql($sql, $paygateledparams, $limitfrom, $limitnumber);
                $users = array();
                foreach ($paygateledusers as $courseuser) {
                    if ($userdetails = user_get_user_details($courseuser, $course, $userfields)) {
                        $users[] = $userdetails;
                    }
                }
                $paygateledusers->close();
     ...
     * @since Moodle 2.4
     */
    public static function get_paygateled_users_with_capability_returns() {
        return  new external_multiple_structure( new external_single_structure (
                ...
                array(
                    ...
                    'paygateledcourses' => new external_multiple_structure(
                        new external_single_structure(
                            array(
                    ...
                    ), 'Courses where the user is paygateled - limited by which courses the user is able to see', VALUE_OPTIONAL)
                )
                        ), 'List of users that are paygateled in the course and have the specified capability'),
                    )
                )
            );
    }
    ...
    /**
     * Get list of courses user is paygateled in (only active paymentgateways are returned).
    ...
    public static function get_users_courses($userid) {
        ...
        $courses = paygate_get_users_courses($params['userid'], true, 'id, shortname, fullname, idnumber, visible');
        $result = array();
        ...
        foreach ($courses as $course) {
            ...
            try {
                ...
                // current user can not access this course, sorry we can not disclose who is paygateled in this course!
                continue;
            }
            ...
            list($paygateledsqlselect, $paygateledparams) = get_paygateled_sql($context);
            $paygateledsql = "SELECT COUNT('x') FROM ($paygateledsqlselect) paygateleduserids";
            $paygateledusercount = $DB->count_records_sql($paygateledsql, $paygateledparams);

            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'paygateledusercount'=>$paygateledusercount);
        }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'paygateledusercount' => new external_value(PARAM_INT, 'Number of paygateled users in this course'),
     ...
     * @return external_function_parameters
     */
    public static function get_paygateled_users_parameters() {
        return new external_function_parameters(
            array(
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) return only users with active paymentgateways and matching time restrictions. This option requires \'moodle/course:paygatereview\' on the course context.
                            * userfields (\'string, string, ...\') return only the values of these user fields.
                            * limitfrom (integer) sql limit from.
     ...
     */
    public static function get_paygateled_users($courseid, $options = array()) {
        global $CFG, $USER, $DB;
        require_once($CFG->dirroot . "/user/lib.php");

        $params = self::validate_parameters(
            self::get_paygateled_users_parameters(),
            array(
                'courseid'=>$courseid,
                'options'=>$options
            )
        ...
        // to overwrite this option, you need course:paygateereview permission
        if ($onlyactive) {
            require_capability('moodle/course:paygatereview', $coursecontext);
        }

        list($paygateledsql, $paygateledparams) = get_paygateled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        $ctxselect = ', ' . context_helper::get_preload_record_columns_sql('ctx');
        $ctxjoin = "LEFT JOIN {context} ctx ON (ctx.instanceid = u.id AND ctx.contextlevel = :contextlevel)";
        $paygateledparams['contextlevel'] = CONTEXT_USER;
        $sql = "SELECT u.* $ctxselect
                  FROM {user} u $ctxjoin
                 WHERE u.id IN ($paygateledsql)
                 ORDER BY u.id ASC";
        $paygateledusers = $DB->get_recordset_sql($sql, $paygateledparams, $limitfrom, $limitnumber);
        $users = array();
        foreach ($paygateledusers as $user) {
     ...
     * @return external_description
     */
    public static function get_paygateled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
    ...
    public static function get_course_paymentgateway_methods($courseid) {
        ...
        $result = array();
        $paygateinstances = paygate_get_instances($params['courseid'], true);
        foreach ($paygateinstances as $paygateinstance) {
            if ($paygateplugin = paygate_get_plugin($paygateinstance->paygate)) {
                if ($instanceinfo = $paygateplugin->get_paygate_info($paygateinstance)) {
...
/**
 * Deprecated paygate and role external functions
 *
 * @package    core_paygate
 * @copyright  2010 Jerome Mouneyrac
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 ...
 * @see core_paygate_external
 * @see core_role_external
 */
class moodle_paygate_external extends external_api {
     ...
     * @see core_paygate_external::get_paygateled_users_parameters()
     */
    public static function get_paygateled_users_parameters() {
        return new external_function_parameters(
            array(
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_paygate_external::get_paygateled_users()
     */
    public static function get_paygateled_users($courseid, $withcapability = null, $groupid = null, $onlyactive = false) {
        global $DB, $CFG, $USER;

        // Do basic automatic PARAM checks on incoming data, using params description
        // If any problems are found then exceptions are thrown with helpful error messages
        $params = self::validate_parameters(self::get_paygateled_users_parameters(), array(
            'courseid'=>$courseid,
        ...
        list($sqlparams, $params) =  get_paygateled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        ...
        $paygateledusers = $DB->get_records_sql($sql, $params);
        $result = array();
        $isadmin = is_siteadmin($USER);
        $canviewfullnames = has_capability('moodle/site:viewfullnames', $context);
        foreach ($paygateledusers as $paygateleduser) {
            $profilimgurl = moodle_url::make_pluginfile_url($paygateleduser->usercontextid, 'user', 'icon', NULL, '/', 'f1');
            $profilimgurlsmall = moodle_url::make_pluginfile_url($paygateleduser->usercontextid, 'user', 'icon', NULL, '/', 'f2');
            $resultuser = array(
                'courseid' => $paygateleduser->courseid,
                'userid' => $paygateleduser->userid,
                'fullname' => fullname($paygateleduser),
                'profileimgurl' => $profilimgurl->out(false),
                'profileimgurlsmall' => $profilimgurlsmall->out(false)
            );
            // check if we can return username
            if ($isadmin) {
                $resultuser['username'] = $paygateleduser->username;
            }
            // check if we can return first and last name
            if ($isadmin or $canviewfullnames) {
                $resultuser['firstname'] = $paygateleduser->firstname;
                $resultuser['lastname'] = $paygateleduser->lastname;
            }
            $result[] = $resultuser;
        }
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_paygate_external::get_paygateled_users_returns()
     */
    public static function get_paygateled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_paygate_external::get_users_courses_parameters()
     */
    public static function get_users_courses_parameters() {
        return core_paygate_external::get_users_courses_parameters();
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see use core_paygate_external::get_users_courses()
     */
    public static function get_users_courses($userid) {
        return core_paygate_external::get_users_courses($userid);
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_paygate_external::get_users_courses_returns()
     */
    public static function get_users_courses_returns() {
        return core_paygate_external::get_users_courses_returns();
    }
