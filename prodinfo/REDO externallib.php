 ...
 *
 * This api is mostly read only, the actual prodinfo and unprodinfo
 * support is in each prodinfo plugin.
 *
 ...
 * @since Moodle 2.2
 */
class core_prodinfo_external extends external_api {
     ...
     * @since Moodle 2.4
     */
    public static function get_prodinfoled_users_with_capability_parameters() {
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) only users with active paymentgateways. Requires \'moodle/course:prodinforeview\' .
     ...
     * a list of the users that are prodinfoled in the course and have that capability are returned.
     *
     ...
     * @since  Moodle 2.4
     */
    public static function get_prodinfoled_users_with_capability($coursecapabilities, $options) {
        global $CFG, $DB;
        ...
        $params = self::validate_parameters(self::get_prodinfoled_users_with_capability_parameters(),
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            // The course:prodinfoereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('moodle/course:prodinforeview', $coursecontext);
            }
            ...
            foreach ($coursecapability['capabilities'] as $capability) {
                ...
                list($prodinfoledsql, $prodinfoledparams) = get_prodinfoled_sql($coursecontext, $capability, $groupid, $onlyactive);

                $sql = "SELECT u.* FROM {user} u WHERE u.id IN ($prodinfoledsql) ORDER BY u.id ASC";

                $prodinfoledusers = $DB->get_recordset_sql($sql, $prodinfoledparams, $limitfrom, $limitnumber);
                $users = array();
                foreach ($prodinfoledusers as $courseuser) {
                    if ($userdetails = user_get_user_details($courseuser, $course, $userfields)) {
                        $users[] = $userdetails;
                    }
                }
                $prodinfoledusers->close();
     ...
     * @since Moodle 2.4
     */
    public static function get_prodinfoled_users_with_capability_returns() {
        return  new external_multiple_structure( new external_single_structure (
                ...
                array(
                    ...
                    'prodinfoledcourses' => new external_multiple_structure(
                        new external_single_structure(
                            array(
                    ...
                    ), 'Courses where the user is prodinfoled - limited by which courses the user is able to see', VALUE_OPTIONAL)
                )
                        ), 'List of users that are prodinfoled in the course and have the specified capability'),
                    )
                )
            );
    }
    ...
    /**
     * Get list of courses user is prodinfoled in (only active paymentgateways are returned).
    ...
    public static function get_users_courses($userid) {
        ...
        $courses = prodinfo_get_users_courses($params['userid'], true, 'id, shortname, fullname, idnumber, visible');
        $result = array();
        ...
        foreach ($courses as $course) {
            ...
            try {
                ...
                // current user can not access this course, sorry we can not disclose who is prodinfoled in this course!
                continue;
            }
            ...
            list($prodinfoledsqlselect, $prodinfoledparams) = get_prodinfoled_sql($context);
            $prodinfoledsql = "SELECT COUNT('x') FROM ($prodinfoledsqlselect) prodinfoleduserids";
            $prodinfoledusercount = $DB->count_records_sql($prodinfoledsql, $prodinfoledparams);

            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'prodinfoledusercount'=>$prodinfoledusercount);
        }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'prodinfoledusercount' => new external_value(PARAM_INT, 'Number of prodinfoled users in this course'),
     ...
     * @return external_function_parameters
     */
    public static function get_prodinfoled_users_parameters() {
        return new external_function_parameters(
            array(
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) return only users with active paymentgateways and matching time restrictions. This option requires \'moodle/course:prodinforeview\' on the course context.
                            * userfields (\'string, string, ...\') return only the values of these user fields.
                            * limitfrom (integer) sql limit from.
     ...
     */
    public static function get_prodinfoled_users($courseid, $options = array()) {
        global $CFG, $USER, $DB;
        require_once($CFG->dirroot . "/user/lib.php");

        $params = self::validate_parameters(
            self::get_prodinfoled_users_parameters(),
            array(
                'courseid'=>$courseid,
                'options'=>$options
            )
        ...
        // to overwrite this option, you need course:prodinfoereview permission
        if ($onlyactive) {
            require_capability('moodle/course:prodinforeview', $coursecontext);
        }

        list($prodinfoledsql, $prodinfoledparams) = get_prodinfoled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        $ctxselect = ', ' . context_helper::get_preload_record_columns_sql('ctx');
        $ctxjoin = "LEFT JOIN {context} ctx ON (ctx.instanceid = u.id AND ctx.contextlevel = :contextlevel)";
        $prodinfoledparams['contextlevel'] = CONTEXT_USER;
        $sql = "SELECT u.* $ctxselect
                  FROM {user} u $ctxjoin
                 WHERE u.id IN ($prodinfoledsql)
                 ORDER BY u.id ASC";
        $prodinfoledusers = $DB->get_recordset_sql($sql, $prodinfoledparams, $limitfrom, $limitnumber);
        $users = array();
        foreach ($prodinfoledusers as $user) {
     ...
     * @return external_description
     */
    public static function get_prodinfoled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
    ...
    /**
     * Returns description of get_course_paymentgateway_methods() parameters
     *
     * @return external_function_parameters
     */
    public static function get_course_paymentgateway_methods_parameters() {
        return new external_function_parameters(
            array(
                'courseid' => new external_value(PARAM_INT, 'Course id')
            )
    ...
    /**
     * Get list of active course paymentgateway methods for current user.
     *
     * @param int $courseid
     * @return array of course paymentgateway methods
     */
    public static function get_course_paymentgateway_methods($courseid) {

        $params = self::validate_parameters(self::get_course_paymentgateway_methods_parameters(), array('courseid' => $courseid));
        ...
        $result = array();
        $prodinfoinstances = prodinfo_get_instances($params['courseid'], true);
        foreach ($prodinfoinstances as $prodinfoinstance) {
            if ($prodinfoplugin = prodinfo_get_plugin($prodinfoinstance->prodinfo)) {
                if ($instanceinfo = $prodinfoplugin->get_prodinfo_info($prodinfoinstance)) {
    ...
    /**
     * Returns description of get_course_paymentgateway_methods() result value
     *
     * @return external_description
     */
    public static function get_course_paymentgateway_methods_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'id of course paymentgateway instance'),
                    'courseid' => new external_value(PARAM_INT, 'id of course'),
                    'type' => new external_value(PARAM_PLUGIN, 'type of paymentgateway plugin'),
                    'name' => new external_value(PARAM_RAW, 'name of paymentgateway plugin'),
                    'status' => new external_value(PARAM_RAW, 'status of paymentgateway plugin'),
...
class core_role_external extends external_api {
    ...
    public static function assign_roles($assignments) {
        ...
        foreach ($params['assignments'] as $assignment) {
            ...
            // Ensure the current user is allowed to run this function in the paymentgateway context.
            self::validate_context($context);
...
/**
 * Deprecated prodinfo and role external functions
 *
 * @package    core_prodinfo
 * @copyright  2010 Jerome Mouneyrac
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 ...
 * @see core_prodinfo_external
 * @see core_role_external
 */
class moodle_prodinfo_external extends external_api {
     ...
     * @see core_prodinfo_external::get_prodinfoled_users_parameters()
     */
    public static function get_prodinfoled_users_parameters() {
        return new external_function_parameters(
            array(
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_prodinfo_external::get_prodinfoled_users()
     */
    public static function get_prodinfoled_users($courseid, $withcapability = null, $groupid = null, $onlyactive = false) {
        global $DB, $CFG, $USER;

        // Do basic automatic PARAM checks on incoming data, using params description
        // If any problems are found then exceptions are thrown with helpful error messages
        $params = self::validate_parameters(self::get_prodinfoled_users_parameters(), array(
            'courseid'=>$courseid,
        ...
        list($sqlparams, $params) =  get_prodinfoled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        ...
        $prodinfoledusers = $DB->get_records_sql($sql, $params);
        $result = array();
        $isadmin = is_siteadmin($USER);
        $canviewfullnames = has_capability('moodle/site:viewfullnames', $context);
        foreach ($prodinfoledusers as $prodinfoleduser) {
            $profilimgurl = moodle_url::make_pluginfile_url($prodinfoleduser->usercontextid, 'user', 'icon', NULL, '/', 'f1');
            $profilimgurlsmall = moodle_url::make_pluginfile_url($prodinfoleduser->usercontextid, 'user', 'icon', NULL, '/', 'f2');
            $resultuser = array(
                'courseid' => $prodinfoleduser->courseid,
                'userid' => $prodinfoleduser->userid,
                'fullname' => fullname($prodinfoleduser),
                'profileimgurl' => $profilimgurl->out(false),
                'profileimgurlsmall' => $profilimgurlsmall->out(false)
            );
            // check if we can return username
            if ($isadmin) {
                $resultuser['username'] = $prodinfoleduser->username;
            }
            // check if we can return first and last name
            if ($isadmin or $canviewfullnames) {
                $resultuser['firstname'] = $prodinfoleduser->firstname;
                $resultuser['lastname'] = $prodinfoleduser->lastname;
            }
            $result[] = $resultuser;
        }
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_prodinfo_external::get_prodinfoled_users_returns()
     */
    public static function get_prodinfoled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_prodinfo_external::get_users_courses_parameters()
     */
    public static function get_users_courses_parameters() {
        return core_prodinfo_external::get_users_courses_parameters();
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see use core_prodinfo_external::get_users_courses()
     */
    public static function get_users_courses($userid) {
        return core_prodinfo_external::get_users_courses($userid);
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_prodinfo_external::get_users_courses_returns()
     */
    public static function get_users_courses_returns() {
        return core_prodinfo_external::get_users_courses_returns();
    }
