...
/**
 * This file contains the course_paymentgateway_manager class which is used to interface
 * with the functions that exist in prodinfolib.php in relation to a single course.
 *
...
/**
 * This class provides a targeted tied together means of interfacing the paymentgateway
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_paymentgateway_manager {
    ...
    protected $context;
    /**
     * The course we are managing paymentgateways for
     * @var stdClass
     */
    protected $course = null;
    /**
     * Limits the focus of the manager to one paymentgateway plugin instance
     * @var string
     */
    ...
    /**
     * The total number of users prodinfoled in the course
     * Populated by course_paymentgateway_manager::get_total_users
     * @var int
     */
    protected $totalusers = null;
    /**
     * An array of users currently prodinfoled in the course
     * Populated by course_paymentgateway_manager::get_users
     * @var array
     */
     ...
     * been prodinfoled in the course
     * @var array
     */
    ...
    /**
     * Constructs the course paymentgateway manager
     *
     ...
     * @param int $statusfilter if not -1, filter users with active/inactive paymentgateway.
     */
    ...
    /**
     * Returns the total number of prodinfoled users in the course.
     *
     * If a filter was specificed this will be the total number of users prodinfoled
    ...
    public function get_total_users() {
        ...
        if ($this->totalusers === null) {
            ...
            $sqltotal = "SELECT COUNT(DISTINCT u.id)
                           FROM {user} u
                           JOIN {user_paymentgateways} ue ON (ue.userid = u.id  AND ue.prodinfoid $instancessql)
        ...
        if ($this->totalotherusers === null) {
            ...
            $sql = "SELECT COUNT(DISTINCT u.id)
                 ...
                 LEFT JOIN (
                        ...
                        LEFT JOIN {prodinfo} e ON e.id=ue.prodinfoid
    ...
    /**
     * Gets all of the users prodinfoled in this course.
     *
     * If a filter was specified this will be the users who were prodinfoled
    ...
    protected function get_filter_sql() {
        ...
        // Status condition.
        if ($this->statusfilter === prodinfo_USER_ACTIVE) {
            $sql .= " AND ue.status = :active AND e.status = :enabled AND ue.timestart < :now1
                    AND (ue.timeend = 0 OR ue.timeend > :now2)";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('enabled' => prodinfo_INSTANCE_ENABLED,
                             'active' => prodinfo_USER_ACTIVE,
                             'now1' => $now,
                             'now2' => $now);
        } else if ($this->statusfilter === prodinfo_USER_SUSPENDED) {
            $sql .= " AND (ue.status = :inactive OR e.status = :disabled OR ue.timestart > :now1
                    OR (ue.timeend <> 0 AND ue.timeend < :now2))";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('disabled' => prodinfo_INSTANCE_DISABLED,
                             'inactive' => prodinfo_USER_SUSPENDED,
                             'now1' => $now,
                             'now2' => $now);
        }
     ...
     * within this course but who have not been prodinfoled in the course
     *
     * @global moodle_database $DB
     * @param string $sort
     ...
     * @param int $addedpaymentgateway number of users added to paymentgateway.
     * @return array with two elememts:
     *      int total number of users matching the search.
     *      array of user objects returned by the query.
     */

    protected function execute_search_queries($search, $fields, $countfields, $sql, array $params, $page, $perpage, $addedpaymentgateway=0) {
        global $DB, $CFG;
        ...
        $availableusers = $DB->get_records_sql($fields . $sql . $order,
                array_merge($params, $sortparams), ($page*$perpage) - $addedpaymentgateway, $perpage);

        return array('totalusers' => $totalusers, 'users' => $availableusers);
    }

    /**
     * Gets an array of the users that can be prodinfoled in this course.
     *
     * @global moodle_database $DB
     * @param int $prodinfoid
     * @param string $search
     * @param bool $searchanywhere
     * @param int $page Defaults to 0
     * @param int $perpage Defaults to 25
     * @param int $addedpaymentgateway Defaults to 0
     * @return array Array(totalusers => int, users => array)
     */

    public function get_potential_users($prodinfoid, $search='', $searchanywhere=false, $page=0, $perpage=25, $addedpaymentgateway=0) {
        global $DB;
        ...
        $sql = " FROM {user} u
            LEFT JOIN {user_paymentgateways} ue ON (ue.userid = u.id AND ue.prodinfoid = :prodinfoid)
                WHERE $wherecondition
                      AND ue.id IS NULL";
        $params['prodinfoid'] = $prodinfoid;

        return $this->execute_search_queries($search, $fields, $countfields, $sql, $params, $page, $perpage, $addedpaymentgateway);
    }
...
// ====================================================================================================
    protected function get_instance_sql() {
        ...
        if ($this->_instancessql === null) {
            $instances = $this->get_paymentgateway_instances();
            $filter = $this->get_paymentgateway_filter();
            if ($filter && array_key_exists($filter, $instances)) {
                ...
                $filter = 0;
                if ($instances) {
                    list($sql, $params) = $DB->get_in_or_equal(array_keys($this->get_paymentgateway_instances()), SQL_PARAMS_NAMED);
                } else {
    ...
    /**
     * Returns all of the paymentgateway instances for this course.
     *
    ...
    public function get_paymentgateway_instances() {
        if ($this->_instances === null) {
            $this->_instances = prodinfo_get_instances($this->course->id, false);
        }
        return $this->_instances;
    }

    /**
     * Returns the names for all of the paymentgateway instances for this course.
     *
     * NOTE: since 2.4 it includes instances of disabled plugins too.
     *
     * @return array
    ...
    public function get_paymentgateway_instance_names() {
        if ($this->_inames === null) {
            $instances = $this->get_paymentgateway_instances();
            $plugins = $this->get_paymentgateway_plugins(false);
            foreach ($instances as $key=>$instance) {
                ...
                $this->_inames[$key] = $plugins[$instance->prodinfo]->get_instance_name($instance);
            }
        }
        return $this->_inames;
    }
    ...
    /**
     * Gets all of the paymentgateway plugins that are active for this course.
     *
     * @param bool $onlyenabled return only enabled prodinfo plugins
     * @return array
     */

    public function get_paymentgateway_plugins($onlyenabled = true) {
        if ($this->_plugins === null) {
            $this->_plugins = prodinfo_get_plugins(true);
        }
        ...
        if ($this->_allplugins === null) {
            ...
            foreach (prodinfo_get_plugins(false) as $name=>$plugin) {
    ...
    /**
     * Unprodinfos a user from the course given the users ue entry
     *
     * @global moodle_database $DB
     * @param stdClass $ue
     * @return bool
    ...
    public function unprodinfo_user($ue) {
        global $DB;
        list ($instance, $plugin) = $this->get_user_paymentgateway_components($ue);
        if ($instance && $plugin && $plugin->allow_unprodinfo_user($instance, $ue) && has_capability("prodinfo/$instance->prodinfo:unprodinfo", $this->context)) {
            $plugin->unprodinfo_user($instance, $ue->userid);
            return true;
        }
        return false;
    }
    ...
    /**
     * Given a user paymentgateway record this method returns the plugin and paymentgateway
     * instance that relate to it.
     *
     * @param stdClass|int $userpaymentgateway
     * @return array array($instance, $plugin)
     */

    public function get_user_paymentgateway_components($userpaymentgateway) {
        global $DB;
        if (is_numeric($userpaymentgateway)) {
            $userpaymentgateway = $DB->get_record('user_paymentgateways', array('id'=>(int)$userpaymentgateway));
        }
        $instances = $this->get_paymentgateway_instances();
        $plugins = $this->get_paymentgateway_plugins(false);
        if (!$userpaymentgateway || !isset($instances[$userpaymentgateway->prodinfoid])) {
            return array(false, false);
        }
        $instance = $instances[$userpaymentgateway->prodinfoid];
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        foreach ($ras as $ra) {
            if ($ra->component) {
                if (strpos($ra->component, 'prodinfo_') !== 0) {
                    continue;
                }
                if (!$plugin = prodinfo_get_plugin(substr($ra->component, 6))) {
                    continue;
                }
    ...
    /**
     * Edits an paymentgateway
     *
     * @param stdClass $userpaymentgateway
     * @param stdClass $data
     * @return bool
     */
//============================================================================
...
//============================================================================
//============================================================================
    public function edit_paymentgateway($userpaymentgateway, $data) {
        //Only allow editing if the user has the appropriate capability
        //Already checked in /prodinfo/users.php but checking again in case this function is called from elsewhere
        list($instance, $plugin) = $this->get_user_paymentgateway_components($userpaymentgateway);
        if ($instance && $plugin && $plugin->allow_manage($instance) && has_capability("prodinfo/$instance->prodinfo:manage", $this->context)) {
            if (!isset($data->status)) {
                $data->status = $userpaymentgateway->status;
            }
            $plugin->update_user_prodinfo($instance, $userpaymentgateway->userid, $data->status, $data->timestart, $data->timeend);
            return true;
        }
        return false;
    }
    ...
    /**
     * Returns the current paymentgateway filter that is being applied by this class
     * @return string
     */
    public function get_paymentgateway_filter() {
    ...
    /**
     * Gets the paymentgateways this user has in the course - including all suspended plugins and instances.
     *
     * @global moodle_database $DB
     * @param int $userid
     * @return array
    ...
    public function get_user_paymentgateways($userid) {
        global $DB;
        list($instancessql, $params, $filter) = $this->get_instance_sql();
        $params['userid'] = $userid;
        $userpaymentgateways = $DB->get_records_select('user_paymentgateways', "prodinfoid $instancessql AND userid = :userid", $params);
        $instances = $this->get_paymentgateway_instances();
        $plugins = $this->get_paymentgateway_plugins(false);
        $inames = $this->get_paymentgateway_instance_names();
        foreach ($userpaymentgateways as &$ue) {
            $ue->paymentgatewayinstance     = $instances[$ue->prodinfoid];
            $ue->paymentgatewayplugin       = $plugins[$ue->paymentgatewayinstance->prodinfo];
            $ue->paymentgatewayinstancename = $inames[$ue->paymentgatewayinstance->id];
        }
        return $userpaymentgateways;
    }
...
//============================================================================
    ...
    /**
     * Returns the course this object is managing paymentgateways for
     *
     * @return stdClass
     */
...
//============================================================================
     ...
     * course but have not been prodinfoled.
     *
     * @param core_prodinfo_renderer $renderer
    ...
    public function get_other_users_for_display(core_prodinfo_renderer $renderer, moodle_url $pageurl, $sort, $direction, $page, $perpage) {
        ...
        foreach ($userroles as $userrole) {
            ...
            if ($contextid == $this->context->id) {
                ...
                if ($userrole->component) {
                    $changeable = false;
                    if (strpos($userrole->component, 'prodinfo_') === 0) {
                ...
                $roletext = get_string('rolefromthiscourse', 'prodinfo', $a);
            } else {
                $changeable = false;
                switch ($userrole->contextlevel) {
                    case CONTEXT_COURSE :
                        // Meta course
                        $roletext = get_string('rolefrommetacourse', 'prodinfo', $a);
                        break;
                    case CONTEXT_COURSECAT :
                        $roletext = get_string('rolefromcategory', 'prodinfo', $a);
                        break;
                    case CONTEXT_SYSTEM:
                    default:
                        $roletext = get_string('rolefromsystem', 'prodinfo', $a);
                        break;
                }
            }
     ...
     * as well as minimal information on the users roles, groups, and paymentgateways.
     *
     * @param core_prodinfo_renderer $renderer
     * @param moodle_url $pageurl
     * @param int $sort
    ...
    public function get_users_for_display(course_paymentgateway_manager $manager, $sort, $direction, $page, $perpage) {
        $pageurl = $manager->get_moodlepage()->url;
        $users = $this->get_users($sort, $direction, $page, $perpage);

        $now = time();
        $straddgroup = get_string('addgroup', 'group');
        $strunprodinfo = get_string('unprodinfo', 'prodinfo');
        ...
        $enabledplugins = $this->get_paymentgateway_plugins(true);
        ...
        foreach ($users as $user) {
            ...
            // paymentgateways
            $details['paymentgateways'] = array();
            foreach ($this->get_user_paymentgateways($user->id) as $ue) {
                if (!isset($enabledplugins[$ue->paymentgatewayinstance->prodinfo])) {
                    $details['paymentgateways'][$ue->id] = array(
                        'text' => $ue->paymentgatewayinstancename,
                        'period' => null,
                        'dimmed' =>  true,
                        'actions' => array()
                    );
                    ...
                    $period = get_string('periodstartend', 'prodinfo', array('start'=>userdate($ue->timestart), 'end'=>userdate($ue->timeend)));
                    $periodoutside = ($ue->timestart && $ue->timeend && ($now < $ue->timestart || $now > $ue->timeend));
                } else if ($ue->timestart) {
                    $period = get_string('periodstart', 'prodinfo', userdate($ue->timestart));
                    $periodoutside = ($ue->timestart && $now < $ue->timestart);
                } else if ($ue->timeend) {
                    $period = get_string('periodend', 'prodinfo', userdate($ue->timeend));
                    $periodoutside = ($ue->timeend && $now > $ue->timeend);
                } else {
                    // If there is no start or end show when user was prodinfoled.
                    $period = get_string('periodnone', 'prodinfo', userdate($ue->timecreated));
                    $periodoutside = false;
                }
                $details['paymentgateways'][$ue->id] = array(
                    'text' => $ue->paymentgatewayinstancename,
                    'period' => $period,
                    'dimmed' =>  ($periodoutside or $ue->status != prodinfo_USER_ACTIVE or $ue->paymentgatewayinstance->status != prodinfo_INSTANCE_ENABLED),
                    'actions' => $ue->paymentgatewayplugin->get_user_paymentgateway_actions($manager, $ue)
                );
            }
    ...
    public function get_manual_prodinfo_buttons() {
        $plugins = $this->get_paymentgateway_plugins(true); // Skip disabled plugins.
        $buttons = array();
        foreach ($plugins as $plugin) {
            $newbutton = $plugin->get_manual_prodinfo_button($this);
            if (is_array($newbutton)) {
                $buttons += $newbutton;
            } else if ($newbutton instanceof prodinfo_user_button) {
                $buttons[] = $newbutton;
            }
        }
        return $buttons;
    ...
    public function has_instance($prodinfopluginname) {
        // Make sure manual paymentgateways instance exists
        foreach ($this->get_paymentgateway_instances() as $instance) {
            if ($instance->prodinfo == $prodinfopluginname) {
                return true;
            }
        }
        return false;
...
//============================================================================
    /**
     * Returns the paymentgateway plugin that the course manager was being filtered to.
     *
     * If no filter was being applied then this function returns false.
     *
     * @return prodinfo_plugin
     */
    public function get_filtered_paymentgateway_plugin() {
        $instances = $this->get_paymentgateway_instances();
        $plugins = $this->get_paymentgateway_plugins(false);
        ...
        return $plugins[$instance->prodinfo];
    }

    /**
     * Returns and array of users + paymentgateway details.
     *
     * Given an array of user id's this function returns and array of user paymentgateways for those users
     * as well as enough user information to display the users name and picture for each paymentgateway.
     *
     * @global moodle_database $DB
     * @param array $userids
     * @return array
    ...
    public function get_users_paymentgateways(array $userids) {
        global $DB;
        ...
        $sql = "SELECT ue.id AS ueid, ue.status, ue.prodinfoid, ue.userid, ue.timestart, ue.timeend, ue.modifierid, ue.timecreated, ue.timemodified, $userfields
                  FROM {user_paymentgateways} ue
             LEFT JOIN {user} u ON u.id = ue.userid
                 WHERE ue.prodinfoid $instancesql AND
                       u.id $idsql
              ORDER BY $sort";
        ...
        foreach ($rs as $ue) {
            ...
            if (!array_key_exists($user->id, $users)) {
                $user->paymentgateways = array();
                $users[$user->id] = $user;
            }
            $ue->paymentgatewayinstance = $instances[$ue->prodinfoid];
            $ue->paymentgatewayplugin = $plugins[$ue->paymentgatewayinstance->prodinfo];
            $users[$user->id]->paymentgateways[$ue->id] = $ue;
        }
        $rs->close();
        return $users;
    }
...
/**
 * A button that is used to prodinfo users in a course
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
//============================================================================
//============================================================================
class prodinfo_user_button extends single_button {
    ...
    /**
     * Initialises the new prodinfo_user_button
     *
     * @staticvar int $count The number of prodinfo user buttons already created
    ...
    public function __construct(moodle_url $url, $label, $method = 'post') {
        ...
        $this->class = 'singlebutton prodinfousersbutton';
        $this->formid = 'prodinfousersbutton-'.$count;
    }
...
/**
 * User paymentgateway action
 *
 * This class is used to manage a renderable ue action such as editing an user paymentgateway or deleting
 * a user paymentgateway.
 *
 * @copyright  2011 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_paymentgateway_action implements renderable {
...
class prodinfo_ajax_exception extends moodle_exception {
    /**
     * Constructor
    ...
    public function __construct($errorcode, $link = '', $a = NULL, $debuginfo = null) {
        parent::__construct($errorcode, 'prodinfo', $link, $a, $debuginfo);
    }
}

/**
 * This class is used to manage a bulk operations for paymentgateway plugins.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class prodinfo_bulk_paymentgateway_operation {

    /**
     * The course paymentgateway manager
     * @var course_paymentgateway_manager
     */
    protected $manager;

    /**
     * The paymentgateway plugin to which this operation belongs
     * @var prodinfo_plugin
     */
    protected $plugin;
     ...
     */
    public function __construct(course_paymentgateway_manager $manager, prodinfo_plugin $plugin = null) {
     ...
     * @return prodinfo_bulk_paymentgateway_change_form|moodleform|false
     */
     ...
     */
    abstract public function process(course_paymentgateway_manager $manager, array $users, stdClass $properties);
}
