...
/**
 * This is the main renderer for the prodinfo section.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_prodinfo_renderer extends plugin_renderer_base {

    /**
     * Renders a course paymentgateway table
     *
     * @param course_paymentgateway_table $table
     * @param moodleform $mform Form that contains filter controls
     * @return string
     */
    public function render_course_paymentgateway_users_table(course_paymentgateway_users_table $table,
            moodleform $mform) {

        $table->initialise_javascript();

        $buttons = $table->get_manual_prodinfo_buttons();
        $buttonhtml = '';
        if (count($buttons) > 0) {
            $buttonhtml .= html_writer::start_tag('div', array('class' => 'prodinfo_user_buttons'));
        ...
        if ($table->has_bulk_user_paymentgateway_operations()) {
            $content .= html_writer::start_tag('form', array('action' => new moodle_url('/prodinfo/bulkchange.php'), 'method' => 'post'));
            ...
            $content .= html_writer::tag('option', get_string('withselectedusers', 'prodinfo'), array('value' => ''));
            $options = array('' => get_string('withselectedusers', 'prodinfo'));
            foreach ($table->get_bulk_user_paymentgateway_operations() as $operation) {
    ...
    /**
     * Renderers the prodinfo_user_button.
     *
     * @param prodinfo_user_button $button
     * @return string XHTML
     */
    protected function render_prodinfo_user_button(prodinfo_user_button $button) {
     ...
     * @return string
     */
    protected function render_course_paymentgateway_other_users_table(course_paymentgateway_other_users_table $table) {
        ...
        $content .= html_writer::tag('div', get_string('otheruserdesc', 'prodinfo'), array('class'=>'otherusersdesc'));
        $content .= $this->output->render($table->get_paging_bar());
        $content .= html_writer::table($table);
        $content .= $this->output->render($table->get_paging_bar());
        $searchbutton = $table->get_user_search_button();
    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        $iconprodinforemove = $this->output->pix_url('t/delete');
        ...
        foreach ($roles as $roleid=>$role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $icon = html_writer::empty_tag('img', array('alt'=>$strunassign, 'src'=>$iconprodinforemove));
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach($groups as $groupid=>$name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                $icon = html_writer::empty_tag('img', array('alt'=>get_string('removefromgroup', 'group', $name), 'src'=>$iconprodinforemove));
    ...
    /**
     * Generates the HTML for the given paymentgateways + available actions
     *
     * @param int $userid
     ...
     * @return string
     */
    public function user_paymentgateways_and_actions($paymentgateways) {
        $output = '';
        foreach ($paymentgateways as $ue) {
            $paymentgatewayoutput = $ue['text'].' '.$ue['period'];
            if ($ue['dimmed']) {
                $paymentgatewayoutput = html_writer::tag('span', $paymentgatewayoutput, array('class'=>'dimmed_text'));
            } else {
                $paymentgatewayoutput = html_writer::tag('span', $paymentgatewayoutput);
            }
            foreach ($ue['actions'] as $action) {
                $paymentgatewayoutput .= $this->render($action);
            }
            $output .= html_writer::tag('div', $paymentgatewayoutput, array('class'=>'paymentgateway'));
        }
        return $output;
    }

    /**
     * Renders a user paymentgateway action
     * @param user_paymentgateway_action $icon
     * @return string
     */
    protected function render_user_paymentgateway_action(user_paymentgateway_action $icon) {
        return html_writer::link($icon->get_url(), $this->output->render($icon->get_icon()), $icon->get_attributes());
    }
}

/**
 * Main course paymentgateway table
 *
 * This table is used to display the paymentgateway information for a course.
 * It requires that a course paymentgateway manager be provided during constuct with
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_paymentgateway_table extends html_table implements renderable {
    ...
    /**
     * The default sort, options are course_paymentgateway_table::$sortablefields
     * Default: lastname
     */
    ...
    /**
     * The sort field for this table, should be one of course_paymentgateway_table::$sortablefields
     * @var string
     */
    public $sort;
    ...
    /**
     * The users prodinfoled in this course
     * @var array
     */
    ...
    /**
     * An array of bulk user paymentgateway operations
     * @var array
     */
     ...
     */
    public function __construct(course_paymentgateway_manager $manager) {
        ...
        $this->attributes = array('class'=>'userpaymentgateway');
        ...
        $plugin = $manager->get_filtered_paymentgateway_plugin();
        if ($plugin and prodinfo_is_enabled($plugin->get_name())) {
            $this->bulkoperations = $plugin->get_bulk_operations($manager);
        }
    }

    /**
     * Returns an array of prodinfo_user_buttons that are created by the different
     * paymentgateway plugins available.
     *
     * @return array
     */
    public function get_manual_prodinfo_buttons() {
        return $this->manager->get_manual_prodinfo_buttons();
    }
    ...
    public function initialise_javascript() {
        if (has_capability('moodle/role:assign', $this->manager->get_context())) {
            ...
            $modules = array('moodle-prodinfo-rolemanager', 'moodle-prodinfo-rolemanager-skin');
            $function = 'M.prodinfo.rolemanager.init';
            $arguments = array(
                'containerId'=>$this->id,
     ...
     */
    public function set_bulk_user_paymentgateway_operations(array $bulkoperations) {
     ...
     * @return array
     */
    public function get_bulk_user_paymentgateway_operations() {
        return $this->bulkoperations;
    }

    /**
     * Returns true fi the table is aware of any bulk operations that can be performed on users
     * selected from the currently filtered paymentgateway plugins.
     *
     * @return bool
     */
    public function has_bulk_user_paymentgateway_operations() {
        return !empty($this->bulkoperations);
    }
}

/**
 * Table control used for prodinfoled users
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_paymentgateway_users_table extends course_paymentgateway_table {
 ...
 *
 * Other users are users who have roles but are not prodinfoled.
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_paymentgateway_other_users_table extends course_paymentgateway_table {
    ...
    public function __construct(course_paymentgateway_manager $manager) {
        ...
        $this->attributes = array('class'=>'userpaymentgateway otheruserpaymentgateway');
    }
    ...
    public function get_user_search_button() {
        ...
        if ($count == 1) {
            $this->manager->get_moodlepage()->requires->strings_for_js(array(
                    ...
                    'startdatetoday',
                    'durationdays',
                    'prodinfoperiod'), 'prodinfo');
            $this->manager->get_moodlepage()->requires->string_for_js('assignrole', 'role');

            $modules = array('moodle-prodinfo-otherusersmanager', 'moodle-prodinfo-otherusersmanager-skin');
            $function = 'M.prodinfo.otherusersmanager.init';
            $arguments = array(
                'courseId'=> $this->manager->get_course()->id,
                'ajaxUrl' => '/prodinfo/ajax.php',
