...
require('../config.php');
require_once("$CFG->dirroot/prodinfo/locallib.php");
require_once("$CFG->dirroot/prodinfo/renderer.php");
...
$PAGE->set_url(new moodle_url('/prodinfo/ajax.php', array('id'=>$id, 'action'=>$action)));
...
require_capability('moodle/course:prodinforeview', $context);
require_sesskey();
...
switch ($action) {
    case 'unprodinfo':
        ...
        if (!$instance || !$plugin || !prodinfo_is_enabled($instance->prodinfo) || !$plugin->allow_unprodinfo_user($instance, $ue) || !has_capability("prodinfo/$instance->prodinfo:unprodinfo", $manager->get_context()) || !$manager->unprodinfo_user($ue)) {
            throw new prodinfo_ajax_exception('unprodinfonotpermitted');
        }
        break;
    case 'unassign':
        ...
        if (!has_capability('moodle/role:assign', $manager->get_context()) || !$manager->unassign_role_from_user($user, $role)) {
            throw new prodinfo_ajax_exception('unassignnotpermitted');
        }
        break;
    case 'assign':
    ...
    case 'searchotherusers':
        ...
        $useroptions = array();
        // User is not prodinfoled, either link to site profile or do not link at all.
        ...
        break;
    default:
        throw new prodinfo_ajax_exception('unknowajaxaction');
}
