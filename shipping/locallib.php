...
/**
 * This file contains the course_shipment_manager class which is used to interface
 * with the functions that exist in shippinglib.php in relation to a single course.
 *
...
/**
 * This class provides a targeted tied together means of interfacing the shipment
...
class course_shipment_manager {
    ...
    protected $context;
    /**
     * The course we are managing shipments for
     * @var stdClass
     */
    protected $course = null;
    /**
     * Limits the focus of the manager to one shipment plugin instance
     * @var string
     */
    ...
    /**
     * The total number of users shippingled in the course
     * Populated by course_shipment_manager::get_total_users
     * @var int
     */
    protected $totalusers = null;
    /**
     * An array of users currently shippingled in the course
     * Populated by course_shipment_manager::get_users
     * @var array
     */
     ...
     * been shippingled in the course
     * @var array
     */
    ...
    /**
     * Constructs the course shipment manager
     *
     ...
     * @param int $statusfilter if not -1, filter users with active/inactive shipment.
     */
    ...
    /**
     * Returns the total number of shippingled users in the course.
     *
     * If a filter was specificed this will be the total number of users shippingled
    ...
    public function get_total_users() {
        ...
        if ($this->totalusers === null) {
            ...
            $sqltotal = "SELECT COUNT(DISTINCT u.id)
                           FROM {user} u
                           JOIN {user_shipments} ue ON (ue.userid = u.id  AND ue.shippingid $instancessql)
        ...
        if ($this->totalotherusers === null) {
            ...
            $sql = "SELECT COUNT(DISTINCT u.id)
                 ...
                 LEFT JOIN (
                        ...
                        LEFT JOIN {shipping} e ON e.id=ue.shippingid
    ...
    /**
     * Gets all of the users shippingled in this course.
     *
     * If a filter was specified this will be the users who were shippingled
    ...
    protected function get_filter_sql() {
        ...
        // Status condition.
        if ($this->statusfilter === shipping_USER_ACTIVE) {
            $sql .= " AND ue.status = :active AND e.status = :enabled AND ue.timestart < :now1
                    AND (ue.timeend = 0 OR ue.timeend > :now2)";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('enabled' => shipping_INSTANCE_ENABLED,
                             'active' => shipping_USER_ACTIVE,
                             'now1' => $now,
                             'now2' => $now);
        } else if ($this->statusfilter === shipping_USER_SUSPENDED) {
            $sql .= " AND (ue.status = :inactive OR e.status = :disabled OR ue.timestart > :now1
                    OR (ue.timeend <> 0 AND ue.timeend < :now2))";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('disabled' => shipping_INSTANCE_DISABLED,
                             'inactive' => shipping_USER_SUSPENDED,
                             'now1' => $now,
                             'now2' => $now);
        }
     ...
     * within this course but who have not been shippingled in the course
     *
     * @global moodle_database $DB
     * @param string $sort
     ...
     * @param int $addedshipment number of users added to shipment.
     * @return array with two elememts:
     *      int total number of users matching the search.
     *      array of user objects returned by the query.
     */

    protected function execute_search_queries($search, $fields, $countfields, $sql, array $params, $page, $perpage, $addedshipment=0) {
        global $DB, $CFG;
        ...
        $availableusers = $DB->get_records_sql($fields . $sql . $order,
                array_merge($params, $sortparams), ($page*$perpage) - $addedshipment, $perpage);

        return array('totalusers' => $totalusers, 'users' => $availableusers);
    }

    /**
     * Gets an array of the users that can be shippingled in this course.
     *
     * @global moodle_database $DB
     ...
     * @param int $perpage Defaults to 25
     * @param int $addedshipment Defaults to 0
     * @return array Array(totalusers => int, users => array)
     */

    public function get_potential_users($shippingid, $search='', $searchanywhere=false, $page=0, $perpage=25, $addedshipment=0) {
        global $DB;
        ...
        $sql = " FROM {user} u
            LEFT JOIN {user_shipments} ue ON (ue.userid = u.id AND ue.shippingid = :shippingid)
                WHERE $wherecondition
                      AND ue.id IS NULL";
        $params['shippingid'] = $shippingid;

        return $this->execute_search_queries($search, $fields, $countfields, $sql, $params, $page, $perpage, $addedshipment);
    }
...
// ====================================================================================================
    protected function get_instance_sql() {
        ...
        if ($this->_instancessql === null) {
            $instances = $this->get_shipment_instances();
            $filter = $this->get_shipment_filter();
            if ($filter && array_key_exists($filter, $instances)) {
                ...
                $filter = 0;
                if ($instances) {
                    list($sql, $params) = $DB->get_in_or_equal(array_keys($this->get_shipment_instances()), SQL_PARAMS_NAMED);
                } else {
    ...
    /**
     * Returns all of the shipment instances for this course.
     *
    ...
    public function get_shipment_instances() {
        if ($this->_instances === null) {
            $this->_instances = shipping_get_instances($this->course->id, false);
        }
        return $this->_instances;
    }

    /**
     * Returns the names for all of the shipment instances for this course.
     *
     * NOTE: since 2.4 it includes instances of disabled plugins too.
     *
     * @return array
    ...
    public function get_shipment_instance_names() {
        if ($this->_inames === null) {
            $instances = $this->get_shipment_instances();
            $plugins = $this->get_shipment_plugins(false);
            foreach ($instances as $key=>$instance) {
                ...
                $this->_inames[$key] = $plugins[$instance->shipping]->get_instance_name($instance);
            }
        }
        return $this->_inames;
    }
    ...
    /**
     * Gets all of the shipment plugins that are active for this course.
     *
     * @param bool $onlyenabled return only enabled shipping plugins
     * @return array
     */

    public function get_shipment_plugins($onlyenabled = true) {
        if ($this->_plugins === null) {
            $this->_plugins = shipping_get_plugins(true);
        }
        ...
        if ($this->_allplugins === null) {
            ...
            foreach (shipping_get_plugins(false) as $name=>$plugin) {
    ...
    /**
     * Unshippings a user from the course given the users ue entry
     *
     * @global moodle_database $DB
     * @param stdClass $ue
     * @return bool
    ...
    public function unshipping_user($ue) {
        global $DB;
        list ($instance, $plugin) = $this->get_user_shipment_components($ue);
        if ($instance && $plugin && $plugin->allow_unshipping_user($instance, $ue) && has_capability("shipping/$instance->shipping:unshipping", $this->context)) {
            $plugin->unshipping_user($instance, $ue->userid);
            return true;
        }
        return false;
    }
    ...
    /**
     * Given a user shipment record this method returns the plugin and shipment
     * instance that relate to it.
     *
     * @param stdClass|int $usershipment
     * @return array array($instance, $plugin)
     */

    public function get_user_shipment_components($usershipment) {
        global $DB;
        if (is_numeric($usershipment)) {
            $usershipment = $DB->get_record('user_shipments', array('id'=>(int)$usershipment));
        }
        $instances = $this->get_shipment_instances();
        $plugins = $this->get_shipment_plugins(false);
        if (!$usershipment || !isset($instances[$usershipment->shippingid])) {
            return array(false, false);
        }
        $instance = $instances[$usershipment->shippingid];
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        foreach ($ras as $ra) {
            if ($ra->component) {
                if (strpos($ra->component, 'shipping_') !== 0) {
                    continue;
                }
                if (!$plugin = shipping_get_plugin(substr($ra->component, 6))) {
                    continue;
                }
...
//============================================================================
//============================================================================
    public function edit_shipment($usershipment, $data) {
        //Only allow editing if the user has the appropriate capability
        //Already checked in /shipping/users.php but checking again in case this function is called from elsewhere
        list($instance, $plugin) = $this->get_user_shipment_components($usershipment);
        if ($instance && $plugin && $plugin->allow_manage($instance) && has_capability("shipping/$instance->shipping:manage", $this->context)) {
            if (!isset($data->status)) {
                $data->status = $usershipment->status;
            }
            $plugin->update_user_shipping($instance, $usershipment->userid, $data->status, $data->timestart, $data->timeend);
            return true;
        }
        return false;
    }
    ...
    /**
     * Returns the current shipment filter that is being applied by this class
     * @return string
     */
    public function get_shipment_filter() {
    ...
    /**
     * Gets the shipments this user has in the course - including all suspended plugins and instances.
     *
     * @global moodle_database $DB
     * @param int $userid
     * @return array
    ...
    public function get_user_shipments($userid) {
        global $DB;
        list($instancessql, $params, $filter) = $this->get_instance_sql();
        $params['userid'] = $userid;
        $usershipments = $DB->get_records_select('user_shipments', "shippingid $instancessql AND userid = :userid", $params);
        $instances = $this->get_shipment_instances();
        $plugins = $this->get_shipment_plugins(false);
        $inames = $this->get_shipment_instance_names();
        foreach ($usershipments as &$ue) {
            $ue->shipmentinstance     = $instances[$ue->shippingid];
            $ue->shipmentplugin       = $plugins[$ue->shipmentinstance->shipping];
            $ue->shipmentinstancename = $inames[$ue->shipmentinstance->id];
        }
...
//============================================================================
    ...
    /**
     * Returns the course this object is managing shipments for
     *
     * @return stdClass
     */
...
//============================================================================
     ...
     * course but have not been shippingled.
     *
     * @param core_shipping_renderer $renderer
    ...
    public function get_other_users_for_display(core_shipping_renderer $renderer, moodle_url $pageurl, $sort, $direction, $page, $perpage) {
        ...
        foreach ($userroles as $userrole) {
            ...
            if ($contextid == $this->context->id) {
                ...
                if ($userrole->component) {
                    $changeable = false;
                    if (strpos($userrole->component, 'shipping_') === 0) {
                ...
                $roletext = get_string('rolefromthiscourse', 'shipping', $a);
            } else {
                $changeable = false;
                switch ($userrole->contextlevel) {
                    case CONTEXT_COURSE :
                        // Meta course
                        $roletext = get_string('rolefrommetacourse', 'shipping', $a);
                        break;
                    case CONTEXT_COURSECAT :
                        $roletext = get_string('rolefromcategory', 'shipping', $a);
                        break;
                    case CONTEXT_SYSTEM:
                    default:
                        $roletext = get_string('rolefromsystem', 'shipping', $a);
                        break;
                }
            }
     ...
     * as well as minimal information on the users roles, groups, and shipments.
     *
     * @param core_shipping_renderer $renderer
     * @param moodle_url $pageurl
     * @param int $sort
    ...
    public function get_users_for_display(course_shipment_manager $manager, $sort, $direction, $page, $perpage) {
        $pageurl = $manager->get_moodlepage()->url;
        $users = $this->get_users($sort, $direction, $page, $perpage);

        $now = time();
        $straddgroup = get_string('addgroup', 'group');
        $strunshipping = get_string('unshipping', 'shipping');
        ...
        $enabledplugins = $this->get_shipment_plugins(true);
        ...
        foreach ($users as $user) {
            ...
            // shipments
            $details['shipments'] = array();
            foreach ($this->get_user_shipments($user->id) as $ue) {
                if (!isset($enabledplugins[$ue->shipmentinstance->shipping])) {
                    $details['shipments'][$ue->id] = array(
                        'text' => $ue->shipmentinstancename,
                        'period' => null,
                        'dimmed' =>  true,
                        'actions' => array()
                    );
                    ...
                    $period = get_string('periodstartend', 'shipping', array('start'=>userdate($ue->timestart), 'end'=>userdate($ue->timeend)));
                    $periodoutside = ($ue->timestart && $ue->timeend && ($now < $ue->timestart || $now > $ue->timeend));
                } else if ($ue->timestart) {
                    $period = get_string('periodstart', 'shipping', userdate($ue->timestart));
                    $periodoutside = ($ue->timestart && $now < $ue->timestart);
                } else if ($ue->timeend) {
                    $period = get_string('periodend', 'shipping', userdate($ue->timeend));
                    $periodoutside = ($ue->timeend && $now > $ue->timeend);
                } else {
                    // If there is no start or end show when user was shippingled.
                    $period = get_string('periodnone', 'shipping', userdate($ue->timecreated));
                    $periodoutside = false;
                }
                $details['shipments'][$ue->id] = array(
                    'text' => $ue->shipmentinstancename,
                    'period' => $period,
                    'dimmed' =>  ($periodoutside or $ue->status != shipping_USER_ACTIVE or $ue->shipmentinstance->status != shipping_INSTANCE_ENABLED),
                    'actions' => $ue->shipmentplugin->get_user_shipment_actions($manager, $ue)
                );
            }
    ...
    public function get_manual_shipping_buttons() {
        $plugins = $this->get_shipment_plugins(true); // Skip disabled plugins.
        $buttons = array();
        foreach ($plugins as $plugin) {
            $newbutton = $plugin->get_manual_shipping_button($this);
            if (is_array($newbutton)) {
                $buttons += $newbutton;
            } else if ($newbutton instanceof shipping_user_button) {
                $buttons[] = $newbutton;
            }
        }
        return $buttons;
    ...
    public function has_instance($shippingpluginname) {
        // Make sure manual shipments instance exists
        foreach ($this->get_shipment_instances() as $instance) {
            if ($instance->shipping == $shippingpluginname) {
                return true;
            }
        }
        return false;
...
//============================================================================
    /**
     * Returns the shipment plugin that the course manager was being filtered to.
     *
     * If no filter was being applied then this function returns false.
     *
     * @return shipping_plugin
     */
    public function get_filtered_shipment_plugin() {
        $instances = $this->get_shipment_instances();
        $plugins = $this->get_shipment_plugins(false);
        ...
        return $plugins[$instance->shipping];
    }

    /**
     * Returns and array of users + shipment details.
     *
     * Given an array of user id's this function returns and array of user shipments for those users
     * as well as enough user information to display the users name and picture for each shipment.
     *
     * @global moodle_database $DB
     * @param array $userids
     * @return array
    ...
    public function get_users_shipments(array $userids) {
        global $DB;
        ...
        $sql = "SELECT ue.id AS ueid, ue.status, ue.shippingid, ue.userid, ue.timestart, ue.timeend, ue.modifierid, ue.timecreated, ue.timemodified, $userfields
                  FROM {user_shipments} ue
             LEFT JOIN {user} u ON u.id = ue.userid
                 WHERE ue.shippingid $instancesql AND
                       u.id $idsql
              ORDER BY $sort";
        ...
        foreach ($rs as $ue) {
            ...
            if (!array_key_exists($user->id, $users)) {
                $user->shipments = array();
                $users[$user->id] = $user;
            }
            $ue->shipmentinstance = $instances[$ue->shippingid];
            $ue->shipmentplugin = $plugins[$ue->shipmentinstance->shipping];
            $users[$user->id]->shipments[$ue->id] = $ue;
        }
        $rs->close();
        return $users;
    }
...
/**
 * A button that is used to shipping users in a course
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
//============================================================================
//============================================================================
class shipping_user_button extends single_button {
    ...
    /**
     * Initialises the new shipping_user_button
     *
     * @staticvar int $count The number of shipping user buttons already created
    ...
    public function __construct(moodle_url $url, $label, $method = 'post') {
        ...
        $this->class = 'singlebutton shippingusersbutton';
        $this->formid = 'shippingusersbutton-'.$count;
    }
...
/**
 * User shipment action
 *
 * This class is used to manage a renderable ue action such as editing an user shipment or deleting
 * a user shipment.
 *
 * @copyright  2011 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_shipment_action implements renderable {
...
class shipping_ajax_exception extends moodle_exception {
    /**
     * Constructor
    ...
    public function __construct($errorcode, $link = '', $a = NULL, $debuginfo = null) {
        parent::__construct($errorcode, 'shipping', $link, $a, $debuginfo);
    }
}

/**
 * This class is used to manage a bulk operations for shipment plugins.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class shipping_bulk_shipment_operation {

    /**
     * The course shipment manager
     * @var course_shipment_manager
     */
    protected $manager;

    /**
     * The shipment plugin to which this operation belongs
     * @var shipping_plugin
     */
    protected $plugin;
     ...
     */
    public function __construct(course_shipment_manager $manager, shipping_plugin $plugin = null) {
     ...
     * @return shipping_bulk_shipment_change_form|moodleform|false
     */
     ...
     */
    abstract public function process(course_shipment_manager $manager, array $users, stdClass $properties);
}
