...
/**
 * This is the main renderer for the shipping section.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_shipping_renderer extends plugin_renderer_base {

    /**
     * Renders a course shipment table
     *
     * @param course_shipment_table $table
     * @param moodleform $mform Form that contains filter controls
     * @return string
     */
    public function render_course_shipment_users_table(course_shipment_users_table $table,
            moodleform $mform) {

        $table->initialise_javascript();

        $buttons = $table->get_manual_shipping_buttons();
        $buttonhtml = '';
        if (count($buttons) > 0) {
            $buttonhtml .= html_writer::start_tag('div', array('class' => 'shipping_user_buttons'));
        ...
        if ($table->has_bulk_user_shipment_operations()) {
            $content .= html_writer::start_tag('form', array('action' => new moodle_url('/shipping/bulkchange.php'), 'method' => 'post'));
            ...
            $content .= html_writer::tag('option', get_string('withselectedusers', 'shipping'), array('value' => ''));
            $options = array('' => get_string('withselectedusers', 'shipping'));
            foreach ($table->get_bulk_user_shipment_operations() as $operation) {
    ...
    /**
     * Renderers the shipping_user_button.
     *
     * @param shipping_user_button $button
     * @return string XHTML
     */
    protected function render_shipping_user_button(shipping_user_button $button) {
     ...
     * @return string
     */
    protected function render_course_shipment_other_users_table(course_shipment_other_users_table $table) {
        ...
        $content .= html_writer::tag('div', get_string('otheruserdesc', 'shipping'), array('class'=>'otherusersdesc'));
        $content .= $this->output->render($table->get_paging_bar());
        $content .= html_writer::table($table);
        $content .= $this->output->render($table->get_paging_bar());
        $searchbutton = $table->get_user_search_button();
    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        $iconshippingremove = $this->output->pix_url('t/delete');
        ...
        foreach ($roles as $roleid=>$role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $icon = html_writer::empty_tag('img', array('alt'=>$strunassign, 'src'=>$iconshippingremove));
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach($groups as $groupid=>$name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                $icon = html_writer::empty_tag('img', array('alt'=>get_string('removefromgroup', 'group', $name), 'src'=>$iconshippingremove));
    ...
    /**
     * Generates the HTML for the given shipments + available actions
     *
     * @param int $userid
     ...
     * @return string
     */
    public function user_shipments_and_actions($shipments) {
        $output = '';
        foreach ($shipments as $ue) {
            $shipmentoutput = $ue['text'].' '.$ue['period'];
            if ($ue['dimmed']) {
                $shipmentoutput = html_writer::tag('span', $shipmentoutput, array('class'=>'dimmed_text'));
            } else {
                $shipmentoutput = html_writer::tag('span', $shipmentoutput);
            }
            foreach ($ue['actions'] as $action) {
                $shipmentoutput .= $this->render($action);
            }
            $output .= html_writer::tag('div', $shipmentoutput, array('class'=>'shipment'));
        }
        return $output;
    }

    /**
     * Renders a user shipment action
     * @param user_shipment_action $icon
     * @return string
     */
    protected function render_user_shipment_action(user_shipment_action $icon) {
        return html_writer::link($icon->get_url(), $this->output->render($icon->get_icon()), $icon->get_attributes());
    }
}

/**
 * Main course shipment table
 *
 * This table is used to display the shipment information for a course.
 * It requires that a course shipment manager be provided during constuct with
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_shipment_table extends html_table implements renderable {
    ...
    /**
     * The default sort, options are course_shipment_table::$sortablefields
     * Default: lastname
     */
    ...
    /**
     * The sort field for this table, should be one of course_shipment_table::$sortablefields
     * @var string
     */
    public $sort;
    ...
    /**
     * The users shippingled in this course
     * @var array
     */
    ...
    /**
     * An array of bulk user shipment operations
     * @var array
     */
     ...
     */
    public function __construct(course_shipment_manager $manager) {
        ...
        $this->attributes = array('class'=>'usershipment');
        ...
        $plugin = $manager->get_filtered_shipment_plugin();
        if ($plugin and shipping_is_enabled($plugin->get_name())) {
            $this->bulkoperations = $plugin->get_bulk_operations($manager);
        }
    }

    /**
     * Returns an array of shipping_user_buttons that are created by the different
     * shipment plugins available.
     *
     * @return array
     */
    public function get_manual_shipping_buttons() {
        return $this->manager->get_manual_shipping_buttons();
    }
    ...
    public function initialise_javascript() {
        if (has_capability('moodle/role:assign', $this->manager->get_context())) {
            ...
            $modules = array('moodle-shipping-rolemanager', 'moodle-shipping-rolemanager-skin');
            $function = 'M.shipping.rolemanager.init';
            $arguments = array(
                'containerId'=>$this->id,
     ...
     */
    public function set_bulk_user_shipment_operations(array $bulkoperations) {
     ...
     * @return array
     */
    public function get_bulk_user_shipment_operations() {
        return $this->bulkoperations;
    }

    /**
     * Returns true fi the table is aware of any bulk operations that can be performed on users
     * selected from the currently filtered shipment plugins.
     *
     * @return bool
     */
    public function has_bulk_user_shipment_operations() {
        return !empty($this->bulkoperations);
    }
}

/**
 * Table control used for shippingled users
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_shipment_users_table extends course_shipment_table {
 ...
 *
 * Other users are users who have roles but are not shippingled.
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_shipment_other_users_table extends course_shipment_table {
    ...
    public function __construct(course_shipment_manager $manager) {
        ...
        $this->attributes = array('class'=>'usershipment otherusershipment');
    }
    ...
    public function get_user_search_button() {
        ...
        if ($count == 1) {
            ...
            $modules = array('moodle-shipping-otherusersmanager', 'moodle-shipping-otherusersmanager-skin');
            $function = 'M.shipping.otherusersmanager.init';
            $arguments = array(
                'courseId'=> $this->manager->get_course()->id,
                'ajaxUrl' => '/shipping/ajax.php',
