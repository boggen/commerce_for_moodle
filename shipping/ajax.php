...
/**
 * This file processes AJAX shipment actions and returns JSON
 *
...
require('../config.php');
require_once("$CFG->dirroot/shipping/locallib.php");
require_once("$CFG->dirroot/shipping/renderer.php");
...
$PAGE->set_url(new moodle_url('/shipping/ajax.php', array('id'=>$id, 'action'=>$action)));
...
require_capability('moodle/course:shippingreview', $context);
require_sesskey();

echo $OUTPUT->header(); // send headers

$manager = new course_shipment_manager($PAGE, $course);
...
switch ($action) {
    case 'unshipping':
        //$ue = $DB->get_record('user_shipments', array('id'=>required_param('ue', PARAM_INT)), '*', MUST_EXIST);
        list ($instance, $plugin) = $manager->get_user_shipment_components($ue);
        if (!$instance || !$plugin || !shipping_is_enabled($instance->shipping) || !$plugin->allow_unshipping_user($instance, $ue) || !has_capability("shipping/$instance->shipping:unshipping", $manager->get_context()) || !$manager->unshipping_user($ue)) {
            throw new shipping_ajax_exception('unshippingnotpermitted');
        }
        break;
    case 'unassign':
        ...
        if (!has_capability('moodle/role:assign', $manager->get_context()) || !$manager->unassign_role_from_user($user, $role)) {
            throw new shipping_ajax_exception('unassignnotpermitted');
        }
        break;
    case 'assign':
    ...
    case 'searchotherusers':
        ...
        $useroptions = array();
        // User is not shippingled, either link to site profile or do not link at all.
        ...
        break;
    default:
        throw new shipping_ajax_exception('unknowajaxaction');
}
