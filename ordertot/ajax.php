...
/**
 * This file processes AJAX ordertotal actions and returns JSON
 *
...
require('../config.php');
require_once("$CFG->dirroot/ordertot/locallib.php");
require_once("$CFG->dirroot/ordertot/renderer.php");
...
$PAGE->set_url(new moodle_url('/ordertot/ajax.php', array('id'=>$id, 'action'=>$action)));
...
require_capability('moodle/course:ordertotreview', $context);
require_sesskey();

echo $OUTPUT->header(); // send headers

$manager = new course_ordertotal_manager($PAGE, $course);
...
switch ($action) {
    case 'unordertot':
	/*
        $ue = $DB->get_record('user_ordertotals', array('id'=>required_param('ue', PARAM_INT)), '*', MUST_EXIST);
        list ($instance, $plugin) = $manager->get_user_ordertotal_components($ue);
        if (!$instance || !$plugin || !ordertot_is_enabled($instance->ordertot) || !$plugin->allow_unordertot_user($instance, $ue) || !has_capability("ordertot/$instance->ordertot:unordertot", $manager->get_context()) || !$manager->unordertot_user($ue)) {
            throw new ordertot_ajax_exception('unordertotnotpermitted');
        }
	*/
        break;
    case 'unassign':
        ...
        if (!has_capability('moodle/role:assign', $manager->get_context()) || !$manager->unassign_role_from_user($user, $role)) {
            throw new ordertot_ajax_exception('unassignnotpermitted');
        }
        break;
    case 'assign':
    ...
    case 'searchotherusers':
        ...
        $useroptions = array();
        // User is not ordertotled, either link to site profile or do not link at all.
        ...
        break;
    default:
        throw new ordertot_ajax_exception('unknowajaxaction');
}
