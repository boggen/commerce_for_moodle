...
/**
 * handling ordertotal plugin.
 *
 * This plugin lets the user specify a "handling" (CSV) containing ordertotal information.
...
/**
 * handling ordertotal plugin implementation.
 *
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ordertot_handling_plugin extends ordertot_plugin {
    ...
    public function can_delete_instance($instance) {
        ...
        return has_capability('ordertot/handling:manage', $context);
    }
    ...
    /**
     * Returns the user who is responsible for handling ordertotals in given curse.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'ordertot/handling:manage'
     * or 'moodle/role:assign' capability.
     *
     * @param int $courseid ordertotal instance id
     * @return stdClass user record
     */
    protected function get_ordertotler($courseid) {
        ...
        $users = get_ordertotled_users($context, 'ordertot/handling:manage');
        if (!$users) {
            $users = get_ordertotled_users($context, 'moodle/role:assign');
        }
    ...
    public function restore_instance(restore_ordertotals_structure_step $step, stdClass $data, $course, $oldid) {
        ...
        if ($instance = $DB->get_record('ordertot', array('courseid'=>$course->id, 'ordertot'=>$this->get_name()))) {
            $instanceid = $instance->id;
        } else {
            $instanceid = $this->add_instance($course);
        }
        $step->set_mapping('ordertot', $oldid, $instanceid);
    }
    ...
    public function restore_user_ordertotal(restore_ordertotals_structure_step $step, $data, $instance, $userid, $oldinstancestatus) {
        $this->ordertot_user($instance, $userid, null, $data->timestart, $data->timeend, $data->status);
    }
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        role_assign($roleid, $userid, $contextid, 'ordertot_'.$instance->ordertot, $instance->id);
    }
}
