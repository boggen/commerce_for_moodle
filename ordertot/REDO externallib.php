 ...
 *
 * This api is mostly read only, the actual ordertot and unordertot
 * support is in each ordertot plugin.
 *
 ...
 * @since Moodle 2.2
 */
class core_ordertot_external extends external_api {
     ...
     * @since Moodle 2.4
     */
    public static function get_ordertotled_users_with_capability_parameters() {
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) only users with active ordertotals. Requires \'moodle/course:ordertotreview\' .
     ...
     * a list of the users that are ordertotled in the course and have that capability are returned.
     *
     ...
     * @since  Moodle 2.4
     */
    public static function get_ordertotled_users_with_capability($coursecapabilities, $options) {
        global $CFG, $DB;
        ...
        $params = self::validate_parameters(self::get_ordertotled_users_with_capability_parameters(),
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            // The course:ordertotereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('moodle/course:ordertotreview', $coursecontext);
            }
            ...
            foreach ($coursecapability['capabilities'] as $capability) {
                ...
                list($ordertotledsql, $ordertotledparams) = get_ordertotled_sql($coursecontext, $capability, $groupid, $onlyactive);

                $sql = "SELECT u.* FROM {user} u WHERE u.id IN ($ordertotledsql) ORDER BY u.id ASC";

                $ordertotledusers = $DB->get_recordset_sql($sql, $ordertotledparams, $limitfrom, $limitnumber);
                $users = array();
                foreach ($ordertotledusers as $courseuser) {
                    if ($userdetails = user_get_user_details($courseuser, $course, $userfields)) {
                        $users[] = $userdetails;
                    }
                }
                $ordertotledusers->close();
     ...
     * @since Moodle 2.4
     */
    public static function get_ordertotled_users_with_capability_returns() {
        return  new external_multiple_structure( new external_single_structure (
                ...
                array(
                    ...
                    'ordertotledcourses' => new external_multiple_structure(
                        new external_single_structure(
                            array(
                    ...
                    ), 'Courses where the user is ordertotled - limited by which courses the user is able to see', VALUE_OPTIONAL)
                )
                        ), 'List of users that are ordertotled in the course and have the specified capability'),
                    )
                )
            );
    }
    ...
    /**
     * Get list of courses user is ordertotled in (only active ordertotals are returned).
    ...
    public static function get_users_courses($userid) {
        ...
        $courses = ordertot_get_users_courses($params['userid'], true, 'id, shortname, fullname, idnumber, visible');
        $result = array();
        ...
        foreach ($courses as $course) {
            ...
            try {
                ...
                // current user can not access this course, sorry we can not disclose who is ordertotled in this course!
                continue;
            }
            ...
            list($ordertotledsqlselect, $ordertotledparams) = get_ordertotled_sql($context);
            $ordertotledsql = "SELECT COUNT('x') FROM ($ordertotledsqlselect) ordertotleduserids";
            $ordertotledusercount = $DB->count_records_sql($ordertotledsql, $ordertotledparams);

            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'ordertotledusercount'=>$ordertotledusercount);
        }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'ordertotledusercount' => new external_value(PARAM_INT, 'Number of ordertotled users in this course'),
     ...
     * @return external_function_parameters
     */
    public static function get_ordertotled_users_parameters() {
        return new external_function_parameters(
            array(
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) return only users with active ordertotals and matching time restrictions. This option requires \'moodle/course:ordertotreview\' on the course context.
                            * userfields (\'string, string, ...\') return only the values of these user fields.
                            * limitfrom (integer) sql limit from.
     ...
     */
    public static function get_ordertotled_users($courseid, $options = array()) {
        global $CFG, $USER, $DB;
        require_once($CFG->dirroot . "/user/lib.php");

        $params = self::validate_parameters(
            self::get_ordertotled_users_parameters(),
            array(
                'courseid'=>$courseid,
                'options'=>$options
            )
        ...
        // to overwrite this option, you need course:ordertotereview permission
        if ($onlyactive) {
            require_capability('moodle/course:ordertotreview', $coursecontext);
        }

        list($ordertotledsql, $ordertotledparams) = get_ordertotled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        $ctxselect = ', ' . context_helper::get_preload_record_columns_sql('ctx');
        $ctxjoin = "LEFT JOIN {context} ctx ON (ctx.instanceid = u.id AND ctx.contextlevel = :contextlevel)";
        $ordertotledparams['contextlevel'] = CONTEXT_USER;
        $sql = "SELECT u.* $ctxselect
                  FROM {user} u $ctxjoin
                 WHERE u.id IN ($ordertotledsql)
                 ORDER BY u.id ASC";
        $ordertotledusers = $DB->get_recordset_sql($sql, $ordertotledparams, $limitfrom, $limitnumber);
        $users = array();
        foreach ($ordertotledusers as $user) {
     ...
     * @return external_description
     */
    public static function get_ordertotled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
    ...
    /**
     * Returns description of get_course_ordertotal_methods() parameters
     *
     * @return external_function_parameters
     */
    public static function get_course_ordertotal_methods_parameters() {
        return new external_function_parameters(
            array(
                'courseid' => new external_value(PARAM_INT, 'Course id')
            )
    ...
    /**
     * Get list of active course ordertotal methods for current user.
     *
     * @param int $courseid
     * @return array of course ordertotal methods
     */
    public static function get_course_ordertotal_methods($courseid) {

        $params = self::validate_parameters(self::get_course_ordertotal_methods_parameters(), array('courseid' => $courseid));
        ...
        $result = array();
        $ordertotinstances = ordertot_get_instances($params['courseid'], true);
        foreach ($ordertotinstances as $ordertotinstance) {
            if ($ordertotplugin = ordertot_get_plugin($ordertotinstance->ordertot)) {
                if ($instanceinfo = $ordertotplugin->get_ordertot_info($ordertotinstance)) {
    ...
    /**
     * Returns description of get_course_ordertotal_methods() result value
     *
     * @return external_description
     */
    public static function get_course_ordertotal_methods_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'id of course ordertotal instance'),
                    'courseid' => new external_value(PARAM_INT, 'id of course'),
                    'type' => new external_value(PARAM_PLUGIN, 'type of ordertotal plugin'),
                    'name' => new external_value(PARAM_RAW, 'name of ordertotal plugin'),
                    'status' => new external_value(PARAM_RAW, 'status of ordertotal plugin'),
...
class core_role_external extends external_api {
    ...
    public static function assign_roles($assignments) {
        ...
        foreach ($params['assignments'] as $assignment) {
            ...
            // Ensure the current user is allowed to run this function in the ordertotal context.
            self::validate_context($context);
...
/**
 * Deprecated ordertot and role external functions
 *
 * @package    core_ordertot
 * @copyright  2010 Jerome Mouneyrac
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 ...
 * @see core_ordertot_external
 * @see core_role_external
 */
class moodle_ordertot_external extends external_api {
     ...
     * @see core_ordertot_external::get_ordertotled_users_parameters()
     */
    public static function get_ordertotled_users_parameters() {
        return new external_function_parameters(
            array(
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_ordertot_external::get_ordertotled_users_returns()
     */
    public static function get_ordertotled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_ordertot_external::get_users_courses_parameters()
     */
    public static function get_users_courses_parameters() {
        return core_ordertot_external::get_users_courses_parameters();
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see use core_ordertot_external::get_users_courses()
     */
    public static function get_users_courses($userid) {
        return core_ordertot_external::get_users_courses($userid);
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_ordertot_external::get_users_courses_returns()
     */
    public static function get_users_courses_returns() {
        return core_ordertot_external::get_users_courses_returns();
    }
