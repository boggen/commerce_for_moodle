...
/**
 * This file contains the course_ordertotal_manager class which is used to interface
 * with the functions that exist in ordertotlib.php in relation to a single course.
 *
...
/**
 * This class provides a targeted tied together means of interfacing the ordertotal
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_ordertotal_manager {
    ...
    protected $context;
    /**
     * The course we are managing ordertotals for
     * @var stdClass
     */
    protected $course = null;
    /**
     * Limits the focus of the manager to one ordertotal plugin instance
     * @var string
     */
    ...
    /**
     * The total number of users ordertotled in the course
     * Populated by course_ordertotal_manager::get_total_users
     * @var int
     */
    protected $totalusers = null;
    /**
     * An array of users currently ordertotled in the course
     * Populated by course_ordertotal_manager::get_users
     * @var array
     */
     ...
     * been ordertotled in the course
     * @var array
     */
    ...
    /**
     * Constructs the course ordertotal manager
     *
     ...
     * @param int $statusfilter if not -1, filter users with active/inactive ordertotal.
     */
    ...
    protected function get_filter_sql() {
        ...
        // Status condition.
        if ($this->statusfilter === ordertot_USER_ACTIVE) {
            $sql .= " AND ue.status = :active AND e.status = :enabled AND ue.timestart < :now1
                    AND (ue.timeend = 0 OR ue.timeend > :now2)";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('enabled' => ordertot_INSTANCE_ENABLED,
                             'active' => ordertot_USER_ACTIVE,
                             'now1' => $now,
                             'now2' => $now);
        } else if ($this->statusfilter === ordertot_USER_SUSPENDED) {
            $sql .= " AND (ue.status = :inactive OR e.status = :disabled OR ue.timestart > :now1
                    OR (ue.timeend <> 0 AND ue.timeend < :now2))";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('disabled' => ordertot_INSTANCE_DISABLED,
                             'inactive' => ordertot_USER_SUSPENDED,
                             'now1' => $now,
                             'now2' => $now);
        }
...
// ====================================================================================================
    protected function get_instance_sql() {
        ...
        if ($this->_instancessql === null) {
            $instances = $this->get_ordertotal_instances();
            $filter = $this->get_ordertotal_filter();
            if ($filter && array_key_exists($filter, $instances)) {
                ...
                $filter = 0;
                if ($instances) {
                    list($sql, $params) = $DB->get_in_or_equal(array_keys($this->get_ordertotal_instances()), SQL_PARAMS_NAMED);
                } else {
    ...
    /**
     * Returns all of the ordertotal instances for this course.
     *
    ...
    public function get_ordertotal_instances() {
        if ($this->_instances === null) {
            $this->_instances = ordertot_get_instances($this->course->id, false);
        }
        return $this->_instances;
    }

    /**
     * Returns the names for all of the ordertotal instances for this course.
     *
     * NOTE: since 2.4 it includes instances of disabled plugins too.
     *
     * @return array
    ...
    public function get_ordertotal_instance_names() {
        if ($this->_inames === null) {
            $instances = $this->get_ordertotal_instances();
            $plugins = $this->get_ordertotal_plugins(false);
            foreach ($instances as $key=>$instance) {
                ...
                $this->_inames[$key] = $plugins[$instance->ordertot]->get_instance_name($instance);
            }
        }
        return $this->_inames;
    }
    ...
    /**
     * Gets all of the ordertotal plugins that are active for this course.
     *
     * @param bool $onlyenabled return only enabled ordertot plugins
     * @return array
     */

    public function get_ordertotal_plugins($onlyenabled = true) {
        if ($this->_plugins === null) {
            $this->_plugins = ordertot_get_plugins(true);
        }
        ...
        if ($this->_allplugins === null) {
            ...
            foreach (ordertot_get_plugins(false) as $name=>$plugin) {
    ...
    /**
     * Unordertots a user from the course given the users ue entry
     *
     * @global moodle_database $DB
     * @param stdClass $ue
     * @return bool
    ...
    public function unordertot_user($ue) {
        global $DB;
        list ($instance, $plugin) = $this->get_user_ordertotal_components($ue);
        if ($instance && $plugin && $plugin->allow_unordertot_user($instance, $ue) && has_capability("ordertot/$instance->ordertot:unordertot", $this->context)) {
            $plugin->unordertot_user($instance, $ue->userid);
            return true;
        }
        return false;
    }
    ...
    /**
     * Given a user ordertotal record this method returns the plugin and ordertotal
     * instance that relate to it.
     *
     * @param stdClass|int $userordertotal
     * @return array array($instance, $plugin)
     */

    public function get_user_ordertotal_components($userordertotal) {
        global $DB;
        if (is_numeric($userordertotal)) {
            //$userordertotal = $DB->get_record('user_ordertotals', array('id'=>(int)$userordertotal));
        }
        $instances = $this->get_ordertotal_instances();
        $plugins = $this->get_ordertotal_plugins(false);
        if (!$userordertotal || !isset($instances[$userordertotal->ordertotid])) {
            return array(false, false);
        }
        $instance = $instances[$userordertotal->ordertotid];
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        foreach ($ras as $ra) {
            if ($ra->component) {
                if (strpos($ra->component, 'ordertot_') !== 0) {
                    continue;
                }
                if (!$plugin = ordertot_get_plugin(substr($ra->component, 6))) {
                    continue;
                }
...
//============================================================================
//============================================================================
    public function edit_ordertotal($userordertotal, $data) {
        //Only allow editing if the user has the appropriate capability
        //Already checked in /ordertot/users.php but checking again in case this function is called from elsewhere
        list($instance, $plugin) = $this->get_user_ordertotal_components($userordertotal);
        if ($instance && $plugin && $plugin->allow_manage($instance) && has_capability("ordertot/$instance->ordertot:manage", $this->context)) {
            if (!isset($data->status)) {
                $data->status = $userordertotal->status;
            }
            $plugin->update_user_ordertot($instance, $userordertotal->userid, $data->status, $data->timestart, $data->timeend);
            return true;
        }
        return false;
    }
    ...
    /**
     * Returns the current ordertotal filter that is being applied by this class
     * @return string
     */
    public function get_ordertotal_filter() {
...
//============================================================================
    ...
    /**
     * Returns the course this object is managing ordertotals for
     *
     * @return stdClass
     */
...
//============================================================================
     ...
     * course but have not been ordertotled.
     *
     * @param core_ordertot_renderer $renderer
    ...
    public function get_other_users_for_display(core_ordertot_renderer $renderer, moodle_url $pageurl, $sort, $direction, $page, $perpage) {
        ...
        foreach ($userroles as $userrole) {
            ...
            if ($contextid == $this->context->id) {
                ...
                if ($userrole->component) {
                    $changeable = false;
                    if (strpos($userrole->component, 'ordertot_') === 0) {
                ...
                $roletext = get_string('rolefromthiscourse', 'ordertot', $a);
            } else {
                $changeable = false;
                switch ($userrole->contextlevel) {
                    case CONTEXT_COURSE :
                        // Meta course
                        $roletext = get_string('rolefrommetacourse', 'ordertot', $a);
                        break;
                    case CONTEXT_COURSECAT :
                        $roletext = get_string('rolefromcategory', 'ordertot', $a);
                        break;
                    case CONTEXT_SYSTEM:
                    default:
                        $roletext = get_string('rolefromsystem', 'ordertot', $a);
                        break;
                }
            }
    ...
    public function get_manual_ordertot_buttons() {
        $plugins = $this->get_ordertotal_plugins(true); // Skip disabled plugins.
        $buttons = array();
        foreach ($plugins as $plugin) {
            $newbutton = $plugin->get_manual_ordertot_button($this);
            if (is_array($newbutton)) {
                $buttons += $newbutton;
            } else if ($newbutton instanceof ordertot_user_button) {
                $buttons[] = $newbutton;
            }
        }
        return $buttons;
    ...
    public function has_instance($ordertotpluginname) {
        // Make sure manual ordertotals instance exists
        foreach ($this->get_ordertotal_instances() as $instance) {
            if ($instance->ordertot == $ordertotpluginname) {
                return true;
            }
        }
        return false;
...
//============================================================================
    /**
     * Returns the ordertotal plugin that the course manager was being filtered to.
     *
     * If no filter was being applied then this function returns false.
     *
     * @return ordertot_plugin
     */
    public function get_filtered_ordertotal_plugin() {
        $instances = $this->get_ordertotal_instances();
        $plugins = $this->get_ordertotal_plugins(false);
        ...
        return $plugins[$instance->ordertot];
    }



/**
 * A button that is used to ordertot users in a course
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
//============================================================================
//============================================================================
class ordertot_user_button extends single_button {
    ...
    /**
     * Initialises the new ordertot_user_button
     *
     * @staticvar int $count The number of ordertot user buttons already created
    ...
    public function __construct(moodle_url $url, $label, $method = 'post') {
        ...
        $this->class = 'singlebutton ordertotusersbutton';
        $this->formid = 'ordertotusersbutton-'.$count;
    }
...
/**
 * User ordertotal action
 *
 * This class is used to manage a renderable ue action such as editing an user ordertotal or deleting
 * a user ordertotal.
 *
 * @copyright  2011 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_ordertotal_action implements renderable {
...
class ordertot_ajax_exception extends moodle_exception {
    /**
     * Constructor
    ...
    public function __construct($errorcode, $link = '', $a = NULL, $debuginfo = null) {
        parent::__construct($errorcode, 'ordertot', $link, $a, $debuginfo);
    }
}

/**
 * This class is used to manage a bulk operations for ordertotal plugins.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class ordertot_bulk_ordertotal_operation {

    /**
     * The course ordertotal manager
     * @var course_ordertotal_manager
     */
    protected $manager;

    /**
     * The ordertotal plugin to which this operation belongs
     * @var ordertot_plugin
     */
    protected $plugin;
     ...
     */
    public function __construct(course_ordertotal_manager $manager, ordertot_plugin $plugin = null) {
     ...
     * @return ordertot_bulk_ordertotal_change_form|moodleform|false
     */
     ...
     */
    abstract public function process(course_ordertotal_manager $manager, array $users, stdClass $properties);
}
