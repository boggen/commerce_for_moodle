...
/**
 * This is the main renderer for the ordertot section.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_ordertot_renderer extends plugin_renderer_base {

    /**
     * Renders a course ordertotal table
     *
     * @param course_ordertotal_table $table
     * @param moodleform $mform Form that contains filter controls
     * @return string
     */
    public function render_course_ordertotal_users_table(course_ordertotal_users_table $table,
            moodleform $mform) {

        $table->initialise_javascript();

        $buttons = $table->get_manual_ordertot_buttons();
        $buttonhtml = '';
        if (count($buttons) > 0) {
            $buttonhtml .= html_writer::start_tag('div', array('class' => 'ordertot_user_buttons'));
        ...
        if ($table->has_bulk_user_ordertotal_operations()) {
            $content .= html_writer::start_tag('form', array('action' => new moodle_url('/ordertot/bulkchange.php'), 'method' => 'post'));
            ...
            $content .= html_writer::tag('option', get_string('withselectedusers', 'ordertot'), array('value' => ''));
            $options = array('' => get_string('withselectedusers', 'ordertot'));
            foreach ($table->get_bulk_user_ordertotal_operations() as $operation) {
    ...
    /**
     * Renderers the ordertot_user_button.
     *
     * @param ordertot_user_button $button
     * @return string XHTML
     */
    protected function render_ordertot_user_button(ordertot_user_button $button) {
     ...
     * @return string
     */
    protected function render_course_ordertotal_other_users_table(course_ordertotal_other_users_table $table) {
        ...
        $content .= html_writer::tag('div', get_string('otheruserdesc', 'ordertot'), array('class'=>'otherusersdesc'));
        $content .= $this->output->render($table->get_paging_bar());
        $content .= html_writer::table($table);
        $content .= $this->output->render($table->get_paging_bar());
        $searchbutton = $table->get_user_search_button();
    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        $iconordertotremove = $this->output->pix_url('t/delete');
        ...
        foreach ($roles as $roleid=>$role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $icon = html_writer::empty_tag('img', array('alt'=>$strunassign, 'src'=>$iconordertotremove));
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach($groups as $groupid=>$name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                $icon = html_writer::empty_tag('img', array('alt'=>get_string('removefromgroup', 'group', $name), 'src'=>$iconordertotremove));
    ...
    /**
     * Generates the HTML for the given ordertotals + available actions
     *
     * @param int $userid
     ...
     * @return string
     */
    public function user_ordertotals_and_actions($ordertotals) {
        $output = '';
/*
        foreach ($ordertotals as $ue) {
            $ordertotaloutput = $ue['text'].' '.$ue['period'];
            if ($ue['dimmed']) {
                $ordertotaloutput = html_writer::tag('span', $ordertotaloutput, array('class'=>'dimmed_text'));
            } else {
                $ordertotaloutput = html_writer::tag('span', $ordertotaloutput);
            }
            foreach ($ue['actions'] as $action) {
                $ordertotaloutput .= $this->render($action);
            }
            $output .= html_writer::tag('div', $ordertotaloutput, array('class'=>'ordertotal'));
        }
*/
        return $output;
    ...
    /**
     * Renders a user ordertotal action
     * @param user_ordertotal_action $icon
     * @return string
     */
    protected function render_user_ordertotal_action(user_ordertotal_action $icon) {
        return html_writer::link($icon->get_url(), $this->output->render($icon->get_icon()), $icon->get_attributes());
    }
}

/**
 * Main course ordertotal table
 *
 * This table is used to display the ordertotal information for a course.
 * It requires that a course ordertotal manager be provided during constuct with
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_ordertotal_table extends html_table implements renderable {
    ...
    /**
     * The default sort, options are course_ordertotal_table::$sortablefields
     * Default: lastname
     */
    ...
    /**
     * The sort field for this table, should be one of course_ordertotal_table::$sortablefields
     * @var string
     */
    public $sort;
    ...
    /**
     * The users ordertotled in this course
     * @var array
     */
    ...
    /**
     * An array of bulk user ordertotal operations
     * @var array
     */
     ...
     */
    public function __construct(course_ordertotal_manager $manager) {
        ...
        $this->attributes = array('class'=>'userordertotal');
        ...
        $plugin = $manager->get_filtered_ordertotal_plugin();
        if ($plugin and ordertot_is_enabled($plugin->get_name())) {
            $this->bulkoperations = $plugin->get_bulk_operations($manager);
        }
    }

    /**
     * Returns an array of ordertot_user_buttons that are created by the different
     * ordertotal plugins available.
     *
     * @return array
     */
    public function get_manual_ordertot_buttons() {
        return $this->manager->get_manual_ordertot_buttons();
    }
    ...
    public function initialise_javascript() {
        if (has_capability('moodle/role:assign', $this->manager->get_context())) {
            ...
            $modules = array('moodle-ordertot-rolemanager', 'moodle-ordertot-rolemanager-skin');
            $function = 'M.ordertot.rolemanager.init';
            $arguments = array(
                'containerId'=>$this->id,
     ...
     */
    public function set_bulk_user_ordertotal_operations(array $bulkoperations) {
     ...
     * @return array
     */
    public function get_bulk_user_ordertotal_operations() {
        return $this->bulkoperations;
    }

    /**
     * Returns true fi the table is aware of any bulk operations that can be performed on users
     * selected from the currently filtered ordertotal plugins.
     *
     * @return bool
     */
    public function has_bulk_user_ordertotal_operations() {
        return !empty($this->bulkoperations);
    }
}

/**
 * Table control used for ordertotled users
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_ordertotal_users_table extends course_ordertotal_table {
 ...
 *
 * Other users are users who have roles but are not ordertotled.
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_ordertotal_other_users_table extends course_ordertotal_table {
    ...
    public function __construct(course_ordertotal_manager $manager) {
        ...
        $this->attributes = array('class'=>'userordertotal otheruserordertotal');
    }
    ...
    public function get_user_search_button() {
        ...
        if ($count == 1) {
            $this->manager->get_moodlepage()->requires->strings_for_js(array(
                    ...
                    'startdatetoday',
                    'durationdays',
                    'ordertotperiod'), 'ordertot');
            $this->manager->get_moodlepage()->requires->string_for_js('assignrole', 'role');

            $modules = array('moodle-ordertot-otherusersmanager', 'moodle-ordertot-otherusersmanager-skin');
            $function = 'M.ordertot.otherusersmanager.init';
            $arguments = array(
                'courseId'=> $this->manager->get_course()->id,
                'ajaxUrl' => '/ordertot/ajax.php',
