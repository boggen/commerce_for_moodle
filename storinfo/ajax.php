...
require('../config.php');
require_once("$CFG->dirroot/storinfo/locallib.php");
require_once("$CFG->dirroot/storinfo/renderer.php");
...
$PAGE->set_url(new moodle_url('/storinfo/ajax.php', array('id'=>$id, 'action'=>$action)));
...
require_capability('moodle/course:storinforeview', $context);
require_sesskey();
...
//        list ($instance, $plugin) = $manager->get_user_storemultiinfo_components($ue);
        if (!$instance || !$plugin || !storinfo_is_enabled($instance->storinfo) || !$plugin->allow_unstorinfo_user($instance, $ue) || !has_capability("storinfo/$instance->storinfo:unstorinfo", $manager->get_context()) || !$manager->unstorinfo_user($ue)) {
            throw new storinfo_ajax_exception('unstorinfonotpermitted');
        }
        break;
    case 'unassign':
        ...
        if (!has_capability('moodle/role:assign', $manager->get_context()) || !$manager->unassign_role_from_user($user, $role)) {
            throw new storinfo_ajax_exception('unassignnotpermitted');
        }
        break;
    case 'assign':
    ...
    case 'searchotherusers':
        ...
        $useroptions = array();
        // User is not storinfoled, either link to site profile or do not link at all.
        ...
        break;
    default:
        throw new storinfo_ajax_exception('unknowajaxaction');
}
