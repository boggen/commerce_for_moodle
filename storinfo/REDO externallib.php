 ...
 *
 * This api is mostly read only, the actual storinfo and unstorinfo
 * support is in each storinfo plugin.
 *
 ...
 * @since Moodle 2.2
 */
class core_storinfo_external extends external_api {
     ...
     * @since Moodle 2.4
     */
    public static function get_storinfoled_users_with_capability_parameters() {
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) only users with active storemultiinfos. Requires \'moodle/course:storinforeview\' .
     ...
     * a list of the users that are storinfoled in the course and have that capability are returned.
     *
     ...
     * @since  Moodle 2.4
     */
    public static function get_storinfoled_users_with_capability($coursecapabilities, $options) {
        global $CFG, $DB;
        ...
        $params = self::validate_parameters(self::get_storinfoled_users_with_capability_parameters(),
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            // The course:storinfoereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('moodle/course:storinforeview', $coursecontext);
            }
            ...
            foreach ($coursecapability['capabilities'] as $capability) {
                ...
                list($storinfoledsql, $storinfoledparams) = get_storinfoled_sql($coursecontext, $capability, $groupid, $onlyactive);

                $sql = "SELECT u.* FROM {user} u WHERE u.id IN ($storinfoledsql) ORDER BY u.id ASC";

                $storinfoledusers = $DB->get_recordset_sql($sql, $storinfoledparams, $limitfrom, $limitnumber);
                $users = array();
                foreach ($storinfoledusers as $courseuser) {
                    if ($userdetails = user_get_user_details($courseuser, $course, $userfields)) {
                        $users[] = $userdetails;
                    }
                }
                $storinfoledusers->close();
     ...
     * @since Moodle 2.4
     */
    public static function get_storinfoled_users_with_capability_returns() {
        return  new external_multiple_structure( new external_single_structure (
                ...
                array(
                    ...
                    'storinfoledcourses' => new external_multiple_structure(
                        new external_single_structure(
                            array(
                    ...
                    ), 'Courses where the user is storinfoled - limited by which courses the user is able to see', VALUE_OPTIONAL)
                )
                        ), 'List of users that are storinfoled in the course and have the specified capability'),
                    )
                )
            );
    }
    ...
    /**
     * Get list of courses user is storinfoled in (only active storemultiinfos are returned).
    ...
    public static function get_users_courses($userid) {
        ...
        $courses = storinfo_get_users_courses($params['userid'], true, 'id, shortname, fullname, idnumber, visible');
        $result = array();
        ...
        foreach ($courses as $course) {
            ...
            try {
                ...
                // current user can not access this course, sorry we can not disclose who is storinfoled in this course!
                continue;
            }
            ...
            list($storinfoledsqlselect, $storinfoledparams) = get_storinfoled_sql($context);
            $storinfoledsql = "SELECT COUNT('x') FROM ($storinfoledsqlselect) storinfoleduserids";
            $storinfoledusercount = $DB->count_records_sql($storinfoledsql, $storinfoledparams);

            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'storinfoledusercount'=>$storinfoledusercount);
        }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'storinfoledusercount' => new external_value(PARAM_INT, 'Number of storinfoled users in this course'),
     ...
     * @return external_function_parameters
     */
    public static function get_storinfoled_users_parameters() {
        return new external_function_parameters(
            array(
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) return only users with active storemultiinfos and matching time restrictions. This option requires \'moodle/course:storinforeview\' on the course context.
                            * userfields (\'string, string, ...\') return only the values of these user fields.
                            * limitfrom (integer) sql limit from.
     ...
     */
    public static function get_storinfoled_users($courseid, $options = array()) {
        global $CFG, $USER, $DB;
        require_once($CFG->dirroot . "/user/lib.php");

        $params = self::validate_parameters(
            self::get_storinfoled_users_parameters(),
            array(
                'courseid'=>$courseid,
                'options'=>$options
            )
        ...
        // to overwrite this option, you need course:storinfoereview permission
        if ($onlyactive) {
            require_capability('moodle/course:storinforeview', $coursecontext);
        }

        list($storinfoledsql, $storinfoledparams) = get_storinfoled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        $ctxselect = ', ' . context_helper::get_preload_record_columns_sql('ctx');
        $ctxjoin = "LEFT JOIN {context} ctx ON (ctx.instanceid = u.id AND ctx.contextlevel = :contextlevel)";
        $storinfoledparams['contextlevel'] = CONTEXT_USER;
        $sql = "SELECT u.* $ctxselect
                  FROM {user} u $ctxjoin
                 WHERE u.id IN ($storinfoledsql)
                 ORDER BY u.id ASC";
        $storinfoledusers = $DB->get_recordset_sql($sql, $storinfoledparams, $limitfrom, $limitnumber);
        $users = array();
        foreach ($storinfoledusers as $user) {
     ...
     * @return external_description
     */
    public static function get_storinfoled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
    ...
    public static function get_course_storemultiinfo_methods($courseid) {
        ...
        $result = array();
        $storinfoinstances = storinfo_get_instances($params['courseid'], true);
        foreach ($storinfoinstances as $storinfoinstance) {
            if ($storinfoplugin = storinfo_get_plugin($storinfoinstance->storinfo)) {
                if ($instanceinfo = $storinfoplugin->get_storinfo_info($storinfoinstance)) {
...
/**
 * Deprecated storinfo and role external functions
 *
 * @package    core_storinfo
 * @copyright  2010 Jerome Mouneyrac
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 ...
 * @see core_storinfo_external
 * @see core_role_external
 */
class moodle_storinfo_external extends external_api {
     ...
     * @see core_storinfo_external::get_storinfoled_users_parameters()
     */
    public static function get_storinfoled_users_parameters() {
        return new external_function_parameters(
            array(
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storinfo_external::get_storinfoled_users()
     */
    public static function get_storinfoled_users($courseid, $withcapability = null, $groupid = null, $onlyactive = false) {
        global $DB, $CFG, $USER;

        // Do basic automatic PARAM checks on incoming data, using params description
        // If any problems are found then exceptions are thrown with helpful error messages
        $params = self::validate_parameters(self::get_storinfoled_users_parameters(), array(
            'courseid'=>$courseid,
        ...
        list($sqlparams, $params) =  get_storinfoled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
...
//                  GROUP BY ue.userid, e.courseid, u.firstname, u.lastname, u.username, c.id";
        ...
        $storinfoledusers = $DB->get_records_sql($sql, $params);
        $result = array();
        $isadmin = is_siteadmin($USER);
        $canviewfullnames = has_capability('moodle/site:viewfullnames', $context);
        foreach ($storinfoledusers as $storinfoleduser) {
            $profilimgurl = moodle_url::make_pluginfile_url($storinfoleduser->usercontextid, 'user', 'icon', NULL, '/', 'f1');
            $profilimgurlsmall = moodle_url::make_pluginfile_url($storinfoleduser->usercontextid, 'user', 'icon', NULL, '/', 'f2');
            $resultuser = array(
                'courseid' => $storinfoleduser->courseid,
                'userid' => $storinfoleduser->userid,
                'fullname' => fullname($storinfoleduser),
                'profileimgurl' => $profilimgurl->out(false),
                'profileimgurlsmall' => $profilimgurlsmall->out(false)
            );
            // check if we can return username
            if ($isadmin) {
                $resultuser['username'] = $storinfoleduser->username;
            }
            // check if we can return first and last name
            if ($isadmin or $canviewfullnames) {
                $resultuser['firstname'] = $storinfoleduser->firstname;
                $resultuser['lastname'] = $storinfoleduser->lastname;
            }
            $result[] = $resultuser;
        }
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storinfo_external::get_storinfoled_users_returns()
     */
    public static function get_storinfoled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storinfo_external::get_users_courses_parameters()
     */
    public static function get_users_courses_parameters() {
        return core_storinfo_external::get_users_courses_parameters();
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see use core_storinfo_external::get_users_courses()
     */
    public static function get_users_courses($userid) {
        return core_storinfo_external::get_users_courses($userid);
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storinfo_external::get_users_courses_returns()
     */
    public static function get_users_courses_returns() {
        return core_storinfo_external::get_users_courses_returns();
    }
