...
if ($hassiteconfig){
    // checkout plugins
    $ADMIN->add('modules', new admin_category('checkingouts', new lang_string('checkingouts', 'checkout')));
    $temp = new admin_settingpage('managecheckouts', new lang_string('managecheckouts', 'checkout'));
    $temp->add(new admin_setting_managecheckouts());
    $ADMIN->add('checkingouts', $temp);
	
    $temp = new admin_externalpage('checkouttestsettings', get_string('testsettings', 'core_checkout'), new moodle_url("/checkout/test_settings.php"), 'moodle/site:config', true);
    $ADMIN->add('checkingouts', $temp);
	
    foreach(core_plugin_manager::instance()->get_plugins_of_type('checkout') as $plugin) {
        /** @var \core\plugininfo\checkout $plugin */
        $plugin->load_settings($ADMIN, 'checkingouts', $hassiteconfig);
    }
// ordertot plugins
    $ADMIN->add('modules', new admin_category('ordertotals', new lang_string('ordertotals', 'ordertot')));
    $temp = new admin_settingpage('manageordertots', new lang_string('manageordertots', 'ordertot'));
    $temp->add(new admin_setting_manageordertots());
    $ADMIN->add('ordertotals', $temp);
	
    $temp = new admin_externalpage('ordertottestsettings', get_string('testsettings', 'core_ordertot'), new moodle_url("/ordertot/test_settings.php"), 'moodle/site:config', true);
    $ADMIN->add('ordertotals', $temp);
	
    foreach(core_plugin_manager::instance()->get_plugins_of_type('ordertot') as $plugin) {
        /** @var \core\plugininfo\ordertot $plugin */
        $plugin->load_settings($ADMIN, 'ordertotals', $hassiteconfig);
    }
// paygate plugins
    $ADMIN->add('modules', new admin_category('paymentgateways', new lang_string('paymentgateways', 'paygate')));
    $temp = new admin_settingpage('managepaygates', new lang_string('managepaygates', 'paygate'));
    $temp->add(new admin_setting_managepaygates());
    $ADMIN->add('paymentgateways', $temp);
	
    $temp = new admin_externalpage('paygatetestsettings', get_string('testsettings', 'core_paygate'), new moodle_url("/paygate/test_settings.php"), 'moodle/site:config', true);
    $ADMIN->add('paymentgateways', $temp);
	
    foreach(core_plugin_manager::instance()->get_plugins_of_type('paygate') as $plugin) {
        /** @var \core\plugininfo\paygate $plugin */
        $plugin->load_settings($ADMIN, 'paymentgateways', $hassiteconfig);
    }
// prodinfo plugins
    $ADMIN->add('modules', new admin_category('productinfos', new lang_string('productinfos', 'prodinfo')));
    $temp = new admin_settingpage('manageprodinfos', new lang_string('manageprodinfos', 'prodinfo'));
    $temp->add(new admin_setting_manageprodinfos());
    $ADMIN->add('productinfos', $temp);
	
    $temp = new admin_externalpage('prodinfotestsettings', get_string('testsettings', 'core_prodinfo'), new moodle_url("/prodinfo/test_settings.php"), 'moodle/site:config', true);
    $ADMIN->add('productinfos', $temp);
	
    foreach(core_plugin_manager::instance()->get_plugins_of_type('prodinfo') as $plugin) {
        /** @var \core\plugininfo\prodinfo $plugin */
        $plugin->load_settings($ADMIN, 'productinfos', $hassiteconfig);
    }
// shipping plugins
    $ADMIN->add('modules', new admin_category('shipments', new lang_string('shipments', 'shipping')));
    $temp = new admin_settingpage('manageshippings', new lang_string('manageshippings', 'shipping'));
    $temp->add(new admin_setting_manageshippings());
    $ADMIN->add('shipments', $temp);
	
    $temp = new admin_externalpage('shippingtestsettings', get_string('testsettings', 'core_shipping'), new moodle_url("/shipping/test_settings.php"), 'moodle/site:config', true);
    $ADMIN->add('shipments', $temp);
	
    foreach(core_plugin_manager::instance()->get_plugins_of_type('shipping') as $plugin) {
