...
class helper {
    ...
    public static function get_course_detail_array(\course_in_list $course) {
		...
		//checkout 
        if ($course->can_review_checkingouts()) {
            ...
            foreach ($instances as $instance) {
                ...
                $checkingoutlines[] = $plugin->get_instance_name($instance);
            }
            $details['checkingoutmethods'] = array(
			'key' => \get_string('checkingoutmethods'),
			'value' => join('<br />', $checkingoutlines)
            );
        }
		//ordertot 
        if ($course->can_review_ordertotals()) {
            $paymentgatwaylines = array();
            ...
            foreach ($instances as $instance) {
                ...
                $ordertotallines[] = $plugin->get_instance_name($instance);
            }
            $details['ordertotalmethods'] = array(
			'key' => \get_string('ordertotalmethods'),
			'value' => join('<br />', $ordertotallines)
            );
        }
		//paygate 
        if ($course->can_review_paymentgateways()) {
            $paymentgatwaylines = array();
            ...
            foreach ($instances as $instance) {
                ...
                $paymentgatewaylines[] = $plugin->get_instance_name($instance);
            }
            $details['paymentgatewaymethods'] = array(
			'key' => \get_string('paymentgatewaymethods'),
			'value' => join('<br />', $paymentgatewaylines)
            );
        }
		//prodinfo 
        if ($course->can_review_productinfos()) {
            $paymentgatwaylines = array();
            ...
            foreach ($instances as $instance) {
                ...
                $productinfolines[] = $plugin->get_instance_name($instance);
            }
            $details['productinfomethods'] = array(
			'key' => \get_string('productinfomethods'),
			'value' => join('<br />', $productinfolines)
            );
        }
		//shipping 
        if ($course->can_review_shipments()) {
            $paymentgatwaylines = array();
            ...
            foreach ($instances as $instance) {
                ...
                $shipmentlines[] = $plugin->get_instance_name($instance);
            }
            $details['shipmentmethods'] = array(
			'key' => \get_string('shipmentmethods'),
        ...
        if ($course->can_review_storemultiinfos()) {
			...
			'key' => \get_string('storemultiinfomethods'),
			'value' => join('<br />', $storemultiinfolines)
            );
        }
        if ($canaccess) {
    ...
    public static function get_course_detail_actions(\course_in_list $course) {
		...
		/** users.php removed from general plugin no need for permissions here
		// Permissions checkout.
        if ($course->can_review_checkoutments()) {
            $actions['checkoutledusers'] = array(
			'url' => new \moodle_url('/checkout/users.php', array('id' => $course->id)),
			'string' => \get_string('checkoutledusers', 'checkout')
            );
        }
		// Permissions ordertot.
        if ($course->can_review_ordertotments()) {
            $actions['ordertotledusers'] = array(
			'url' => new \moodle_url('/ordertot/users.php', array('id' => $course->id)),
			'string' => \get_string('ordertotledusers', 'ordertot')
            );
        }
		// Permissions paygate.
        if ($course->can_review_paygatements()) {
            $actions['paygateledusers'] = array(
			'url' => new \moodle_url('/paygate/users.php', array('id' => $course->id)),
			'string' => \get_string('paygateledusers', 'paygate')
            );
        }
		// Permissions prodinfo.
        if ($course->can_review_prodinfoments()) {
            $actions['prodinfoledusers'] = array(
			'url' => new \moodle_url('/prodinfo/users.php', array('id' => $course->id)),
			'string' => \get_string('prodinfoledusers', 'prodinfo')
            );
        }
		// Permissions shipping.
        if ($course->can_review_shippingments()) {
            $actions['shippingledusers'] = array(
