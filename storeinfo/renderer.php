...
/**
 * This is the main renderer for the storeinfo section.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_storeinfo_renderer extends plugin_renderer_base {

    /**
     * Renders a course storemultiinfo table
     *
     * @param course_storemultiinfo_table $table
     * @param moodleform $mform Form that contains filter controls
     * @return string
     */
    public function render_course_storemultiinfo_users_table(course_storemultiinfo_users_table $table,
            moodleform $mform) {

        $table->initialise_javascript();

        $buttons = $table->get_manual_storeinfo_buttons();
        $buttonhtml = '';
        if (count($buttons) > 0) {
            $buttonhtml .= html_writer::start_tag('div', array('class' => 'storeinfo_user_buttons'));
        ...
        if ($table->has_bulk_user_storemultiinfo_operations()) {
            $content .= html_writer::start_tag('form', array('action' => new moodle_url('/storeinfo/bulkchange.php'), 'method' => 'post'));
            ...
            $content .= html_writer::tag('option', get_string('withselectedusers', 'storeinfo'), array('value' => ''));
            $options = array('' => get_string('withselectedusers', 'storeinfo'));
            foreach ($table->get_bulk_user_storemultiinfo_operations() as $operation) {
    ...
    /**
     * Renderers the storeinfo_user_button.
     *
     * @param storeinfo_user_button $button
     * @return string XHTML
     */
    protected function render_storeinfo_user_button(storeinfo_user_button $button) {
     ...
     * @return string
     */
    protected function render_course_storemultiinfo_other_users_table(course_storemultiinfo_other_users_table $table) {
        ...
        $content .= html_writer::tag('div', get_string('otheruserdesc', 'storeinfo'), array('class'=>'otherusersdesc'));
        $content .= $this->output->render($table->get_paging_bar());
        $content .= html_writer::table($table);
        $content .= $this->output->render($table->get_paging_bar());
        $searchbutton = $table->get_user_search_button();
    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        $iconstoreinforemove = $this->output->pix_url('t/delete');
        ...
        foreach ($roles as $roleid=>$role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $icon = html_writer::empty_tag('img', array('alt'=>$strunassign, 'src'=>$iconstoreinforemove));
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach($groups as $groupid=>$name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                $icon = html_writer::empty_tag('img', array('alt'=>get_string('removefromgroup', 'group', $name), 'src'=>$iconstoreinforemove));
    ...
    /**
     * Generates the HTML for the given storemultiinfos + available actions
     *
     * @param int $userid
     * @param array $storemultiinfos
     * @param moodle_url $pageurl
     * @return string
     */
    public function user_storemultiinfos_and_actions($storemultiinfos) {
        $output = '';
        foreach ($storemultiinfos as $ue) {
            $storemultiinfooutput = $ue['text'].' '.$ue['period'];
            if ($ue['dimmed']) {
                $storemultiinfooutput = html_writer::tag('span', $storemultiinfooutput, array('class'=>'dimmed_text'));
            } else {
                $storemultiinfooutput = html_writer::tag('span', $storemultiinfooutput);
            }
            foreach ($ue['actions'] as $action) {
                $storemultiinfooutput .= $this->render($action);
            }
            $output .= html_writer::tag('div', $storemultiinfooutput, array('class'=>'storemultiinfo'));
        }
        return $output;
    }

    /**
     * Renders a user storemultiinfo action
     * @param user_storemultiinfo_action $icon
     * @return string
     */
    protected function render_user_storemultiinfo_action(user_storemultiinfo_action $icon) {
        return html_writer::link($icon->get_url(), $this->output->render($icon->get_icon()), $icon->get_attributes());
    }
}

/**
 * Main course storemultiinfo table
 *
 * This table is used to display the storemultiinfo information for a course.
 * It requires that a course storemultiinfo manager be provided during constuct with
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_storemultiinfo_table extends html_table implements renderable {
    ...
    /**
     * The default sort, options are course_storemultiinfo_table::$sortablefields
     * Default: lastname
     */
    ...
    /**
     * The sort field for this table, should be one of course_storemultiinfo_table::$sortablefields
     * @var string
     */
    public $sort;
    ...
    /**
     * The users storeinfoled in this course
     * @var array
     */
    ...
    /**
     * An array of bulk user storemultiinfo operations
     * @var array
     */
     ...
     */
    public function __construct(course_storemultiinfo_manager $manager) {
        ...
        $this->attributes = array('class'=>'userstoremultiinfo');
        ...
        $plugin = $manager->get_filtered_storemultiinfo_plugin();
        if ($plugin and storeinfo_is_enabled($plugin->get_name())) {
            $this->bulkoperations = $plugin->get_bulk_operations($manager);
        }
    }

    /**
     * Returns an array of storeinfo_user_buttons that are created by the different
     * storemultiinfo plugins available.
     *
     * @return array
     */
    public function get_manual_storeinfo_buttons() {
        return $this->manager->get_manual_storeinfo_buttons();
    }
    ...
    public function initialise_javascript() {
        if (has_capability('moodle/role:assign', $this->manager->get_context())) {
            ...
            $modules = array('moodle-storeinfo-rolemanager', 'moodle-storeinfo-rolemanager-skin');
            $function = 'M.storeinfo.rolemanager.init';
            $arguments = array(
                'containerId'=>$this->id,
     ...
     */
    public function set_bulk_user_storemultiinfo_operations(array $bulkoperations) {
     ...
     * @return array
     */
    public function get_bulk_user_storemultiinfo_operations() {
        return $this->bulkoperations;
    }

    /**
     * Returns true fi the table is aware of any bulk operations that can be performed on users
     * selected from the currently filtered storemultiinfo plugins.
     *
     * @return bool
     */
    public function has_bulk_user_storemultiinfo_operations() {
        return !empty($this->bulkoperations);
    }
}

/**
 * Table control used for storeinfoled users
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_storemultiinfo_users_table extends course_storemultiinfo_table {
 ...
 *
 * Other users are users who have roles but are not storeinfoled.
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_storemultiinfo_other_users_table extends course_storemultiinfo_table {
    ...
    public function __construct(course_storemultiinfo_manager $manager) {
        ...
        $this->attributes = array('class'=>'userstoremultiinfo otheruserstoremultiinfo');
    }
    ...
    public function get_user_search_button() {
        ...
        if ($count == 1) {
            $this->manager->get_moodlepage()->requires->strings_for_js(array(
                    ...
                    'ajaxnext25',
                    'storeinfo',
                    'storemultiinfooptions',
                    'storeinfousers',
                    'errajaxfailedstoreinfo',
                    'errajaxsearch',
                    'none',
                    'usersearch',
                    'unlimitedduration',
                    'startdatetoday',
                    'durationdays',
                    'storeinfoperiod'), 'storeinfo');
            $this->manager->get_moodlepage()->requires->string_for_js('assignrole', 'role');

            $modules = array('moodle-storeinfo-otherusersmanager', 'moodle-storeinfo-otherusersmanager-skin');
            $function = 'M.storeinfo.otherusersmanager.init';
            $arguments = array(
                'courseId'=> $this->manager->get_course()->id,
                'ajaxUrl' => '/storeinfo/ajax.php',
