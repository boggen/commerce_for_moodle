...
/**
 * taxclass storemultiinfo plugin.
 *
 * This plugin lets the user specify a "taxclass" (CSV) containing storemultiinfo information.
...
/**
 * taxclass storemultiinfo plugin implementation.
 *
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class storeinfo_taxclass_plugin extends storeinfo_plugin {
    ...
    public function can_delete_instance($instance) {
        ...
        return has_capability('storeinfo/taxclass:manage', $context);
    }
    ...
    /**
     * Returns the user who is responsible for taxclass storemultiinfos in given curse.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'storeinfo/taxclass:manage'
     * or 'moodle/role:assign' capability.
     *
     * @param int $courseid storemultiinfo instance id
     * @return stdClass user record
     */
    protected function get_storeinfoler($courseid) {
        ...
        $users = get_storeinfoled_users($context, 'storeinfo/taxclass:manage');
        if (!$users) {
            $users = get_storeinfoled_users($context, 'moodle/role:assign');
        }
    ...
    public function restore_instance(restore_storemultiinfos_structure_step $step, stdClass $data, $course, $oldid) {
        ...
        if ($instance = $DB->get_record('storeinfo', array('courseid'=>$course->id, 'storeinfo'=>$this->get_name()))) {
            $instanceid = $instance->id;
        } else {
            $instanceid = $this->add_instance($course);
        }
        $step->set_mapping('storeinfo', $oldid, $instanceid);
    }
    ...
    public function restore_user_storemultiinfo(restore_storemultiinfos_structure_step $step, $data, $instance, $userid, $oldinstancestatus) {
        $this->storeinfo_user($instance, $userid, null, $data->timestart, $data->timeend, $data->status);
    }
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        role_assign($roleid, $userid, $contextid, 'storeinfo_'.$instance->storeinfo, $instance->id);
    }
}
