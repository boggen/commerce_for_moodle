 ...
 *
 * This api is mostly read only, the actual storeinfo and unstoreinfo
 * support is in each storeinfo plugin.
 *
 ...
 * @since Moodle 2.2
 */
class core_storeinfo_external extends external_api {
     ...
     * @since Moodle 2.4
     */
    public static function get_storeinfoled_users_with_capability_parameters() {
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) only users with active storemultiinfos. Requires \'moodle/course:storeinforeview\' .
     ...
     * a list of the users that are storeinfoled in the course and have that capability are returned.
     *
     ...
     * @since  Moodle 2.4
     */
    public static function get_storeinfoled_users_with_capability($coursecapabilities, $options) {
        global $CFG, $DB;
        ...
        $params = self::validate_parameters(self::get_storeinfoled_users_with_capability_parameters(),
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            // The course:storeinfoereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('moodle/course:storeinforeview', $coursecontext);
            }
            ...
            foreach ($coursecapability['capabilities'] as $capability) {
                ...
                list($storeinfoledsql, $storeinfoledparams) = get_storeinfoled_sql($coursecontext, $capability, $groupid, $onlyactive);

                $sql = "SELECT u.* FROM {user} u WHERE u.id IN ($storeinfoledsql) ORDER BY u.id ASC";

                $storeinfoledusers = $DB->get_recordset_sql($sql, $storeinfoledparams, $limitfrom, $limitnumber);
                $users = array();
                foreach ($storeinfoledusers as $courseuser) {
                    if ($userdetails = user_get_user_details($courseuser, $course, $userfields)) {
                        $users[] = $userdetails;
                    }
                }
                $storeinfoledusers->close();
     ...
     * @since Moodle 2.4
     */
    public static function get_storeinfoled_users_with_capability_returns() {
        return  new external_multiple_structure( new external_single_structure (
                ...
                array(
                    ...
                    'storeinfoledcourses' => new external_multiple_structure(
                        new external_single_structure(
                            array(
                    ...
                    ), 'Courses where the user is storeinfoled - limited by which courses the user is able to see', VALUE_OPTIONAL)
                )
                        ), 'List of users that are storeinfoled in the course and have the specified capability'),
                    )
                )
            );
    }
    ...
    /**
     * Get list of courses user is storeinfoled in (only active storemultiinfos are returned).
    ...
    public static function get_users_courses($userid) {
        ...
        $courses = storeinfo_get_users_courses($params['userid'], true, 'id, shortname, fullname, idnumber, visible');
        $result = array();
        ...
        foreach ($courses as $course) {
            ...
            try {
                ...
                // current user can not access this course, sorry we can not disclose who is storeinfoled in this course!
                continue;
            }
            ...
            list($storeinfoledsqlselect, $storeinfoledparams) = get_storeinfoled_sql($context);
            $storeinfoledsql = "SELECT COUNT('x') FROM ($storeinfoledsqlselect) storeinfoleduserids";
            $storeinfoledusercount = $DB->count_records_sql($storeinfoledsql, $storeinfoledparams);

            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'storeinfoledusercount'=>$storeinfoledusercount);
        }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'storeinfoledusercount' => new external_value(PARAM_INT, 'Number of storeinfoled users in this course'),
     ...
     * @return external_function_parameters
     */
    public static function get_storeinfoled_users_parameters() {
        return new external_function_parameters(
            array(
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) return only users with active storemultiinfos and matching time restrictions. This option requires \'moodle/course:storeinforeview\' on the course context.
                            * userfields (\'string, string, ...\') return only the values of these user fields.
                            * limitfrom (integer) sql limit from.
     ...
     */
    public static function get_storeinfoled_users($courseid, $options = array()) {
        global $CFG, $USER, $DB;
        require_once($CFG->dirroot . "/user/lib.php");

        $params = self::validate_parameters(
            self::get_storeinfoled_users_parameters(),
            array(
                'courseid'=>$courseid,
                'options'=>$options
            )
        ...
        // to overwrite this option, you need course:storeinfoereview permission
        if ($onlyactive) {
            require_capability('moodle/course:storeinforeview', $coursecontext);
        }

        list($storeinfoledsql, $storeinfoledparams) = get_storeinfoled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        $ctxselect = ', ' . context_helper::get_preload_record_columns_sql('ctx');
        $ctxjoin = "LEFT JOIN {context} ctx ON (ctx.instanceid = u.id AND ctx.contextlevel = :contextlevel)";
        $storeinfoledparams['contextlevel'] = CONTEXT_USER;
        $sql = "SELECT u.* $ctxselect
                  FROM {user} u $ctxjoin
                 WHERE u.id IN ($storeinfoledsql)
                 ORDER BY u.id ASC";
        $storeinfoledusers = $DB->get_recordset_sql($sql, $storeinfoledparams, $limitfrom, $limitnumber);
        $users = array();
        foreach ($storeinfoledusers as $user) {
     ...
     * @return external_description
     */
    public static function get_storeinfoled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
    ...
    /**
     * Returns description of get_course_storemultiinfo_methods() parameters
     *
     * @return external_function_parameters
     */
    public static function get_course_storemultiinfo_methods_parameters() {
        return new external_function_parameters(
            array(
                'courseid' => new external_value(PARAM_INT, 'Course id')
            )
    ...
    /**
     * Get list of active course storemultiinfo methods for current user.
     *
     * @param int $courseid
     * @return array of course storemultiinfo methods
     */
    public static function get_course_storemultiinfo_methods($courseid) {

        $params = self::validate_parameters(self::get_course_storemultiinfo_methods_parameters(), array('courseid' => $courseid));
        ...
        $result = array();
        $storeinfoinstances = storeinfo_get_instances($params['courseid'], true);
        foreach ($storeinfoinstances as $storeinfoinstance) {
            if ($storeinfoplugin = storeinfo_get_plugin($storeinfoinstance->storeinfo)) {
                if ($instanceinfo = $storeinfoplugin->get_storeinfo_info($storeinfoinstance)) {
    ...
    /**
     * Returns description of get_course_storemultiinfo_methods() result value
     *
     * @return external_description
     */
    public static function get_course_storemultiinfo_methods_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'id of course storemultiinfo instance'),
                    'courseid' => new external_value(PARAM_INT, 'id of course'),
                    'type' => new external_value(PARAM_PLUGIN, 'type of storemultiinfo plugin'),
                    'name' => new external_value(PARAM_RAW, 'name of storemultiinfo plugin'),
                    'status' => new external_value(PARAM_RAW, 'status of storemultiinfo plugin'),
...
class core_role_external extends external_api {
    ...
    public static function assign_roles($assignments) {
        ...
        foreach ($params['assignments'] as $assignment) {
            ...
            // Ensure the current user is allowed to run this function in the storemultiinfo context.
            self::validate_context($context);
...
/**
 * Deprecated storeinfo and role external functions
 *
 * @package    core_storeinfo
 * @copyright  2010 Jerome Mouneyrac
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 ...
 * @see core_storeinfo_external
 * @see core_role_external
 */
class moodle_storeinfo_external extends external_api {
     ...
     * @see core_storeinfo_external::get_storeinfoled_users_parameters()
     */
    public static function get_storeinfoled_users_parameters() {
        return new external_function_parameters(
            array(
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storeinfo_external::get_storeinfoled_users()
     */
    public static function get_storeinfoled_users($courseid, $withcapability = null, $groupid = null, $onlyactive = false) {
        global $DB, $CFG, $USER;

        // Do basic automatic PARAM checks on incoming data, using params description
        // If any problems are found then exceptions are thrown with helpful error messages
        $params = self::validate_parameters(self::get_storeinfoled_users_parameters(), array(
            'courseid'=>$courseid,
        ...
        list($sqlparams, $params) =  get_storeinfoled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
...
//                  GROUP BY ue.userid, e.courseid, u.firstname, u.lastname, u.username, c.id";
        ...
        $storeinfoledusers = $DB->get_records_sql($sql, $params);
        $result = array();
        $isadmin = is_siteadmin($USER);
        $canviewfullnames = has_capability('moodle/site:viewfullnames', $context);
        foreach ($storeinfoledusers as $storeinfoleduser) {
            $profilimgurl = moodle_url::make_pluginfile_url($storeinfoleduser->usercontextid, 'user', 'icon', NULL, '/', 'f1');
            $profilimgurlsmall = moodle_url::make_pluginfile_url($storeinfoleduser->usercontextid, 'user', 'icon', NULL, '/', 'f2');
            $resultuser = array(
                'courseid' => $storeinfoleduser->courseid,
                'userid' => $storeinfoleduser->userid,
                'fullname' => fullname($storeinfoleduser),
                'profileimgurl' => $profilimgurl->out(false),
                'profileimgurlsmall' => $profilimgurlsmall->out(false)
            );
            // check if we can return username
            if ($isadmin) {
                $resultuser['username'] = $storeinfoleduser->username;
            }
            // check if we can return first and last name
            if ($isadmin or $canviewfullnames) {
                $resultuser['firstname'] = $storeinfoleduser->firstname;
                $resultuser['lastname'] = $storeinfoleduser->lastname;
            }
            $result[] = $resultuser;
        }
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storeinfo_external::get_storeinfoled_users_returns()
     */
    public static function get_storeinfoled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storeinfo_external::get_users_courses_parameters()
     */
    public static function get_users_courses_parameters() {
        return core_storeinfo_external::get_users_courses_parameters();
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see use core_storeinfo_external::get_users_courses()
     */
    public static function get_users_courses($userid) {
        return core_storeinfo_external::get_users_courses($userid);
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_storeinfo_external::get_users_courses_returns()
     */
    public static function get_users_courses_returns() {
        return core_storeinfo_external::get_users_courses_returns();
    }
