 ...
 * with the functions that exist in storeinfolib.php in relation to a single course.
 *
...
class course_storemultiinfo_manager {
    ...
    /**
     * The total number of users storeinfoled in the course
     * Populated by course_storemultiinfo_manager::get_total_users
     * @var int
     */
    protected $totalusers = null;
    /**
     * An array of users currently storeinfoled in the course
     ...
     * been storeinfoled in the course
     * @var array
     */
    ...
    /**
     * Returns the total number of storeinfoled users in the course.
     *
     * If a filter was specificed this will be the total number of users storeinfoled
    ...
    public function get_total_users() {
        ...
        if ($this->totalusers === null) {
            ...
            $sqltotal = "SELECT COUNT(DISTINCT u.id)
                           FROM {user} u
                           JOIN {user_storemultiinfos} ue ON (ue.userid = u.id  AND ue.storeinfoid $instancessql)
        ...
        if ($this->totalotherusers === null) {
            ...
            $sql = "SELECT COUNT(DISTINCT u.id)
                 ...
                 LEFT JOIN (
                        ...
                        LEFT JOIN {storeinfo} e ON e.id=ue.storeinfoid
    ...
    /**
     * Gets all of the users storeinfoled in this course.
     *
     * If a filter was specified this will be the users who were storeinfoled
    ...
    protected function get_filter_sql() {
        ...
        // Status condition.
        if ($this->statusfilter === storeinfo_USER_ACTIVE) {
            $sql .= " AND ue.status = :active AND e.status = :enabled AND ue.timestart < :now1
                    AND (ue.timeend = 0 OR ue.timeend > :now2)";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('enabled' => storeinfo_INSTANCE_ENABLED,
                             'active' => storeinfo_USER_ACTIVE,
                             'now1' => $now,
                             'now2' => $now);
        } else if ($this->statusfilter === storeinfo_USER_SUSPENDED) {
            $sql .= " AND (ue.status = :inactive OR e.status = :disabled OR ue.timestart > :now1
                    OR (ue.timeend <> 0 AND ue.timeend < :now2))";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('disabled' => storeinfo_INSTANCE_DISABLED,
                             'inactive' => storeinfo_USER_SUSPENDED,
                             'now1' => $now,
                             'now2' => $now);
        }
     ...
     * within this course but who have not been storeinfoled in the course
     *
     * @global moodle_database $DB
     * @param string $sort
    ...
    /**
     * Gets an array of the users that can be storeinfoled in this course.
     *
     * @global moodle_database $DB
     * @param int $storeinfoid
     * @param string $search
    ...
    public function get_potential_users($storeinfoid, $search='', $searchanywhere=false, $page=0, $perpage=25, $addedstoremultiinfo=0) {
        global $DB;
        ...
        $sql = " FROM {user} u
            LEFT JOIN {user_storemultiinfos} ue ON (ue.userid = u.id AND ue.storeinfoid = :storeinfoid)
                WHERE $wherecondition
                      AND ue.id IS NULL";
        $params['storeinfoid'] = $storeinfoid;
...
// ====================================================================================================
    ...
    public function get_storemultiinfo_instances() {
        if ($this->_instances === null) {
            $this->_instances = storeinfo_get_instances($this->course->id, false);
        }
    ...
    public function get_storemultiinfo_instance_names() {
        if ($this->_inames === null) {
            ...
            foreach ($instances as $key=>$instance) {
                ...
                $this->_inames[$key] = $plugins[$instance->storeinfo]->get_instance_name($instance);
            }
        }
        return $this->_inames;
    }
     ...
     *
     * @param bool $onlyenabled return only enabled storeinfo plugins
     * @return array
     */

    public function get_storemultiinfo_plugins($onlyenabled = true) {
        if ($this->_plugins === null) {
            $this->_plugins = storeinfo_get_plugins(true);
        }
        ...
        if ($this->_allplugins === null) {
            ...
            foreach (storeinfo_get_plugins(false) as $name=>$plugin) {
    ...
    /**
     * Unstoreinfos a user from the course given the users ue entry
     *
     * @global moodle_database $DB
     * @param stdClass $ue
     * @return bool
    ...
    public function unstoreinfo_user($ue) {
        global $DB;
        list ($instance, $plugin) = $this->get_user_storemultiinfo_components($ue);
        if ($instance && $plugin && $plugin->allow_unstoreinfo_user($instance, $ue) && has_capability("storeinfo/$instance->storeinfo:unstoreinfo", $this->context)) {
            $plugin->unstoreinfo_user($instance, $ue->userid);
            return true;
        }
        return false;
    }
    ...
    public function get_user_storemultiinfo_components($userstoremultiinfo) {
        ...
        $instances = $this->get_storemultiinfo_instances();
        $plugins = $this->get_storemultiinfo_plugins(false);
        if (!$userstoremultiinfo || !isset($instances[$userstoremultiinfo->storeinfoid])) {
            return array(false, false);
        }
        $instance = $instances[$userstoremultiinfo->storeinfoid];
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        foreach ($ras as $ra) {
            if ($ra->component) {
                if (strpos($ra->component, 'storeinfo_') !== 0) {
                    continue;
                }
                if (!$plugin = storeinfo_get_plugin(substr($ra->component, 6))) {
                    continue;
                }
...
//============================================================================
    public function edit_storemultiinfo($userstoremultiinfo, $data) {
        ...
        //Already checked in /storeinfo/users.php but checking again in case this function is called from elsewhere
        list($instance, $plugin) = $this->get_user_storemultiinfo_components($userstoremultiinfo);
        if ($instance && $plugin && $plugin->allow_manage($instance) && has_capability("storeinfo/$instance->storeinfo:manage", $this->context)) {
            if (!isset($data->status)) {
                $data->status = $userstoremultiinfo->status;
            }
            $plugin->update_user_storeinfo($instance, $userstoremultiinfo->userid, $data->status, $data->timestart, $data->timeend);
            return true;
        }
        return false;
    }
    ...
    public function get_user_storemultiinfos($userid) {
        ...
        $userstoremultiinfos = $DB->get_records_select('user_storemultiinfos', "storeinfoid $instancessql AND userid = :userid", $params);
        $instances = $this->get_storemultiinfo_instances();
        $plugins = $this->get_storemultiinfo_plugins(false);
        $inames = $this->get_storemultiinfo_instance_names();
        foreach ($userstoremultiinfos as &$ue) {
            $ue->storemultiinfoinstance     = $instances[$ue->storeinfoid];
            $ue->storemultiinfoplugin       = $plugins[$ue->storemultiinfoinstance->storeinfo];
...
//============================================================================
     ...
     * course but have not been storeinfoled.
     *
     * @param core_storeinfo_renderer $renderer
    ...
    public function get_other_users_for_display(core_storeinfo_renderer $renderer, moodle_url $pageurl, $sort, $direction, $page, $perpage) {
        ...
        foreach ($userroles as $userrole) {
            ...
            if ($contextid == $this->context->id) {
                ...
                if ($userrole->component) {
                    $changeable = false;
                    if (strpos($userrole->component, 'storeinfo_') === 0) {
                ...
                $roletext = get_string('rolefromthiscourse', 'storeinfo', $a);
            } else {
                $changeable = false;
                switch ($userrole->contextlevel) {
                    case CONTEXT_COURSE :
                        // Meta course
                        $roletext = get_string('rolefrommetacourse', 'storeinfo', $a);
                        break;
                    case CONTEXT_COURSECAT :
                        $roletext = get_string('rolefromcategory', 'storeinfo', $a);
                        break;
                    case CONTEXT_SYSTEM:
                    default:
                        $roletext = get_string('rolefromsystem', 'storeinfo', $a);
                        break;
                }
            }
    ...
    public function get_users_for_display(course_storemultiinfo_manager $manager, $sort, $direction, $page, $perpage) {
        ...
        $strunstoreinfo = get_string('unstoreinfo', 'storeinfo');
        ...
        foreach ($users as $user) {
            ...
            foreach ($this->get_user_storemultiinfos($user->id) as $ue) {
                if (!isset($enabledplugins[$ue->storemultiinfoinstance->storeinfo])) {
                    ...
                    $period = get_string('periodstartend', 'storeinfo', array('start'=>userdate($ue->timestart), 'end'=>userdate($ue->timeend)));
                    $periodoutside = ($ue->timestart && $ue->timeend && ($now < $ue->timestart || $now > $ue->timeend));
                } else if ($ue->timestart) {
                    $period = get_string('periodstart', 'storeinfo', userdate($ue->timestart));
                    $periodoutside = ($ue->timestart && $now < $ue->timestart);
                } else if ($ue->timeend) {
                    $period = get_string('periodend', 'storeinfo', userdate($ue->timeend));
                    $periodoutside = ($ue->timeend && $now > $ue->timeend);
                } else {
                    // If there is no start or end show when user was storeinfoled.
                    $period = get_string('periodnone', 'storeinfo', userdate($ue->timecreated));
                    $periodoutside = false;
                }
                $details['storemultiinfos'][$ue->id] = array(
                    'text' => $ue->storemultiinfoinstancename,
                    'period' => $period,
                    'dimmed' =>  ($periodoutside or $ue->status != storeinfo_USER_ACTIVE or $ue->storemultiinfoinstance->status != storeinfo_INSTANCE_ENABLED),
    ...
    public function get_manual_storeinfo_buttons() {
        $plugins = $this->get_storemultiinfo_plugins(true); // Skip disabled plugins.
        $buttons = array();
        foreach ($plugins as $plugin) {
            $newbutton = $plugin->get_manual_storeinfo_button($this);
            if (is_array($newbutton)) {
                $buttons += $newbutton;
            } else if ($newbutton instanceof storeinfo_user_button) {
                $buttons[] = $newbutton;
            }
        }
        return $buttons;
...
//============================================================================
    ...
    public function get_filtered_storemultiinfo_plugin() {
        ...
        return $plugins[$instance->storeinfo];
    }
    ...
    public function get_users_storemultiinfos(array $userids) {
        ...
        $sql = "SELECT ue.id AS ueid, ue.status, ue.storeinfoid, ue.userid, ue.timestart, ue.timeend, ue.modifierid, ue.timecreated, ue.timemodified, $userfields
                  FROM {user_storemultiinfos} ue
             LEFT JOIN {user} u ON u.id = ue.userid
                 WHERE ue.storeinfoid $instancesql AND
                       u.id $idsql
              ORDER BY $sort";
...
/**
 * A button that is used to storeinfo users in a course
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
//============================================================================
//============================================================================
class storeinfo_user_button extends single_button {
    ...
    /**
     * Initialises the new storeinfo_user_button
     *
     * @staticvar int $count The number of storeinfo user buttons already created
    ...
    public function __construct(moodle_url $url, $label, $method = 'post') {
        ...
        $this->class = 'singlebutton storeinfousersbutton';
        $this->formid = 'storeinfousersbutton-'.$count;
    }
...
class storeinfo_ajax_exception extends moodle_exception {
    /**
     * Constructor
    ...
    public function __construct($errorcode, $link = '', $a = NULL, $debuginfo = null) {
        parent::__construct($errorcode, 'storeinfo', $link, $a, $debuginfo);
    }
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class storeinfo_bulk_storemultiinfo_operation {
     ...
     */
    public function __construct(course_storemultiinfo_manager $manager, storeinfo_plugin $plugin = null) {
     ...
     * @return storeinfo_bulk_storemultiinfo_change_form|moodleform|false
     */
