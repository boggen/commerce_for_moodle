...
/**
 * This file processes AJAX storemultiinfo actions and returns JSON
 *
...
require('../config.php');
require_once("$CFG->dirroot/storeinfo/locallib.php");
require_once("$CFG->dirroot/storeinfo/renderer.php");
...
$PAGE->set_url(new moodle_url('/storeinfo/ajax.php', array('id'=>$id, 'action'=>$action)));
...
require_capability('moodle/course:storeinforeview', $context);
require_sesskey();

echo $OUTPUT->header(); // send headers

$manager = new course_storemultiinfo_manager($PAGE, $course);
...
switch ($action) {
    case 'unstoreinfo':
//        $ue = $DB->get_record('user_storemultiinfos', array('id'=>required_param('ue', PARAM_INT)), '*', MUST_EXIST);
//        list ($instance, $plugin) = $manager->get_user_storemultiinfo_components($ue);
        if (!$instance || !$plugin || !storeinfo_is_enabled($instance->storeinfo) || !$plugin->allow_unstoreinfo_user($instance, $ue) || !has_capability("storeinfo/$instance->storeinfo:unstoreinfo", $manager->get_context()) || !$manager->unstoreinfo_user($ue)) {
            throw new storeinfo_ajax_exception('unstoreinfonotpermitted');
        }
        break;
    case 'unassign':
        ...
        if (!has_capability('moodle/role:assign', $manager->get_context()) || !$manager->unassign_role_from_user($user, $role)) {
            throw new storeinfo_ajax_exception('unassignnotpermitted');
        }
        break;
    case 'assign':
        ...
        if (!array_key_exists($roleid, $manager->get_assignable_roles())) {
            throw new storeinfo_ajax_exception('invalidrole');
        }
        if (!has_capability('moodle/role:assign', $manager->get_context()) || !$manager->assign_role_to_user($roleid, $user->id)) {
            throw new storeinfo_ajax_exception('assignnotpermitted');
        }
    ...
    case 'searchotherusers':
        ...
        $useroptions = array();
        // User is not storeinfoled, either link to site profile or do not link at all.
        ...
        break;
    default:
        throw new storeinfo_ajax_exception('unknowajaxaction');
}
