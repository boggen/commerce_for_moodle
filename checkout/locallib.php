...
/**
 * This file contains the course_checkingout_manager class which is used to interface
 * with the functions that exist in checkoutlib.php in relation to a single course.
 *
...
/**
 * This class provides a targeted tied together means of interfacing the checkingout
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_checkingout_manager {
    ...
    protected $context;
    /**
     * The course we are managing checkingouts for
     * @var stdClass
     */
    protected $course = null;
    /**
     * Limits the focus of the manager to one checkingout plugin instance
     * @var string
     */
    ...
    /**
     * The total number of users checkoutled in the course
     * Populated by course_checkingout_manager::get_total_users
     * @var int
     */
    protected $totalusers = null;
    /**
     * An array of users currently checkoutled in the course
     * Populated by course_checkingout_manager::get_users
     * @var array
     */
     ...
     * been checkoutled in the course
     * @var array
     */
    ...
    /**
     * Constructs the course checkingout manager
     *
     ...
     * @param int $statusfilter if not -1, filter users with active/inactive checkingout.
     */
    ...
    /**
     * Returns the total number of checkoutled users in the course.
     *
     * If a filter was specificed this will be the total number of users checkoutled
    ...
    public function get_total_users() {
        ...
        if ($this->totalusers === null) {
            ...
            $sqltotal = "SELECT COUNT(DISTINCT u.id)
                           FROM {user} u
                           JOIN {user_checkingouts} ue ON (ue.userid = u.id  AND ue.checkoutid $instancessql)
        ...
        if ($this->totalotherusers === null) {
            ...
            $sql = "SELECT COUNT(DISTINCT u.id)
                 ...
                 LEFT JOIN (
                        ...
                        LEFT JOIN {checkout} e ON e.id=ue.checkoutid
    ...
    /**
     * Gets all of the users checkoutled in this course.
     *
     * If a filter was specified this will be the users who were checkoutled
    ...
    protected function get_filter_sql() {
        ...
        // Status condition.
        if ($this->statusfilter === checkout_USER_ACTIVE) {
            $sql .= " AND ue.status = :active AND e.status = :enabled AND ue.timestart < :now1
                    AND (ue.timeend = 0 OR ue.timeend > :now2)";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('enabled' => checkout_INSTANCE_ENABLED,
                             'active' => checkout_USER_ACTIVE,
                             'now1' => $now,
                             'now2' => $now);
        } else if ($this->statusfilter === checkout_USER_SUSPENDED) {
            $sql .= " AND (ue.status = :inactive OR e.status = :disabled OR ue.timestart > :now1
                    OR (ue.timeend <> 0 AND ue.timeend < :now2))";
            $now = round(time(), -2); // rounding helps caching in DB
            $params += array('disabled' => checkout_INSTANCE_DISABLED,
                             'inactive' => checkout_USER_SUSPENDED,
                             'now1' => $now,
                             'now2' => $now);
        }
     ...
     * within this course but who have not been checkoutled in the course
     *
     * @global moodle_database $DB
     * @param string $sort
     ...
     * @param int $addedcheckingout number of users added to checkingout.
     * @return array with two elememts:
     *      int total number of users matching the search.
     *      array of user objects returned by the query.
     */

    protected function execute_search_queries($search, $fields, $countfields, $sql, array $params, $page, $perpage, $addedcheckingout=0) {
        global $DB, $CFG;
        ...
        $availableusers = $DB->get_records_sql($fields . $sql . $order,
                array_merge($params, $sortparams), ($page*$perpage) - $addedcheckingout, $perpage);

        return array('totalusers' => $totalusers, 'users' => $availableusers);
    }

    /**
     * Gets an array of the users that can be checkoutled in this course.
     *
     * @global moodle_database $DB
     ...
     * @param int $perpage Defaults to 25
     * @param int $addedcheckingout Defaults to 0
     * @return array Array(totalusers => int, users => array)
     */

    public function get_potential_users($checkoutid, $search='', $searchanywhere=false, $page=0, $perpage=25, $addedcheckingout=0) {
        global $DB;
        ...
        $sql = " FROM {user} u
            LEFT JOIN {user_checkingouts} ue ON (ue.userid = u.id AND ue.checkoutid = :checkoutid)
                WHERE $wherecondition
                      AND ue.id IS NULL";
        $params['checkoutid'] = $checkoutid;

        return $this->execute_search_queries($search, $fields, $countfields, $sql, $params, $page, $perpage, $addedcheckingout);
    }
...
// ====================================================================================================
    protected function get_instance_sql() {
        ...
        if ($this->_instancessql === null) {
            $instances = $this->get_checkingout_instances();
            $filter = $this->get_checkingout_filter();
            if ($filter && array_key_exists($filter, $instances)) {
                ...
                $filter = 0;
                if ($instances) {
                    list($sql, $params) = $DB->get_in_or_equal(array_keys($this->get_checkingout_instances()), SQL_PARAMS_NAMED);
                } else {
    ...
    /**
     * Returns all of the checkingout instances for this course.
     *
    ...
    public function get_checkingout_instances() {
        if ($this->_instances === null) {
            $this->_instances = checkout_get_instances($this->course->id, false);
        }
        return $this->_instances;
    }

    /**
     * Returns the names for all of the checkingout instances for this course.
     *
     * NOTE: since 2.4 it includes instances of disabled plugins too.
     *
     * @return array
    ...
    public function get_checkingout_instance_names() {
        if ($this->_inames === null) {
            $instances = $this->get_checkingout_instances();
            $plugins = $this->get_checkingout_plugins(false);
            foreach ($instances as $key=>$instance) {
                ...
                $this->_inames[$key] = $plugins[$instance->checkout]->get_instance_name($instance);
            }
        }
        return $this->_inames;
    }
    ...
    /**
     * Gets all of the checkingout plugins that are active for this course.
     *
     * @param bool $onlyenabled return only enabled checkout plugins
     * @return array
     */

    public function get_checkingout_plugins($onlyenabled = true) {
        if ($this->_plugins === null) {
            $this->_plugins = checkout_get_plugins(true);
        }
        ...
        if ($this->_allplugins === null) {
            ...
            foreach (checkout_get_plugins(false) as $name=>$plugin) {
    ...
    /**
     * Uncheckouts a user from the course given the users ue entry
     *
     * @global moodle_database $DB
     * @param stdClass $ue
     * @return bool
    ...
    public function uncheckout_user($ue) {
        global $DB;
        list ($instance, $plugin) = $this->get_user_checkingout_components($ue);
        if ($instance && $plugin && $plugin->allow_uncheckout_user($instance, $ue) && has_capability("checkout/$instance->checkout:uncheckout", $this->context)) {
            $plugin->uncheckout_user($instance, $ue->userid);
            return true;
        }
        return false;
    }
    ...
    /**
     * Given a user checkingout record this method returns the plugin and checkingout
     * instance that relate to it.
     *
     * @param stdClass|int $usercheckingout
     * @return array array($instance, $plugin)
     */

    public function get_user_checkingout_components($usercheckingout) {
        global $DB;
        if (is_numeric($usercheckingout)) {
            $usercheckingout = $DB->get_record('user_checkingouts', array('id'=>(int)$usercheckingout));
        }
        $instances = $this->get_checkingout_instances();
        $plugins = $this->get_checkingout_plugins(false);
        if (!$usercheckingout || !isset($instances[$usercheckingout->checkoutid])) {
            return array(false, false);
        }
        $instance = $instances[$usercheckingout->checkoutid];
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        foreach ($ras as $ra) {
            if ($ra->component) {
                if (strpos($ra->component, 'checkout_') !== 0) {
                    continue;
                }
                if (!$plugin = checkout_get_plugin(substr($ra->component, 6))) {
                    continue;
                }
    ...
    /**
     * Edits an checkingout
     *
     * @param stdClass $usercheckingout
     * @param stdClass $data
     * @return bool
     */
//============================================================================
...
//============================================================================
//============================================================================
    public function edit_checkingout($usercheckingout, $data) {
        //Only allow editing if the user has the appropriate capability
        //Already checked in /checkout/users.php but checking again in case this function is called from elsewhere
        list($instance, $plugin) = $this->get_user_checkingout_components($usercheckingout);
        if ($instance && $plugin && $plugin->allow_manage($instance) && has_capability("checkout/$instance->checkout:manage", $this->context)) {
            if (!isset($data->status)) {
                $data->status = $usercheckingout->status;
            }
            $plugin->update_user_checkout($instance, $usercheckingout->userid, $data->status, $data->timestart, $data->timeend);
            return true;
        }
        return false;
    }
    ...
    /**
     * Returns the current checkingout filter that is being applied by this class
     * @return string
     */
    public function get_checkingout_filter() {
    ...
    /**
     * Gets the checkingouts this user has in the course - including all suspended plugins and instances.
     *
     * @global moodle_database $DB
     * @param int $userid
     * @return array
    ...
    public function get_user_checkingouts($userid) {
        global $DB;
        list($instancessql, $params, $filter) = $this->get_instance_sql();
        $params['userid'] = $userid;
        $usercheckingouts = $DB->get_records_select('user_checkingouts', "checkoutid $instancessql AND userid = :userid", $params);
        $instances = $this->get_checkingout_instances();
        $plugins = $this->get_checkingout_plugins(false);
        $inames = $this->get_checkingout_instance_names();
        foreach ($usercheckingouts as &$ue) {
            $ue->checkingoutinstance     = $instances[$ue->checkoutid];
            $ue->checkingoutplugin       = $plugins[$ue->checkingoutinstance->checkout];
            $ue->checkingoutinstancename = $inames[$ue->checkingoutinstance->id];
        }
        return $usercheckingouts;
    }
...
//============================================================================
    ...
    /**
     * Returns the course this object is managing checkingouts for
     *
     * @return stdClass
     */
...
//============================================================================
     ...
     * course but have not been checkoutled.
     *
     * @param core_checkout_renderer $renderer
    ...
    public function get_other_users_for_display(core_checkout_renderer $renderer, moodle_url $pageurl, $sort, $direction, $page, $perpage) {
        ...
        foreach ($userroles as $userrole) {
            ...
            if ($contextid == $this->context->id) {
                ...
                if ($userrole->component) {
                    $changeable = false;
                    if (strpos($userrole->component, 'checkout_') === 0) {
                ...
                $roletext = get_string('rolefromthiscourse', 'checkout', $a);
            } else {
                $changeable = false;
                switch ($userrole->contextlevel) {
                    case CONTEXT_COURSE :
                        // Meta course
                        $roletext = get_string('rolefrommetacourse', 'checkout', $a);
                        break;
                    case CONTEXT_COURSECAT :
                        $roletext = get_string('rolefromcategory', 'checkout', $a);
                        break;
                    case CONTEXT_SYSTEM:
                    default:
                        $roletext = get_string('rolefromsystem', 'checkout', $a);
                        break;
                }
            }
     ...
     * as well as minimal information on the users roles, groups, and checkingouts.
     *
     * @param core_checkout_renderer $renderer
     * @param moodle_url $pageurl
     * @param int $sort
    ...
    public function get_users_for_display(course_checkingout_manager $manager, $sort, $direction, $page, $perpage) {
        $pageurl = $manager->get_moodlepage()->url;
        $users = $this->get_users($sort, $direction, $page, $perpage);

        $now = time();
        $straddgroup = get_string('addgroup', 'group');
        $struncheckout = get_string('uncheckout', 'checkout');
        ...
        $enabledplugins = $this->get_checkingout_plugins(true);
        ...
        foreach ($users as $user) {
            ...
            // checkingouts
            $details['checkingouts'] = array();
            foreach ($this->get_user_checkingouts($user->id) as $ue) {
                if (!isset($enabledplugins[$ue->checkingoutinstance->checkout])) {
                    $details['checkingouts'][$ue->id] = array(
                        'text' => $ue->checkingoutinstancename,
                        'period' => null,
                        'dimmed' =>  true,
                        'actions' => array()
                    );
                    ...
                    $period = get_string('periodstartend', 'checkout', array('start'=>userdate($ue->timestart), 'end'=>userdate($ue->timeend)));
                    $periodoutside = ($ue->timestart && $ue->timeend && ($now < $ue->timestart || $now > $ue->timeend));
                } else if ($ue->timestart) {
                    $period = get_string('periodstart', 'checkout', userdate($ue->timestart));
                    $periodoutside = ($ue->timestart && $now < $ue->timestart);
                } else if ($ue->timeend) {
                    $period = get_string('periodend', 'checkout', userdate($ue->timeend));
                    $periodoutside = ($ue->timeend && $now > $ue->timeend);
                } else {
                    // If there is no start or end show when user was checkoutled.
                    $period = get_string('periodnone', 'checkout', userdate($ue->timecreated));
                    $periodoutside = false;
                }
                $details['checkingouts'][$ue->id] = array(
                    'text' => $ue->checkingoutinstancename,
                    'period' => $period,
                    'dimmed' =>  ($periodoutside or $ue->status != checkout_USER_ACTIVE or $ue->checkingoutinstance->status != checkout_INSTANCE_ENABLED),
                    'actions' => $ue->checkingoutplugin->get_user_checkingout_actions($manager, $ue)
                );
            }
    ...
    public function get_manual_checkout_buttons() {
        $plugins = $this->get_checkingout_plugins(true); // Skip disabled plugins.
        $buttons = array();
        foreach ($plugins as $plugin) {
            $newbutton = $plugin->get_manual_checkout_button($this);
            if (is_array($newbutton)) {
                $buttons += $newbutton;
            } else if ($newbutton instanceof checkout_user_button) {
                $buttons[] = $newbutton;
            }
        }
        return $buttons;
    ...
    public function has_instance($checkoutpluginname) {
        // Make sure manual checkingouts instance exists
        foreach ($this->get_checkingout_instances() as $instance) {
            if ($instance->checkout == $checkoutpluginname) {
                return true;
            }
        }
        return false;
...
//============================================================================
    /**
     * Returns the checkingout plugin that the course manager was being filtered to.
     *
     * If no filter was being applied then this function returns false.
     *
     * @return checkout_plugin
     */
    public function get_filtered_checkingout_plugin() {
        $instances = $this->get_checkingout_instances();
        $plugins = $this->get_checkingout_plugins(false);
        ...
        return $plugins[$instance->checkout];
    }

    /**
     * Returns and array of users + checkingout details.
     *
     * Given an array of user id's this function returns and array of user checkingouts for those users
     * as well as enough user information to display the users name and picture for each checkingout.
     *
     * @global moodle_database $DB
     * @param array $userids
     * @return array
    ...
    public function get_users_checkingouts(array $userids) {
        global $DB;
        ...
        $sql = "SELECT ue.id AS ueid, ue.status, ue.checkoutid, ue.userid, ue.timestart, ue.timeend, ue.modifierid, ue.timecreated, ue.timemodified, $userfields
                  FROM {user_checkingouts} ue
             LEFT JOIN {user} u ON u.id = ue.userid
                 WHERE ue.checkoutid $instancesql AND
                       u.id $idsql
              ORDER BY $sort";
        ...
        foreach ($rs as $ue) {
            ...
            if (!array_key_exists($user->id, $users)) {
                $user->checkingouts = array();
                $users[$user->id] = $user;
            }
            $ue->checkingoutinstance = $instances[$ue->checkoutid];
            $ue->checkingoutplugin = $plugins[$ue->checkingoutinstance->checkout];
            $users[$user->id]->checkingouts[$ue->id] = $ue;
        }
        $rs->close();
        return $users;
    }
...
/**
 * A button that is used to checkout users in a course
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
//============================================================================
//============================================================================
class checkout_user_button extends single_button {
    ...
    /**
     * Initialises the new checkout_user_button
     *
     * @staticvar int $count The number of checkout user buttons already created
    ...
    public function __construct(moodle_url $url, $label, $method = 'post') {
        ...
        $this->class = 'singlebutton checkoutusersbutton';
        $this->formid = 'checkoutusersbutton-'.$count;
    }
...
/**
 * User checkingout action
 *
 * This class is used to manage a renderable ue action such as editing an user checkingout or deleting
 * a user checkingout.
 *
 * @copyright  2011 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_checkingout_action implements renderable {
...
class checkout_ajax_exception extends moodle_exception {
    /**
     * Constructor
    ...
    public function __construct($errorcode, $link = '', $a = NULL, $debuginfo = null) {
        parent::__construct($errorcode, 'checkout', $link, $a, $debuginfo);
    }
}

/**
 * This class is used to manage a bulk operations for checkingout plugins.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class checkout_bulk_checkingout_operation {

    /**
     * The course checkingout manager
     * @var course_checkingout_manager
     */
    protected $manager;

    /**
     * The checkingout plugin to which this operation belongs
     * @var checkout_plugin
     */
    protected $plugin;
     ...
     */
    public function __construct(course_checkingout_manager $manager, checkout_plugin $plugin = null) {
     ...
     * @return checkout_bulk_checkingout_change_form|moodleform|false
     */
     ...
     */
    abstract public function process(course_checkingout_manager $manager, array $users, stdClass $properties);
}
