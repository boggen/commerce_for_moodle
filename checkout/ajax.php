...
/**
 * This file processes AJAX checkingout actions and returns JSON
 *
...
require('../config.php');
require_once("$CFG->dirroot/checkout/locallib.php");
require_once("$CFG->dirroot/checkout/renderer.php");
...
$PAGE->set_url(new moodle_url('/checkout/ajax.php', array('id'=>$id, 'action'=>$action)));
...
require_capability('moodle/course:checkoutreview', $context);
require_sesskey();

echo $OUTPUT->header(); // send headers

$manager = new course_checkingout_manager($PAGE, $course);
...
switch ($action) {
    case 'uncheckout':
        $ue = $DB->get_record('user_checkingouts', array('id'=>required_param('ue', PARAM_INT)), '*', MUST_EXIST);
        list ($instance, $plugin) = $manager->get_user_checkingout_components($ue);
        if (!$instance || !$plugin || !checkout_is_enabled($instance->checkout) || !$plugin->allow_uncheckout_user($instance, $ue) || !has_capability("checkout/$instance->checkout:uncheckout", $manager->get_context()) || !$manager->uncheckout_user($ue)) {
            throw new checkout_ajax_exception('uncheckoutnotpermitted');
        }
        break;
    case 'unassign':
        ...
        if (!has_capability('moodle/role:assign', $manager->get_context()) || !$manager->unassign_role_from_user($user, $role)) {
            throw new checkout_ajax_exception('unassignnotpermitted');
        }
        break;
    case 'assign':
    ...
    case 'searchotherusers':
        ...
        $useroptions = array();
        // User is not checkoutled, either link to site profile or do not link at all.
        ...
        break;
    default:
        throw new checkout_ajax_exception('unknowajaxaction');
}
