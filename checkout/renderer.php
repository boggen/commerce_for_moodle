...
/**
 * This is the main renderer for the checkout section.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_checkout_renderer extends plugin_renderer_base {

    /**
     * Renders a course checkingout table
     *
     * @param course_checkingout_table $table
     * @param moodleform $mform Form that contains filter controls
     * @return string
     */
    public function render_course_checkingout_users_table(course_checkingout_users_table $table,
            moodleform $mform) {

        $table->initialise_javascript();

        $buttons = $table->get_manual_checkout_buttons();
        $buttonhtml = '';
        if (count($buttons) > 0) {
            $buttonhtml .= html_writer::start_tag('div', array('class' => 'checkout_user_buttons'));
        ...
        if ($table->has_bulk_user_checkingout_operations()) {
            $content .= html_writer::start_tag('form', array('action' => new moodle_url('/checkout/bulkchange.php'), 'method' => 'post'));
            ...
            $content .= html_writer::tag('option', get_string('withselectedusers', 'checkout'), array('value' => ''));
            $options = array('' => get_string('withselectedusers', 'checkout'));
            foreach ($table->get_bulk_user_checkingout_operations() as $operation) {
    ...
    /**
     * Renderers the checkout_user_button.
     *
     * @param checkout_user_button $button
     * @return string XHTML
     */
    protected function render_checkout_user_button(checkout_user_button $button) {
     ...
     * @return string
     */
    protected function render_course_checkingout_other_users_table(course_checkingout_other_users_table $table) {
        ...
        $content .= html_writer::tag('div', get_string('otheruserdesc', 'checkout'), array('class'=>'otherusersdesc'));
        $content .= $this->output->render($table->get_paging_bar());
        $content .= html_writer::table($table);
        $content .= $this->output->render($table->get_paging_bar());
        $searchbutton = $table->get_user_search_button();
    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        $iconcheckoutremove = $this->output->pix_url('t/delete');
        ...
        foreach ($roles as $roleid=>$role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $icon = html_writer::empty_tag('img', array('alt'=>$strunassign, 'src'=>$iconcheckoutremove));
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach($groups as $groupid=>$name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                $icon = html_writer::empty_tag('img', array('alt'=>get_string('removefromgroup', 'group', $name), 'src'=>$iconcheckoutremove));
    ...
    /**
     * Generates the HTML for the given checkingouts + available actions
     *
     * @param int $userid
     * @param array $checkingouts
     * @param moodle_url $pageurl
     * @return string
     */
    public function user_checkingouts_and_actions($checkingouts) {
        $output = '';
        foreach ($checkingouts as $ue) {
            $checkingoutoutput = $ue['text'].' '.$ue['period'];
            if ($ue['dimmed']) {
                $checkingoutoutput = html_writer::tag('span', $checkingoutoutput, array('class'=>'dimmed_text'));
            } else {
                $checkingoutoutput = html_writer::tag('span', $checkingoutoutput);
            }
            foreach ($ue['actions'] as $action) {
                $checkingoutoutput .= $this->render($action);
            }
            $output .= html_writer::tag('div', $checkingoutoutput, array('class'=>'checkingout'));
        }
        return $output;
    }

    /**
     * Renders a user checkingout action
     * @param user_checkingout_action $icon
     * @return string
     */
    protected function render_user_checkingout_action(user_checkingout_action $icon) {
        return html_writer::link($icon->get_url(), $this->output->render($icon->get_icon()), $icon->get_attributes());
    }
}

/**
 * Main course checkingout table
 *
 * This table is used to display the checkingout information for a course.
 * It requires that a course checkingout manager be provided during constuct with
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_checkingout_table extends html_table implements renderable {
    ...
    /**
     * The default sort, options are course_checkingout_table::$sortablefields
     * Default: lastname
     */
    ...
    /**
     * The sort field for this table, should be one of course_checkingout_table::$sortablefields
     * @var string
     */
    public $sort;
    ...
    /**
     * The users checkoutled in this course
     * @var array
     */
    ...
    /**
     * An array of bulk user checkingout operations
     * @var array
     */
     ...
     */
    public function __construct(course_checkingout_manager $manager) {
        ...
        $this->attributes = array('class'=>'usercheckingout');
        ...
        $plugin = $manager->get_filtered_checkingout_plugin();
        if ($plugin and checkout_is_enabled($plugin->get_name())) {
            $this->bulkoperations = $plugin->get_bulk_operations($manager);
        }
    }

    /**
     * Returns an array of checkout_user_buttons that are created by the different
     * checkingout plugins available.
     *
     * @return array
     */
    public function get_manual_checkout_buttons() {
        return $this->manager->get_manual_checkout_buttons();
    }
    ...
    public function initialise_javascript() {
        if (has_capability('moodle/role:assign', $this->manager->get_context())) {
            ...
            $modules = array('moodle-checkout-rolemanager', 'moodle-checkout-rolemanager-skin');
            $function = 'M.checkout.rolemanager.init';
            $arguments = array(
                'containerId'=>$this->id,
     ...
     */
    public function set_bulk_user_checkingout_operations(array $bulkoperations) {
     ...
     * @return array
     */
    public function get_bulk_user_checkingout_operations() {
        return $this->bulkoperations;
    }

    /**
     * Returns true fi the table is aware of any bulk operations that can be performed on users
     * selected from the currently filtered checkingout plugins.
     *
     * @return bool
     */
    public function has_bulk_user_checkingout_operations() {
        return !empty($this->bulkoperations);
    }
}

/**
 * Table control used for checkoutled users
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_checkingout_users_table extends course_checkingout_table {
 ...
 *
 * Other users are users who have roles but are not checkoutled.
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_checkingout_other_users_table extends course_checkingout_table {
    ...
    public function __construct(course_checkingout_manager $manager) {
        ...
        $this->attributes = array('class'=>'usercheckingout otherusercheckingout');
    }
    ...
    public function get_user_search_button() {
        ...
        if ($count == 1) {
            $this->manager->get_moodlepage()->requires->strings_for_js(array(
                    ...
                    'startdatetoday',
                    'durationdays',
                    'checkoutperiod'), 'checkout');
            $this->manager->get_moodlepage()->requires->string_for_js('assignrole', 'role');

            $modules = array('moodle-checkout-otherusersmanager', 'moodle-checkout-otherusersmanager-skin');
            $function = 'M.checkout.otherusersmanager.init';
            $arguments = array(
                'courseId'=> $this->manager->get_course()->id,
                'ajaxUrl' => '/checkout/ajax.php',
