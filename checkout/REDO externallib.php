 ...
 *
 * This api is mostly read only, the actual checkout and uncheckout
 * support is in each checkout plugin.
 *
 ...
 * @since Moodle 2.2
 */
class core_checkout_external extends external_api {
     ...
     * @since Moodle 2.4
     */
    public static function get_checkoutled_users_with_capability_parameters() {
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) only users with active checkingouts. Requires \'moodle/course:checkoutreview\' .
     ...
     * a list of the users that are checkoutled in the course and have that capability are returned.
     *
     ...
     * @since  Moodle 2.4
     */
    public static function get_checkoutled_users_with_capability($coursecapabilities, $options) {
        global $CFG, $DB;
        ...
        $params = self::validate_parameters(self::get_checkoutled_users_with_capability_parameters(),
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            // The course:checkoutereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('moodle/course:checkoutreview', $coursecontext);
            }
            ...
            foreach ($coursecapability['capabilities'] as $capability) {
                ...
                list($checkoutledsql, $checkoutledparams) = get_checkoutled_sql($coursecontext, $capability, $groupid, $onlyactive);

                $sql = "SELECT u.* FROM {user} u WHERE u.id IN ($checkoutledsql) ORDER BY u.id ASC";

                $checkoutledusers = $DB->get_recordset_sql($sql, $checkoutledparams, $limitfrom, $limitnumber);
                $users = array();
                foreach ($checkoutledusers as $courseuser) {
                    if ($userdetails = user_get_user_details($courseuser, $course, $userfields)) {
                        $users[] = $userdetails;
                    }
                }
                $checkoutledusers->close();
     ...
     * @since Moodle 2.4
     */
    public static function get_checkoutled_users_with_capability_returns() {
        return  new external_multiple_structure( new external_single_structure (
                ...
                array(
                    ...
                    'checkoutledcourses' => new external_multiple_structure(
                        new external_single_structure(
                            array(
                    ...
                    ), 'Courses where the user is checkoutled - limited by which courses the user is able to see', VALUE_OPTIONAL)
                )
                        ), 'List of users that are checkoutled in the course and have the specified capability'),
                    )
                )
            );
    }
    ...
    /**
     * Get list of courses user is checkoutled in (only active checkingouts are returned).
    ...
    public static function get_users_courses($userid) {
        ...
        $courses = checkout_get_users_courses($params['userid'], true, 'id, shortname, fullname, idnumber, visible');
        $result = array();
        ...
        foreach ($courses as $course) {
            ...
            try {
                ...
                // current user can not access this course, sorry we can not disclose who is checkoutled in this course!
                continue;
            }
            ...
            list($checkoutledsqlselect, $checkoutledparams) = get_checkoutled_sql($context);
            $checkoutledsql = "SELECT COUNT('x') FROM ($checkoutledsqlselect) checkoutleduserids";
            $checkoutledusercount = $DB->count_records_sql($checkoutledsql, $checkoutledparams);

            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'checkoutledusercount'=>$checkoutledusercount);
        }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'checkoutledusercount' => new external_value(PARAM_INT, 'Number of checkoutled users in this course'),
     ...
     * @return external_function_parameters
     */
    public static function get_checkoutled_users_parameters() {
        return new external_function_parameters(
            array(
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * onlyactive (integer) return only users with active checkingouts and matching time restrictions. This option requires \'moodle/course:checkoutreview\' on the course context.
                            * userfields (\'string, string, ...\') return only the values of these user fields.
                            * limitfrom (integer) sql limit from.
     ...
     */
    public static function get_checkoutled_users($courseid, $options = array()) {
        global $CFG, $USER, $DB;
        require_once($CFG->dirroot . "/user/lib.php");

        $params = self::validate_parameters(
            self::get_checkoutled_users_parameters(),
            array(
                'courseid'=>$courseid,
                'options'=>$options
            )
        ...
        // to overwrite this option, you need course:checkoutereview permission
        if ($onlyactive) {
            require_capability('moodle/course:checkoutreview', $coursecontext);
        }

        list($checkoutledsql, $checkoutledparams) = get_checkoutled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        $ctxselect = ', ' . context_helper::get_preload_record_columns_sql('ctx');
        $ctxjoin = "LEFT JOIN {context} ctx ON (ctx.instanceid = u.id AND ctx.contextlevel = :contextlevel)";
        $checkoutledparams['contextlevel'] = CONTEXT_USER;
        $sql = "SELECT u.* $ctxselect
                  FROM {user} u $ctxjoin
                 WHERE u.id IN ($checkoutledsql)
                 ORDER BY u.id ASC";
        $checkoutledusers = $DB->get_recordset_sql($sql, $checkoutledparams, $limitfrom, $limitnumber);
        $users = array();
        foreach ($checkoutledusers as $user) {
     ...
     * @return external_description
     */
    public static function get_checkoutled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
    ...
    /**
     * Returns description of get_course_checkingout_methods() parameters
     *
     * @return external_function_parameters
     */
    public static function get_course_checkingout_methods_parameters() {
        return new external_function_parameters(
            array(
                'courseid' => new external_value(PARAM_INT, 'Course id')
            )
    ...
    /**
     * Get list of active course checkingout methods for current user.
     *
     * @param int $courseid
     * @return array of course checkingout methods
     */
    public static function get_course_checkingout_methods($courseid) {

        $params = self::validate_parameters(self::get_course_checkingout_methods_parameters(), array('courseid' => $courseid));
        ...
        $result = array();
        $checkoutinstances = checkout_get_instances($params['courseid'], true);
        foreach ($checkoutinstances as $checkoutinstance) {
            if ($checkoutplugin = checkout_get_plugin($checkoutinstance->checkout)) {
                if ($instanceinfo = $checkoutplugin->get_checkout_info($checkoutinstance)) {
    ...
    /**
     * Returns description of get_course_checkingout_methods() result value
     *
     * @return external_description
     */
    public static function get_course_checkingout_methods_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'id of course checkingout instance'),
                    'courseid' => new external_value(PARAM_INT, 'id of course'),
                    'type' => new external_value(PARAM_PLUGIN, 'type of checkingout plugin'),
                    'name' => new external_value(PARAM_RAW, 'name of checkingout plugin'),
                    'status' => new external_value(PARAM_RAW, 'status of checkingout plugin'),
...
class core_role_external extends external_api {
    ...
    public static function assign_roles($assignments) {
        ...
        foreach ($params['assignments'] as $assignment) {
            ...
            // Ensure the current user is allowed to run this function in the checkingout context.
            self::validate_context($context);
...
/**
 * Deprecated checkout and role external functions
 *
 * @package    core_checkout
 * @copyright  2010 Jerome Mouneyrac
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 ...
 * @see core_checkout_external
 * @see core_role_external
 */
class moodle_checkout_external extends external_api {
     ...
     * @see core_checkout_external::get_checkoutled_users_parameters()
     */
    public static function get_checkoutled_users_parameters() {
        return new external_function_parameters(
            array(
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_checkout_external::get_checkoutled_users()
     */
    public static function get_checkoutled_users($courseid, $withcapability = null, $groupid = null, $onlyactive = false) {
        global $DB, $CFG, $USER;

        // Do basic automatic PARAM checks on incoming data, using params description
        // If any problems are found then exceptions are thrown with helpful error messages
        $params = self::validate_parameters(self::get_checkoutled_users_parameters(), array(
            'courseid'=>$courseid,
        ...
        list($sqlparams, $params) =  get_checkoutled_sql($coursecontext, $withcapability, $groupid, $onlyactive);
        ...
        $checkoutledusers = $DB->get_records_sql($sql, $params);
        $result = array();
        $isadmin = is_siteadmin($USER);
        $canviewfullnames = has_capability('moodle/site:viewfullnames', $context);
        foreach ($checkoutledusers as $checkoutleduser) {
            $profilimgurl = moodle_url::make_pluginfile_url($checkoutleduser->usercontextid, 'user', 'icon', NULL, '/', 'f1');
            $profilimgurlsmall = moodle_url::make_pluginfile_url($checkoutleduser->usercontextid, 'user', 'icon', NULL, '/', 'f2');
            $resultuser = array(
                'courseid' => $checkoutleduser->courseid,
                'userid' => $checkoutleduser->userid,
                'fullname' => fullname($checkoutleduser),
                'profileimgurl' => $profilimgurl->out(false),
                'profileimgurlsmall' => $profilimgurlsmall->out(false)
            );
            // check if we can return username
            if ($isadmin) {
                $resultuser['username'] = $checkoutleduser->username;
            }
            // check if we can return first and last name
            if ($isadmin or $canviewfullnames) {
                $resultuser['firstname'] = $checkoutleduser->firstname;
                $resultuser['lastname'] = $checkoutleduser->lastname;
            }
            $result[] = $resultuser;
        }
     ...
     * @since Moodle 2.0
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_checkout_external::get_checkoutled_users_returns()
     */
    public static function get_checkoutled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_checkout_external::get_users_courses_parameters()
     */
    public static function get_users_courses_parameters() {
        return core_checkout_external::get_users_courses_parameters();
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see use core_checkout_external::get_users_courses()
     */
    public static function get_users_courses($userid) {
        return core_checkout_external::get_users_courses($userid);
    }
     ...
     * @since Moodle 2.1
     * @deprecated Moodle 2.2 MDL-29106 - Please do not call this function any more.
     * @see core_checkout_external::get_users_courses_returns()
     */
    public static function get_users_courses_returns() {
        return core_checkout_external::get_users_courses_returns();
    }
