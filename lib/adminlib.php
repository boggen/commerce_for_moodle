...
/**
 * this is a copy of "class admin_settings_manageenrols extends admin_settings"
 * adjusted for checkingouts
 * 
 *@copyright 2014 Ryan sanders
 *@license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
*/
/**
 * Special class for enrol plugins management.
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_setting_managecheckouts extends admin_setting {
    /**
     * Calls parent::__construct with specific arguments
     */
    public function __construct() {
        $this->nosave = true;
        parent::__construct('checkoutsui', get_string('managecheckouts', 'checkout'), '', '');
    }
    ...
    /**
     * Checks if $query is one of the available checkout plugins
		*
     * @param string $query The string to search for
     * @return bool Returns true if found, false if not
     */
    public function is_related($query) {
        ...
        $query = core_text::strtolower($query);
        $checkouts = checkout_get_plugins(false);
        foreach ($checkouts as $name=>$checkout) {
            $localised = get_string('pluginname', 'checkout_'.$name);
            if (strpos(core_text::strtolower($name), $query) !== false) {
                return true;
            }
            if (strpos(core_text::strtolower($localised), $query) !== false) {
    ...
    public function output_html($data, $query='') {
        ...
        $strdisable   = get_string('disable');
        $struninstall = get_string('uninstallplugin', 'core_admin');
        $strusage     = get_string('checkoutusage', 'checkout');
        $strversion   = get_string('version');
        $strtest      = get_string('testsettings', 'core_checkout');
		
        $pluginmanager = core_plugin_manager::instance();
		
        $checkouts_available = checkout_get_plugins(false);
        $active_checkouts    = checkout_get_plugins(true);
		
        $allcheckouts = array();
        foreach ($active_checkouts as $key=>$checkout) {
            $allcheckouts[$key] = true;
        }
        foreach ($checkouts_available as $key=>$checkout) {
            $allcheckouts[$key] = true;
        }
        // Now find all borked plugins and at least allow then to uninstall.
        $condidates = $DB->get_fieldset_sql("SELECT DISTINCT checkout FROM {checkout}");
        foreach ($condidates as $candidate) {
            if (empty($allcheckouts[$candidate])) {
                $allcheckouts[$candidate] = true;
            }
        }
		
        $return = $OUTPUT->heading(get_string('actcheckoutshhdr', 'checkout'), 3, 'main', true);
        $return .= $OUTPUT->box_start('generalbox checkoutsui');
		
        $table = new html_table();
        $table->head  = array(get_string('name'), $strusage, $strversion, $strenable, $strup.'/'.$strdown, $strsettings, $strtest, $struninstall);
        $table->colclasses = array('leftalign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign');
        $table->id = 'coursecheckingoutplugins';
        $table->attributes['class'] = 'admintable generaltable';
        $table->data  = array();
		
        // Iterate through checkout plugins and add to the display table.
        $updowncount = 1;
        $checkoutcount = count($active_checkouts);
        $url = new moodle_url('/admin/checkout.php', array('sesskey'=>sesskey()));
        $printed = array();
        foreach($allcheckouts as $checkout => $unused) {
            $plugininfo = $pluginmanager->get_plugin_info('checkout_'.$checkout);
            $version = get_config('checkout_'.$checkout, 'version');
            if ($version === false) {
                $version = '';
            }
			
            if (get_string_manager()->string_exists('pluginname', 'checkout_'.$checkout)) {
                $name = get_string('pluginname', 'checkout_'.$checkout);
            } else {
                $name = $checkout;
            }
            // Usage
...
//===================================================================================		
            $ci = $DB->count_records('checkout', array('checkout'=>$checkout));
            $cp = $DB->count_records_select('user_checkingouts', "checkoutid IN (SELECT id FROM {checkout} WHERE checkout = ?)", array($checkout));
            $usage = "$ci / $cp";
			//$usage = "$ci / 99";
//===================================================================================
//===================================================================================
...
//===================================================================================			
            // Hide/show links.
            $class = '';
            if (isset($active_checkouts[$checkout])) {
                $aurl = new moodle_url($url, array('action'=>'disable', 'checkout'=>$checkout));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"iconsmall\" alt=\"$strdisable\" /></a>";
                $enabled = true;
                $displayname = $name;
            } else if (isset($checkouts_available[$checkout])) {
                $aurl = new moodle_url($url, array('action'=>'enable', 'checkout'=>$checkout));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"iconsmall\" alt=\"$strenable\" /></a>";
                $enabled = false;
                $displayname = $name;
            ...
            if ($PAGE->theme->resolve_image_location('icon', 'checkout_' . $name, false)) {
                $icon = $OUTPUT->pix_icon('icon', '', 'checkout_' . $name, array('class' => 'icon pluginicon'));
            } else {
                $icon = $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'icon pluginicon noicon'));
            }
			
            // Up/down link (only if checkout is enabled).
            $updown = '';
            if ($enabled) {
                if ($updowncount > 1) {
                    $aurl = new moodle_url($url, array('action'=>'up', 'checkout'=>$checkout));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"$strup\" class=\"iconsmall\" /></a>&nbsp;";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $checkoutcount) {
                    $aurl = new moodle_url($url, array('action'=>'down', 'checkout'=>$checkout));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"$strdown\" class=\"iconsmall\" /></a>";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />";
            ...
            // Add uninstall info.
            $uninstall = '';
            if ($uninstallurl = core_plugin_manager::instance()->get_uninstall_url('checkout_'.$checkout, 'manage')) {
                $uninstall = html_writer::link($uninstallurl, $struninstall);
            }
			
            $test = '';
            if (!empty($checkouts_available[$checkout]) and method_exists($checkouts_available[$checkout], 'test_settings')) {
                $testsettingsurl = new moodle_url('/checkout/test_settings.php', array('checkout'=>$checkout, 'sesskey'=>sesskey()));
                $test = html_writer::link($testsettingsurl, $strtest);
            }
            ...
            $printed[$checkout] = true;
        }
		
        $return .= html_writer::table($table);
        $return .= get_string('configcheckoutplugins', 'checkout').'<br />'.get_string('tablenosave', 'admin');
        $return .= $OUTPUT->box_end();
        return highlight($query, $return);
    }
}
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_setting_manageordertots extends admin_setting {
    /**
     * Calls parent::__construct with specific arguments
     */
    public function __construct() {
        $this->nosave = true;
        parent::__construct('ordertotsui', get_string('manageordertots', 'ordertot'), '', '');
    }
    ...
    /**
     * Checks if $query is one of the available ordertot plugins
		*
     * @param string $query The string to search for
     * @return bool Returns true if found, false if not
     */
    public function is_related($query) {
        ...
        $query = core_text::strtolower($query);
        $ordertots = ordertot_get_plugins(false);
        foreach ($ordertots as $name=>$ordertot) {
            $localised = get_string('pluginname', 'ordertot_'.$name);
            if (strpos(core_text::strtolower($name), $query) !== false) {
                return true;
            }
            if (strpos(core_text::strtolower($localised), $query) !== false) {
    ...
    public function output_html($data, $query='') {
        ...
        $strdisable   = get_string('disable');
        $struninstall = get_string('uninstallplugin', 'core_admin');
        $strusage     = get_string('ordertotusage', 'ordertot');
        $strversion   = get_string('version');
        $strtest      = get_string('testsettings', 'core_ordertot');
		
        $pluginmanager = core_plugin_manager::instance();
		
        $ordertots_available = ordertot_get_plugins(false);
        $active_ordertots    = ordertot_get_plugins(true);
		
        $allordertots = array();
        foreach ($active_ordertots as $key=>$ordertot) {
            $allordertots[$key] = true;
        }
        foreach ($ordertots_available as $key=>$ordertot) {
            $allordertots[$key] = true;
        }
        // Now find all borked plugins and at least allow then to uninstall.
        $condidates = $DB->get_fieldset_sql("SELECT DISTINCT ordertot FROM {ordertot}");
        foreach ($condidates as $candidate) {
            if (empty($allordertots[$candidate])) {
                $allordertots[$candidate] = true;
            }
        }
		
        $return = $OUTPUT->heading(get_string('actordertotshhdr', 'ordertot'), 3, 'main', true);
        $return .= $OUTPUT->box_start('generalbox ordertotsui');
		
        $table = new html_table();
        $table->head  = array(get_string('name'), $strusage, $strversion, $strenable, $strup.'/'.$strdown, $strsettings, $strtest, $struninstall);
        $table->colclasses = array('leftalign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign');
        $table->id = 'courseordertotalplugins';
        $table->attributes['class'] = 'admintable generaltable';
        $table->data  = array();
		
        // Iterate through ordertot plugins and add to the display table.
        $updowncount = 1;
        $ordertotcount = count($active_ordertots);
        $url = new moodle_url('/admin/ordertot.php', array('sesskey'=>sesskey()));
        $printed = array();
        foreach($allordertots as $ordertot => $unused) {
            $plugininfo = $pluginmanager->get_plugin_info('ordertot_'.$ordertot);
            $version = get_config('ordertot_'.$ordertot, 'version');
            if ($version === false) {
                $version = '';
            }
			
            if (get_string_manager()->string_exists('pluginname', 'ordertot_'.$ordertot)) {
                $name = get_string('pluginname', 'ordertot_'.$ordertot);
            } else {
                $name = $ordertot;
            }
            // Usage
...
//=====this is only a cheap hack for now to get away from nonexisting database / function calls====
//===================================================================================		
            $ci = $DB->count_records('ordertot', array('ordertot'=>$ordertot));
            //$cp = $DB->count_records_select('user_ordertotals', "ordertotid IN (SELECT id FROM {ordertot} WHERE ordertot = ?)", array($ordertot));
            //$usage = "$ci / $cp";
			$usage = "$ci / 99";
//===================================================================================
//===================================================================================
...
//===================================================================================			
            // Hide/show links.
            $class = '';
            if (isset($active_ordertots[$ordertot])) {
                $aurl = new moodle_url($url, array('action'=>'disable', 'ordertot'=>$ordertot));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"iconsmall\" alt=\"$strdisable\" /></a>";
                $enabled = true;
                $displayname = $name;
            } else if (isset($ordertots_available[$ordertot])) {
                $aurl = new moodle_url($url, array('action'=>'enable', 'ordertot'=>$ordertot));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"iconsmall\" alt=\"$strenable\" /></a>";
                $enabled = false;
                $displayname = $name;
            ...
            if ($PAGE->theme->resolve_image_location('icon', 'ordertot_' . $name, false)) {
                $icon = $OUTPUT->pix_icon('icon', '', 'ordertot_' . $name, array('class' => 'icon pluginicon'));
            } else {
                $icon = $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'icon pluginicon noicon'));
            }
			
            // Up/down link (only if ordertot is enabled).
            $updown = '';
            if ($enabled) {
                if ($updowncount > 1) {
                    $aurl = new moodle_url($url, array('action'=>'up', 'ordertot'=>$ordertot));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"$strup\" class=\"iconsmall\" /></a>&nbsp;";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $ordertotcount) {
                    $aurl = new moodle_url($url, array('action'=>'down', 'ordertot'=>$ordertot));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"$strdown\" class=\"iconsmall\" /></a>";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />";
            ...
            // Add uninstall info.
            $uninstall = '';
            if ($uninstallurl = core_plugin_manager::instance()->get_uninstall_url('ordertot_'.$ordertot, 'manage')) {
                $uninstall = html_writer::link($uninstallurl, $struninstall);
            }
			
            $test = '';
            if (!empty($ordertots_available[$ordertot]) and method_exists($ordertots_available[$ordertot], 'test_settings')) {
                $testsettingsurl = new moodle_url('/ordertot/test_settings.php', array('ordertot'=>$ordertot, 'sesskey'=>sesskey()));
                $test = html_writer::link($testsettingsurl, $strtest);
            }
            ...
            $printed[$ordertot] = true;
        }
		
        $return .= html_writer::table($table);
        $return .= get_string('configordertotplugins', 'ordertot').'<br />'.get_string('tablenosave', 'admin');
        $return .= $OUTPUT->box_end();
        return highlight($query, $return);
    }
}
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_setting_managepaygates extends admin_setting {
    /**
     * Calls parent::__construct with specific arguments
     */
    public function __construct() {
        $this->nosave = true;
        parent::__construct('paygatesui', get_string('managepaygates', 'paygate'), '', '');
    }
    ...
    /**
     * Checks if $query is one of the available paygate plugins
		*
     * @param string $query The string to search for
     * @return bool Returns true if found, false if not
     */
    public function is_related($query) {
        ...
        $query = core_text::strtolower($query);
        $paygates = paygate_get_plugins(false);
        foreach ($paygates as $name=>$paygate) {
            $localised = get_string('pluginname', 'paygate_'.$name);
            if (strpos(core_text::strtolower($name), $query) !== false) {
                return true;
            }
            if (strpos(core_text::strtolower($localised), $query) !== false) {
    ...
    public function output_html($data, $query='') {
        ...
        $strdisable   = get_string('disable');
        $struninstall = get_string('uninstallplugin', 'core_admin');
        $strusage     = get_string('paygateusage', 'paygate');
        $strversion   = get_string('version');
        $strtest      = get_string('testsettings', 'core_paygate');
		
        $pluginmanager = core_plugin_manager::instance();
		
        $paygates_available = paygate_get_plugins(false);
        $active_paygates    = paygate_get_plugins(true);
		
        $allpaygates = array();
        foreach ($active_paygates as $key=>$paygate) {
            $allpaygates[$key] = true;
        }
        foreach ($paygates_available as $key=>$paygate) {
            $allpaygates[$key] = true;
        }
        // Now find all borked plugins and at least allow then to uninstall.
        $condidates = $DB->get_fieldset_sql("SELECT DISTINCT paygate FROM {paygate}");
        foreach ($condidates as $candidate) {
            if (empty($allpaygates[$candidate])) {
                $allpaygates[$candidate] = true;
            }
        }
		
        $return = $OUTPUT->heading(get_string('actpaygateshhdr', 'paygate'), 3, 'main', true);
        $return .= $OUTPUT->box_start('generalbox paygatesui');
		
        $table = new html_table();
        $table->head  = array(get_string('name'), $strusage, $strversion, $strenable, $strup.'/'.$strdown, $strsettings, $strtest, $struninstall);
        $table->colclasses = array('leftalign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign');
        $table->id = 'coursepaymentgatewayplugins';
        $table->attributes['class'] = 'admintable generaltable';
        $table->data  = array();
		
        // Iterate through paygate plugins and add to the display table.
        $updowncount = 1;
        $paygatecount = count($active_paygates);
        $url = new moodle_url('/admin/paygate.php', array('sesskey'=>sesskey()));
        $printed = array();
        foreach($allpaygates as $paygate => $unused) {
            $plugininfo = $pluginmanager->get_plugin_info('paygate_'.$paygate);
            $version = get_config('paygate_'.$paygate, 'version');
            if ($version === false) {
                $version = '';
            }
			
            if (get_string_manager()->string_exists('pluginname', 'paygate_'.$paygate)) {
                $name = get_string('pluginname', 'paygate_'.$paygate);
            } else {
                $name = $paygate;
            }
            // Usage
...
//=====this is only a cheap hack for now to get away from nonexisting database / function calls====
//===================================================================================		
            $ci = $DB->count_records('paygate', array('paygate'=>$paygate));
            //$cp = $DB->count_records_select('user_paymentgateways', "paygateid IN (SELECT id FROM {paygate} WHERE paygate = ?)", array($paygate));
            //$usage = "$ci / $cp";
			$usage = "$ci / 99";
//===================================================================================
//===================================================================================
...
//===================================================================================			
            // Hide/show links.
            $class = '';
            if (isset($active_paygates[$paygate])) {
                $aurl = new moodle_url($url, array('action'=>'disable', 'paygate'=>$paygate));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"iconsmall\" alt=\"$strdisable\" /></a>";
                $enabled = true;
                $displayname = $name;
            } else if (isset($paygates_available[$paygate])) {
                $aurl = new moodle_url($url, array('action'=>'enable', 'paygate'=>$paygate));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"iconsmall\" alt=\"$strenable\" /></a>";
                $enabled = false;
                $displayname = $name;
            ...
            if ($PAGE->theme->resolve_image_location('icon', 'paygate_' . $name, false)) {
                $icon = $OUTPUT->pix_icon('icon', '', 'paygate_' . $name, array('class' => 'icon pluginicon'));
            } else {
                $icon = $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'icon pluginicon noicon'));
            }
			
            // Up/down link (only if paygate is enabled).
            $updown = '';
            if ($enabled) {
                if ($updowncount > 1) {
                    $aurl = new moodle_url($url, array('action'=>'up', 'paygate'=>$paygate));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"$strup\" class=\"iconsmall\" /></a>&nbsp;";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $paygatecount) {
                    $aurl = new moodle_url($url, array('action'=>'down', 'paygate'=>$paygate));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"$strdown\" class=\"iconsmall\" /></a>";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />";
            ...
            // Add uninstall info.
            $uninstall = '';
            if ($uninstallurl = core_plugin_manager::instance()->get_uninstall_url('paygate_'.$paygate, 'manage')) {
                $uninstall = html_writer::link($uninstallurl, $struninstall);
            }
			
            $test = '';
            if (!empty($paygates_available[$paygate]) and method_exists($paygates_available[$paygate], 'test_settings')) {
                $testsettingsurl = new moodle_url('/paygate/test_settings.php', array('paygate'=>$paygate, 'sesskey'=>sesskey()));
                $test = html_writer::link($testsettingsurl, $strtest);
            }
            ...
            $printed[$paygate] = true;
        }
		
        $return .= html_writer::table($table);
        $return .= get_string('configpaygateplugins', 'paygate').'<br />'.get_string('tablenosave', 'admin');
        $return .= $OUTPUT->box_end();
        return highlight($query, $return);
    }
}
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_setting_manageprodinfos extends admin_setting {
    /**
     * Calls parent::__construct with specific arguments
     */
    public function __construct() {
        $this->nosave = true;
        parent::__construct('prodinfosui', get_string('manageprodinfos', 'prodinfo'), '', '');
    }
    ...
    /**
     * Checks if $query is one of the available prodinfo plugins
		*
     * @param string $query The string to search for
     * @return bool Returns true if found, false if not
     */
    public function is_related($query) {
        ...
        $query = core_text::strtolower($query);
        $prodinfos = prodinfo_get_plugins(false);
        foreach ($prodinfos as $name=>$prodinfo) {
            $localised = get_string('pluginname', 'prodinfo_'.$name);
            if (strpos(core_text::strtolower($name), $query) !== false) {
                return true;
            }
            if (strpos(core_text::strtolower($localised), $query) !== false) {
    ...
    public function output_html($data, $query='') {
        ...
        $strdisable   = get_string('disable');
        $struninstall = get_string('uninstallplugin', 'core_admin');
        $strusage     = get_string('prodinfousage', 'prodinfo');
        $strversion   = get_string('version');
        $strtest      = get_string('testsettings', 'core_prodinfo');
		
        $pluginmanager = core_plugin_manager::instance();
		
        $prodinfos_available = prodinfo_get_plugins(false);
        $active_prodinfos    = prodinfo_get_plugins(true);
		
        $allprodinfos = array();
        foreach ($active_prodinfos as $key=>$prodinfo) {
            $allprodinfos[$key] = true;
        }
        foreach ($prodinfos_available as $key=>$prodinfo) {
            $allprodinfos[$key] = true;
        }
        // Now find all borked plugins and at least allow then to uninstall.
        $condidates = $DB->get_fieldset_sql("SELECT DISTINCT prodinfo FROM {prodinfo}");
        foreach ($condidates as $candidate) {
            if (empty($allprodinfos[$candidate])) {
                $allprodinfos[$candidate] = true;
            }
        }
		
        $return = $OUTPUT->heading(get_string('actprodinfoshhdr', 'prodinfo'), 3, 'main', true);
        $return .= $OUTPUT->box_start('generalbox prodinfosui');
		
        $table = new html_table();
        $table->head  = array(get_string('name'), $strusage, $strversion, $strenable, $strup.'/'.$strdown, $strsettings, $strtest, $struninstall);
        $table->colclasses = array('leftalign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign');
        $table->id = 'courseproductinfoplugins';
        $table->attributes['class'] = 'admintable generaltable';
        $table->data  = array();
		
        // Iterate through prodinfo plugins and add to the display table.
        $updowncount = 1;
        $prodinfocount = count($active_prodinfos);
        $url = new moodle_url('/admin/prodinfo.php', array('sesskey'=>sesskey()));
        $printed = array();
        foreach($allprodinfos as $prodinfo => $unused) {
            $plugininfo = $pluginmanager->get_plugin_info('prodinfo_'.$prodinfo);
            $version = get_config('prodinfo_'.$prodinfo, 'version');
            if ($version === false) {
                $version = '';
            }
			
            if (get_string_manager()->string_exists('pluginname', 'prodinfo_'.$prodinfo)) {
                $name = get_string('pluginname', 'prodinfo_'.$prodinfo);
            } else {
                $name = $prodinfo;
            }
            // Usage
...
//=====this is only a cheap hack for now to get away from nonexisting database / function calls====
//===================================================================================		
            $ci = $DB->count_records('prodinfo', array('prodinfo'=>$prodinfo));
            $cp = $DB->count_records_select('user_productinfos', "prodinfoid IN (SELECT id FROM {prodinfo} WHERE prodinfo = ?)", array($prodinfo));
            $usage = "$ci / $cp";
			//$usage = "$ci / 99";
//===================================================================================
//===================================================================================
...
//===================================================================================			
            // Hide/show links.
            $class = '';
            if (isset($active_prodinfos[$prodinfo])) {
                $aurl = new moodle_url($url, array('action'=>'disable', 'prodinfo'=>$prodinfo));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"iconsmall\" alt=\"$strdisable\" /></a>";
                $enabled = true;
                $displayname = $name;
            } else if (isset($prodinfos_available[$prodinfo])) {
                $aurl = new moodle_url($url, array('action'=>'enable', 'prodinfo'=>$prodinfo));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"iconsmall\" alt=\"$strenable\" /></a>";
                $enabled = false;
                $displayname = $name;
            ...
            if ($PAGE->theme->resolve_image_location('icon', 'prodinfo_' . $name, false)) {
                $icon = $OUTPUT->pix_icon('icon', '', 'prodinfo_' . $name, array('class' => 'icon pluginicon'));
            } else {
                $icon = $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'icon pluginicon noicon'));
            }
			
            // Up/down link (only if prodinfo is enabled).
            $updown = '';
            if ($enabled) {
                if ($updowncount > 1) {
                    $aurl = new moodle_url($url, array('action'=>'up', 'prodinfo'=>$prodinfo));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"$strup\" class=\"iconsmall\" /></a>&nbsp;";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $prodinfocount) {
                    $aurl = new moodle_url($url, array('action'=>'down', 'prodinfo'=>$prodinfo));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"$strdown\" class=\"iconsmall\" /></a>";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />";
            ...
            // Add uninstall info.
            $uninstall = '';
            if ($uninstallurl = core_plugin_manager::instance()->get_uninstall_url('prodinfo_'.$prodinfo, 'manage')) {
                $uninstall = html_writer::link($uninstallurl, $struninstall);
            }
			
            $test = '';
            if (!empty($prodinfos_available[$prodinfo]) and method_exists($prodinfos_available[$prodinfo], 'test_settings')) {
                $testsettingsurl = new moodle_url('/prodinfo/test_settings.php', array('prodinfo'=>$prodinfo, 'sesskey'=>sesskey()));
                $test = html_writer::link($testsettingsurl, $strtest);
            }
            ...
            $printed[$prodinfo] = true;
        }
		
        $return .= html_writer::table($table);
        $return .= get_string('configprodinfoplugins', 'prodinfo').'<br />'.get_string('tablenosave', 'admin');
        $return .= $OUTPUT->box_end();
        return highlight($query, $return);
    }
}
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_setting_manageshippings extends admin_setting {
    /**
     * Calls parent::__construct with specific arguments
     */
    public function __construct() {
        $this->nosave = true;
        parent::__construct('shippingsui', get_string('manageshippings', 'shipping'), '', '');
    }
    ...
    /**
     * Checks if $query is one of the available shipping plugins
		*
     * @param string $query The string to search for
     * @return bool Returns true if found, false if not
     */
    public function is_related($query) {
        ...
        $query = core_text::strtolower($query);
        $shippings = shipping_get_plugins(false);
        foreach ($shippings as $name=>$shipping) {
            $localised = get_string('pluginname', 'shipping_'.$name);
            if (strpos(core_text::strtolower($name), $query) !== false) {
                return true;
            }
            if (strpos(core_text::strtolower($localised), $query) !== false) {
    ...
    public function output_html($data, $query='') {
        ...
        $strdisable   = get_string('disable');
        $struninstall = get_string('uninstallplugin', 'core_admin');
        $strusage     = get_string('shippingusage', 'shipping');
        $strversion   = get_string('version');
        $strtest      = get_string('testsettings', 'core_shipping');
		
        $pluginmanager = core_plugin_manager::instance();
		
        $shippings_available = shipping_get_plugins(false);
        $active_shippings    = shipping_get_plugins(true);
		
        $allshippings = array();
        foreach ($active_shippings as $key=>$shipping) {
            $allshippings[$key] = true;
        }
        foreach ($shippings_available as $key=>$shipping) {
            $allshippings[$key] = true;
        }
        // Now find all borked plugins and at least allow then to uninstall.
        $condidates = $DB->get_fieldset_sql("SELECT DISTINCT shipping FROM {shipping}");
        foreach ($condidates as $candidate) {
            if (empty($allshippings[$candidate])) {
                $allshippings[$candidate] = true;
            }
        }
		
        $return = $OUTPUT->heading(get_string('actshippingshhdr', 'shipping'), 3, 'main', true);
        $return .= $OUTPUT->box_start('generalbox shippingsui');
		
        $table = new html_table();
        $table->head  = array(get_string('name'), $strusage, $strversion, $strenable, $strup.'/'.$strdown, $strsettings, $strtest, $struninstall);
        $table->colclasses = array('leftalign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign');
        $table->id = 'courseshipmentplugins';
        $table->attributes['class'] = 'admintable generaltable';
        $table->data  = array();
		
        // Iterate through shipping plugins and add to the display table.
        $updowncount = 1;
        $shippingcount = count($active_shippings);
        $url = new moodle_url('/admin/shipping.php', array('sesskey'=>sesskey()));
        $printed = array();
        foreach($allshippings as $shipping => $unused) {
            $plugininfo = $pluginmanager->get_plugin_info('shipping_'.$shipping);
            $version = get_config('shipping_'.$shipping, 'version');
            if ($version === false) {
                $version = '';
            }
			
            if (get_string_manager()->string_exists('pluginname', 'shipping_'.$shipping)) {
                $name = get_string('pluginname', 'shipping_'.$shipping);
            } else {
                $name = $shipping;
            }
            // Usage
...
//=====this is only a cheap hack for now to get away from nonexisting database / function calls====
//===================================================================================		
            $ci = $DB->count_records('shipping', array('shipping'=>$shipping));
            //$cp = $DB->count_records_select('user_shipments', "shippingid IN (SELECT id FROM {shipping} WHERE shipping = ?)", array($shipping));
            //$usage = "$ci / $cp";
			$usage = "$ci / 99";
//===================================================================================
//===================================================================================
...
//===================================================================================			
            // Hide/show links.
            $class = '';
            if (isset($active_shippings[$shipping])) {
                $aurl = new moodle_url($url, array('action'=>'disable', 'shipping'=>$shipping));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"iconsmall\" alt=\"$strdisable\" /></a>";
                $enabled = true;
                $displayname = $name;
            } else if (isset($shippings_available[$shipping])) {
                $aurl = new moodle_url($url, array('action'=>'enable', 'shipping'=>$shipping));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"iconsmall\" alt=\"$strenable\" /></a>";
                $enabled = false;
                $displayname = $name;
            ...
            if ($PAGE->theme->resolve_image_location('icon', 'shipping_' . $name, false)) {
                $icon = $OUTPUT->pix_icon('icon', '', 'shipping_' . $name, array('class' => 'icon pluginicon'));
            } else {
                $icon = $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'icon pluginicon noicon'));
            }
			
            // Up/down link (only if shipping is enabled).
            $updown = '';
            if ($enabled) {
                if ($updowncount > 1) {
                    $aurl = new moodle_url($url, array('action'=>'up', 'shipping'=>$shipping));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"$strup\" class=\"iconsmall\" /></a>&nbsp;";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $shippingcount) {
                    $aurl = new moodle_url($url, array('action'=>'down', 'shipping'=>$shipping));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"$strdown\" class=\"iconsmall\" /></a>";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"iconsmall\" alt=\"\" />";
            ...
            // Add uninstall info.
            $uninstall = '';
            if ($uninstallurl = core_plugin_manager::instance()->get_uninstall_url('shipping_'.$shipping, 'manage')) {
                $uninstall = html_writer::link($uninstallurl, $struninstall);
            }
			
            $test = '';
            if (!empty($shippings_available[$shipping]) and method_exists($shippings_available[$shipping], 'test_settings')) {
                $testsettingsurl = new moodle_url('/shipping/test_settings.php', array('shipping'=>$shipping, 'sesskey'=>sesskey()));
                $test = html_writer::link($testsettingsurl, $strtest);
            }
            ...
            $printed[$shipping] = true;
        }
		
        $return .= html_writer::table($table);
        $return .= get_string('configshippingplugins', 'shipping').'<br />'.get_string('tablenosave', 'admin');
        $return .= $OUTPUT->box_end();
        return highlight($query, $return);
    }
}
...
class admin_setting_managestorinfos extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $table->head  = array(get_string('name'), $strusage, $strversion, $strenable, $strup.'/'.$strdown, $strsettings, $strtest, $struninstall);
        $table->colclasses = array('leftalign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign', 'centeralign');
        $table->id = 'coursestoremultiinfoplugins';
        $table->attributes['class'] = 'admintable generaltable';
        $table->data  = array();
